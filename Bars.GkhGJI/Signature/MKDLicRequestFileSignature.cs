namespace Bars.GkhGji.DomainService.Impl
{
    ...
    public class MKDLicRequestFileSignature : ISignature<MKDLicRequestFile>
    {
        private IFileManager _fileManager;
        private IDomainService<FileInfo> _fileDomain;

        public MKDLicRequestFileSignature(IFileManager fileManager, IDomainService<FileInfo> fileDomain)
        {
        ...
        public IDomainService<MKDLicRequestFile> DomainService { get; set; }
        ...
        {
            var pdfId = DomainService.Get(id).FileInfo.Id;
            byte[] data;
            if (pdfId == 0)
            {
                data = new byte[0];
