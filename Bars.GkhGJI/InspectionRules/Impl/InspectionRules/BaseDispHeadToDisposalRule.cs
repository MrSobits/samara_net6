namespace Bars.GkhGji.InspectionRules
{
    ...
    /// </summary>
    public class BaseDispHeadToDisposalRule : BaseDispHeadToDisposalBaseRule<Disposal>
    {
    }

    /// <summary>
    /// Базовый класс с функционалом для создания "Распоряжения" (или наследованного от него документа)
    /// на основе проверки по поручению руководства
    /// </summary>
    public class BaseDispHeadToDisposalBaseRule<T> : IInspectionGjiRule
        where T : Disposal, new()
    {
        ...
        public virtual string Id => "BaseDispHeadToDisposalRule";

        public string Description => "Правило создание документа Распоряжения из основания проверки по поручению руководства";

        public string ActionUrl => "B4.controller.Disposal";

        public virtual string ResultName => DisposalTextService.SubjectiveCase;

        public TypeBase TypeInspectionInitiator => TypeBase.DisposalHead;
