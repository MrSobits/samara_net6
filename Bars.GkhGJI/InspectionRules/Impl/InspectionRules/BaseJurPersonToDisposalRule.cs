namespace Bars.GkhGji.InspectionRules
{
    ...
    /// </summary>
    public class BaseJurPersonToDisposalRule : BaseJurPersonToDisposalBaseRule<Disposal>
    {
    }

    /// <summary>
    /// Базовый класс с функционалом для создания "Распоряжения" (или наследованного от него документа)
    /// из основания плановой проверки ЮЛ
    /// </summary>
    public class BaseJurPersonToDisposalBaseRule<T> : IInspectionGjiRule
        where T : Disposal, new()
    {
        ...
        public virtual string Id => "BaseJurPersonToDisposalRule";

        public string Description => $"Правило создание документа '{this.ResultName}' из основания плановой проверки ЮЛ";

        public string ActionUrl => "B4.controller.Disposal";

        public virtual string ResultName => this.DisposalTextService.SubjectiveCase;

        public TypeBase TypeInspectionInitiator => TypeBase.PlanJuridicalPerson;
        ...
        public virtual IDataResult ValidationRule(InspectionGji inspection)
        {
            ...
            */
            if (inspection != null)
            {
                ...
                {
                    return new BaseDataResult(false, $"По плановой проверке юр. лиц уже создано '{this.ResultName}'");
                }
            }
