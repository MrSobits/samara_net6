namespace Bars.GkhGji.Report
{
    ...
    public class JournalAppeals : BasePrintForm
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            var appealCitsQuery = this.AppealCitsService.FilterByActiveAppealCits(this.AppealCitizensViewDomain.GetAll(), x => x.State)
            ...
            var appealCitsWithRo = appealCitsQuery
                .Select(x => new
                {
                    x.Id,
                    x.DateFrom,
                    NumberGji = string.IsNullOrEmpty(x.NumberGji) ? x.Number : x.NumberGji,
                    x.Municipality,
                    x.Correspondent,
                    IsFile = x.FileId != null,
                    KindStatement = x.KindStatement.Name,
                    x.Description,
                    PreviousAppealCits = x.PreviousAppealCits.NumberGji ?? string.Empty,
                    ManagingOrganization = x.ContragentName ?? string.Empty,
            ...
            {
                var appealCitsWithRoIdsQuery = appealCitsQuery.Select(x => x.Id);
