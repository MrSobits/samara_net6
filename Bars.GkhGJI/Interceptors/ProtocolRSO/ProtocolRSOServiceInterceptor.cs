namespace Bars.GkhGji.Interceptors
{
    ...
    public class ProtocolRSOServiceInterceptor : ProtocolRSOServiceInterceptor<ProtocolRSO>
    {
        // доработки вносить в Genetric
    }

    public class ProtocolRSOServiceInterceptor<T> : DocumentGjiInterceptor<T>
        where T : ProtocolRSO
    {
        ...
        {
            var domainServiceInspection = Container.Resolve<IDomainService<BaseProtocolRSO>>();
            ...
            try
            {
                ...
                var newInspection = new BaseProtocolRSO()
                {
                    TypeBase = TypeBase.ProtocolRSO
                ...
                var newStage = new InspectionGjiStage
                {
                    ...
                    // TODO: Разобраться нужно ли это значение в enum
                    //TypeStage = TypeStage.ProtocolRSO,
                    Position = 0,
                    ObjectVersion = 0,
        ...
        {
            var annexService = this.Container.Resolve<IDomainService<ProtocolRSOAnnex>>();
            var lawService = this.Container.Resolve<IDomainService<ProtocolRSOArticleLaw>>();
            var definitionService = this.Container.Resolve<IDomainService<ProtocolRSODefinition>>();
            var roService = Container.Resolve<IDomainService<ProtocolRSORealityObject>>();
            ...
            try
            {
                ...
                var refFuncs = new List<Func<long, string>>
                               {
                                  id => annexService.GetAll().Any(x => x.ProtocolRSO.Id == id) ? "Приложения" : null,
                                  id => lawService.GetAll().Any(x => x.ProtocolRSO.Id == id) ? "Статьи закона" : null,
                                  id => definitionService.GetAll().Any(x => x.ProtocolRSO.Id == id) ? "Определения" : null
                               };
                ...
                var roIds = roService.GetAll().Where(x => x.ProtocolRSO.Id == entity.Id).Select(x => x.Id).ToList();
        ...
        {
            var baseService = Container.Resolve<IDomainService<BaseProtocolRSO>>();
        ...
        private Dictionary<string, string> GetPropertyValues()
        {
            var result = new Dictionary<string, string>
            {
                ...
                { "TypeSupplierProtocol", "Тип РСО составившей протокол" },
                { "Contragent", "Контрагент" },
                { "PhysicalPerson", "Физическое лицо" },
                { "PhysicalPersonInfo", "Реквизиты физ.лица" },
                { "GasSupplier", "Газоснабжающая организация, вынесшая протокол" },
