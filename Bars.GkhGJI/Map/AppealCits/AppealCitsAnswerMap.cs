namespace Bars.GkhGji.Map
{
    ...
    public class AppealCitsAnswerMap : BaseEntityMap<AppealCitsAnswer>
    {
        public AppealCitsAnswerMap() : base("Ответ по обращению", "GJI_APPCIT_ANSWER")
        {
        }
        ...
        protected override void Map()
        {
            ...
            this.Property(x => x.IsSent, "Ответ направлен").Column("IS_SENT");

            this.Reference(x => x.AppealCits, "Проверка").Column("APPCIT_ID").NotNull().Fetch();
            ...
            Property(x => x.Hash, "Hash").Column("HASH").Length(36);
            ...
            Reference(x => x.SignedFile, "Подписанный файл").Column("SIGN_FILE").Fetch();
            Property(x => x.Registred, "Зарегестрирован").Column("REGISTRED");
            Property(x => x.Sended, "Отправлен").Column("SENDED");
            Property(x => x.SendedToEdm, "Отправлен в СЭД").Column("SENDED_TO_EDM");
        }
    }
}
