namespace Bars.GkhGji.Controllers
{
    ...
    public class ActCheckDefinitionController : ActCheckDefinitionController<ActCheckDefinition>
    {
        public ActCheckDefinitionController(IFileManager fileManager, IDomainService<FileInfo> fileDomain) : base(fileManager, fileDomain)
        {
        }
    }

    public class ActCheckDefinitionController<T> : FileStorageDataController<T>
        where T : ActCheckDefinition
    {
        private IFileManager _fileManager;
        private IDomainService<FileInfo> _fileDomain;
        protected Bitmap stamp = Properties.Resources.stamp2;

        public ActCheckDefinitionController(IFileManager fileManager, IDomainService<FileInfo> fileDomain)
        {
            ...
            using (var tr = Resolve<IDataTransaction>())
            {
                ...
                try
                {
                    ...
                    if (string.Compare(entity.File.Extention, "docx", true) == 0 || string.Compare(entity.File.Extention, "doc", true) == 0)
                    {
                        ...
                        if (string.Compare(entity.File.Extention, "docx", true) == 0)
                        {
