namespace Bars.GkhGji.Controllers
{
   ...
   /* using CryptoPro.Sharpei;
    ...
    public class DisposalAnnexController : DisposalAnnexController<DisposalAnnex>
    {
    ...
    public class DisposalAnnexController<T> : FileStorageDataController<T>
        where T : DisposalAnnex
    {
        ...
        public DisposalAnnexController(IFileManager fileManager, IDomainService<FileInfo> fileDomain)
        {
        ...
        {
            var docIo = Resolve<IDocIo>();
            Stream file = new FileStream("C:\\Users\\Admin\\Downloads\\report.pdf", FileMode.Open, FileAccess.Read);
            MemoryStream docMemStream = new MemoryStream();
            docIo.ConvertToDoc(file).CopyTo(docMemStream);
            return File(docMemStream, "application/msword");
        }
