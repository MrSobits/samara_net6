namespace Bars.GkhGji.Controllers
{
    ...
    public class MKDLicRequestFileController : MKDLicRequestFileController<MKDLicRequestFile>
    {
        public MKDLicRequestFileController(IFileManager fileManager, IDomainService<FileInfo> fileDomain) : base(fileManager, fileDomain)
        {
        }
    }

    public class MKDLicRequestFileController<T> : FileStorageDataController<T>
        where T : MKDLicRequestFile
    {
        private IFileManager _fileManager;
        private IDomainService<FileInfo> _fileDomain;
        protected Bitmap stamp = Properties.Resources.stamp2;

        public MKDLicRequestFileController(IFileManager fileManager, IDomainService<FileInfo> fileDomain)
        {
        ...
        public ActionResult GetXML(BaseParams baseParams)
        {
            ...
            var fileInfo = DomainService.Get(pdfId).FileInfo;

            var fileId = DomainService.Get(pdfId).FileInfo.Id;
        ...
        public ActionResult Sign(BaseParams baseParams)
        {
            ...
            using (var tr = Resolve<IDataTransaction>())
            {
                ...
                try
                {
                    ...
                    var fileName = DomainService.Get(xmlId).FileInfo.Name;

                    // если исходник - Word
                    if (string.Compare(entity.FileInfo.Extention, "docx", true) == 0 || string.Compare(entity.FileInfo.Extention, "doc", true) == 0)
                    {
                        Stream docStream = _fileManager.GetFile(entity.FileInfo);
                    ...
                    // если исходник - pdf
                    else if (string.Compare(entity.FileInfo.Extention, "pdf", true) == 0)
                    {
                        Stream fileStream = _fileManager.GetFile(entity.FileInfo);
                        var signedStream = new MemoryStream();
