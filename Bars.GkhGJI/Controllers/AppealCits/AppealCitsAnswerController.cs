namespace Bars.GkhGji.Controllers
{
    ...
    public class AppealCitsAnswerController : AppealCitsAnswerController<AppealCitsAnswer>
    {
    ...
    public class AppealCitsAnswerController<T> : FileStorageDataController<T>
        where T : AppealCitsAnswer
    {
        protected IFileManager _fileManager;
        protected IDomainService<FileInfo> _fileDomain;
        protected Bitmap stamp = Properties.Resources.stamp2;
        protected Image blazon;

        public AppealCitsAnswerController(IFileManager fileManager, IDomainService<FileInfo> fileDomain)
        {
                ...
                try
                {
                    ...
                    if (string.Compare(entity.File.Extention, "docx", true) == 0 || string.Compare(entity.File.Extention, "doc", true) == 0)
                    {
                        ...
                        if (docGen.TrySetPicture("Џ", picStrm, 225, 99))
                        {
                            ...
                            entity.SignFile = _fileManager.SaveFile(fileName + "_signed", "pdf", signedStream.ToArray());
                        }
                        //если нет маркера
                        else
                        {
