Ext.define('B4.controller.manorglicense.EditRequest', {
    ...
    requires: [
        ...
        'B4.view.manorglicense.ManOrgRequestSMEVGrid',
        'B4.view.manorglicense.TypeRequestSetWindow',
            ...
            listeners: {
                ...
                aftersetpaneldata: function(asp, rec, panel) {
                    var me = this,
                        ...
                        rpguGrid = panel.down('manorglicenserequestrpgugrid'),
                        rpguStore = rpguGrid.getStore(),
                        smevGrid = panel.down('manorglicenserequestsmevgrid'),
                        smevStore = smevGrid.getStore(),
                    ...
                    rpguStore.clearFilter(true);
                    rpguStore.filter('requestId', rec.getId());

                    smevStore.clearFilter(true);
                    smevStore.filter('requestId', rec.getId());
    ...
    init: function() {
        ...
        actions['manOrgLicenseRequestEditPanel manorglicenserequestsmevgrid b4updatebutton']
            = { 'click': { fn: me.updateSMEVGrid, scope: me } };

        actions['manOrgLicenseRequestEditPanel manorglicenserequestsmevgrid b4addbutton']
            = { 'click': { fn: me.addSMEVRequest, scope: me } };

        actions['#manorglicenseTypeRequestSetWindow #btnSetTypeRequest']
            = { 'click': { fn: me.sendSMEVRequest, scope: me } };

        actions['manorglicenserequestsmevgrid b4deletecolumn']
            = {
                'click': {
        ...
        actions['manorglicenserequestsmevgrid b4editcolumn']
            = {
                'click': {
                    fn: function (grid, b, t, y, r, record) {
                        var me2 = this,
                            requestSMEVType = record.get('RequestSMEVType'),
                            portal = me.getController('PortalController'),
                            params = {};
                        switch (requestSMEVType) {
                            case 20:
                                {

                                    controllerEditName = 'B4.controller.SMEVEGRUL';
                                    params.reqId = record.get('RequestId');
                            ...
                            case 30:
                                {
                                    ...
                                    controllerEditName = 'B4.controller.SMEVEGRIP';
                                    params.reqId = record.get('RequestId');
                                    portal.loadController(controllerEditName, params);
    ...
    updateSMEVGrid: function(btn) {
        btn.up('manorglicenserequestsmevgrid').getStore().load();
    },

    addSMEVRequest: function(btn) {
     reqinfo = Ext.create('B4.view.manorglicense.TypeRequestSetWindow');
        reqinfo.show();
    },

    sendSMEVRequest: function (btn) {
        var me = this,
            view = me.getMainView();
        var docId = view.params.requestId;
        var window = btn.up('#manorglicenseTypeRequestSetWindow'),
            chosenOne = window.down('#cbRequestSMEVType').getValue();
        me.mask('Установка плановой даты устранения нарушения', this.getMainComponent());
        var result = B4.Ajax.request(B4.Url.action('AddSMEVRequest', 'ManOrgLicenseRequest', {
            requestType: chosenOne, requestId: docId
        }))
            .next(function (response) {
                me.unmask();
                var data = Ext.decode(response.responseText);
