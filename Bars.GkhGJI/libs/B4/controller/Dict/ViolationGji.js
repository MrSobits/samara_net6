Ext.define('B4.controller.dict.ViolationGji', {
    ...
    aspects: [
        ...
        },
        {
            ...
            onSaveSuccess: function (asp, record) {
                ...
                B4.Ajax.request({
                    url: B4.Url.action('SaveNormativeDocItems', 'ViolationNormativeDocItemGji'),
                    params: {
                    ...
                    docItemsGrid.down('b4addbutton').enable();
                    ndiStore.removeAll();
                    ndiStore.load();
                });
            ...
            listeners: {
                aftersetformdata: function (asp, record) {
                    ...
                    var docItemGridBtn = Ext.ComponentQuery.query('#violationNormativeDocItemGrid b4addbutton')[0];
                    docItemGridBtn.setDisabled(!record.getId());
    ...
    init: function () {
        ...
        me.control({
            ...
            "#violationGroupsGjiGrid b4updatebutton": {
                click: function(btn) {
                    var store = btn.up('violationGroupsGjiGrid').getStore();
                    store.load();
                }
            }
        });
    ...
    onItemChange: function (field, newValue) {
        ...
        me.getStore('dict.NormativeDocItem').load(
        {
            ...
            callback: function (records, operation, success) {
                ...
                var dataStore = me.getStore('dict.ViolationNormativeDocItemGji');
                ...
                for (var index2 = 0; index2 < data.length; index2++) {
                    for (var index = 0; index < records.length; index++) {
                        ...
                        if (records[index].data.Id == newValue.id) {
                        ...
                        dataStore.add({
                            NormativeDocItem: item.data.Id,
                            NormativeDocName: item.data.NormativeDocName,
                            NormativeDocItemName: item.data.Number,
                            NormativeDoc: item.data.NormativeDoc
                        });
                    }
                }
            }
