Ext.define('B4.controller.Disposal', {
    ...
    views: [
        ...
        'disposal.DisposalControlMeasures',
        'disposal.ControlMeasuresEditWindow',
    ...
    aspects: [
        {
            ...
            */
            ...
            onValidateUserParams: function (params) {
                ...
                if (params.ruleId == 'DisposalToActCheckRule') {
                    var has = this.controller.params.hasChildActCheck;
                    if (has == true) {
                        Ext.Msg.alert('Ошибка!', 'Невозможно сформировать акт проверки, так как уже существует \'Акт проверки (общий)\' или \'Акт проверки (на 1 дом)\'.');
                        return false;
                    }
                }
                return true;
        ...
        },
        {
            ...
            */
            ...
            onAfterSetPanelData: function(asp, rec, panel) {
                ...
                this.controller.getAspect('disposalStateButtonAspect').setStateData(rec.get('Id'), rec.get('State'));

                // обновляем отчеты
                this.controller.getAspect('disposalPrintAspect').loadReportStore();
...
/* 
        ...
        {
            /*
             * Аспект взаимодействия Таблицы Мероприятия по контролю с формой редактирования
             */
            xtype: 'grideditwindowaspect',
            name: 'disposalControlMeasuresAspect',
            gridSelector: '#disposalControlMeasuresGrid',
            editFormSelector: '#controlMeasuresEditWindow',
            storeName: 'disposal.DisposalControlMeasures',
            modelName: 'disposal.DisposalControlMeasures',
            editWindowView: 'disposal.ControlMeasuresEditWindow',
            listeners: {
                getdata: function (asp, record) {
                    if (!record.get('Id')) {
                        record.set('Disposal', this.controller.params.documentId);
    ...
    init: function() {
        ...
        this.getStore('disposal.DisposalControlMeasures').on('beforeload', this.onBeforeLoad, this);
    ...
    onLaunch: function() {
        if (this.params) {
            this.getAspect('disposalEditPanelAspect').setData(this.params.documentId);

            //Обновляем таблицу Экспертов
            this.getStore('disposal.Expert').load();

            //Обновляем таблицу Предоставляемых документов
            this.getStore('disposal.ProvidedDoc').load();

            //Обновляем таблицу Приложения
            this.getStore('disposal.Annex').load();

            //обновляем таблицу типов обследования
            this.getStore('disposal.TypeSurvey').load();
            
            //Обновляем таблицу Мероприятия по контролю
            this.getStore('disposal.DisposalControlMeasures').load();
        }
    },
