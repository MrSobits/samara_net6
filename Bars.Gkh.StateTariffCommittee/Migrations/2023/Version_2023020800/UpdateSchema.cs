namespace Bars.Gkh.StateTariffCommittee.Migrations._2023.Version_2023020800
{
    using Bars.B4.Modules.Ecm7.Framework;
    using Bars.Gkh.SqlExecutor;

    [Migration("2023020800")]
    ...
    public class UpdateSchema : Migration
    {
        ...
        public override void Up()
        {
	        ...
	        {
		        sqlExecutor.ExecuteSql($"CREATE SCHEMA IF NOT EXISTS tariff;");
...
$function$;");
		        ...
		        #region Метод с параметрами, функция по помещениям
				sqlExecutor.ExecuteSql($@"
CREATE OR REPLACE FUNCTION tariff.get_main_data_premise(table_name varchar)
...
$function$;");
		        ...
		        #region Метод с параметрами, функция по комнатам
				sqlExecutor.ExecuteSql($@"
CREATE OR REPLACE FUNCTION tariff.get_main_data_room(table_name varchar)
...
$function$;");
		        ...
		        #region Метод с параметрами, функция по параметрам
				sqlExecutor.ExecuteSql($@"
CREATE OR REPLACE FUNCTION tariff.get_parameters_object(table_name varchar, date_s date)
		...
		EXECUTE '
			...
			CREATE TEMP TABLE report_month_prms AS (
				...
				WHERE NOT lap.is_del AND lap.report_month = ''' || date_s || '''::date
					AND EXISTS (SELECT FROM tariff.' || table_name || ' tt WHERE tt.ls_account_id = lap.ls_account_id)
			);
		';
...
$function$;");
		        ...
		        #region Функция основного метода
		        sqlExecutor.ExecuteSql($@"
CREATE OR REPLACE FUNCTION tariff.get_consumer_full_address(table_name varchar)
...
$function$;");
		        ...
		        sqlExecutor.ExecuteSql($@"
					DROP FUNCTION IF EXISTS public.tariff_get_ls_account_info_table_name(oktmo_mo varchar, date_s date);
					DROP FUNCTION IF EXISTS public.tariff_get_information_house(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_get_information_premise(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_get_information_room(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_get_information_tariff(table_name varchar, date_s date);
					DROP FUNCTION IF EXISTS public.tariff_main_data_house(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_main_data_premise(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_main_data_room(table_name varchar);
					DROP FUNCTION IF EXISTS public.tariff_parameters_object(table_name varchar, date_s date);
					DROP FUNCTION IF EXISTS public.tariff_get_consumer_full_address(table_name varchar);
				");
	        }
        }
...
$function$;");
		        ...
		        sqlExecutor.ExecuteSql($@"
					DROP FUNCTION IF EXISTS tariff.create_ls_account_info_table(table_name varchar, oktmo_mo character varying, date_s date);
					...
					DROP FUNCTION IF EXISTS tariff.get_main_data_house(table_name varchar);
					DROP FUNCTION IF EXISTS tariff.get_main_data_premise(table_name varchar);
					DROP FUNCTION IF EXISTS tariff.get_main_data_room(table_name varchar);
					DROP FUNCTION IF EXISTS tariff.get_parameters_object(table_name varchar, date_s date);
					DROP FUNCTION IF EXISTS tariff.get_consumer_full_address(table_name varchar);

					DROP SCHEMA IF EXISTS tariff;
				");
	        }
        }
    }
