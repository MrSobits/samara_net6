        ...
        function cL(c5) {
            ...
            if (y(c5, "shiftKey")) {
                c4 = l("Shift-" + c0, b1.extraKeys, b1.keyMap, function (c6) {
                    ...
                    if (typeof c6 == "string" && /^go[A-ZА-Я]/.test(c6)) {
                        return bi(c6)
                    }
                }, c1)
            } else {
        ...
        afterrender: function (b) {
            if (this.menu) {
                b.el.addListener("click", function (d, a) {
                    this.menu.showBy(this.el, "bl", [50, 0])
                }, this)
            }
        }
    }
...
Ext.define("Docs.store.Search", {
    ...
    fields: ["display_name", "name", "fullName", "icon", "url", "meta", "sort"],
    proxy: {
        type: "memory",
        reader: {
            type: "json"
...
displayResults: function (d) {
    ...
    this.getDropdown().alignTo("search-field", "bl", [0, 5]);
    if (d.length > 0) {
...
Ext.define("Docs.view.Tabs", {
    ...
    initComponent: function () {
        ...
        this.tpl = Ext.create("Ext.XTemplate", '<tpl for=".">', '<div class="doctab overview {cls}{active}">', '<div class="m">', '<tpl if="text">', '<a class="tabUrl ov-tab-text" href="{href}">{text}</a>', "<tpl else>", '<a class="tabUrl ov-tab" href="{href}">&nbsp;</a>', "</tpl>", "</div>", "</div>", "</tpl>", '<div style="float: left; width: 8px">&nbsp;</div>', '<div class="tab-overflow"></div>');
        this.html = this.tpl.applyTemplate(this.staticTabs);
        this.tabTpl = Ext.create("Ext.XTemplate", '<div class="doctab', '{[values.active ? (" active") : ""]}', '" style="', '{[values.width ? ("width: " + values.width + "px;") : ""]}', '{[values.visible ? "" : "visibility: hidden;"]}">', '<div class="m">', '<span class="icn {iconCls}">&nbsp;</span>', '<a class="tabUrl main-tab" href="{href}">{text}</a>', "</div>", '<a class="close" href="#">&nbsp;</a>', "</div>");
    ...
    initListeners: function () {
        ...
        this.el.on("mousedown", function (f, d) {
            if (f.button == 1) {
                f.preventDefault();
                var e = Ext.get(d).findParent(".doctab", 50, true).down(".tabUrl").getAttribute("href");
                e = Docs.History.cleanUrl(e);
                this.removeTab(e);
                this.fireEvent("tabClose", e)
            }
...
Ext.define("Docs.view.cls.Header", {
    ...
    initComponent: function () {
        this.tpl = Ext.create("Ext.XTemplate",
            ...
            '<a href="#" class="class-source-link">{display_name}',
            '<span class="class-source-tip">View source...</span>',
            '</a>',
            '<tpl else>',
            '<strong class="class-source-link">{display_name}</strong>',
            '</tpl>',
            '<tpl if="singleton">',
...
Ext.define("Docs.view.cls.PackageLogic", {
    ...
    classNode: function (b) {
        return {
            text: Docs.ClassRegistry.shortName(b.display_name|| b.name),
            url: "#!/api/" + b.name,
            iconCls: b.icon,
...
Ext.define("Docs.view.cls.Tree", {
    ...
    initComponent: function () {
        ...
        this.dockedItems = [
        {
            ...
            dock: "top",
            cls: "cls-grouping",
            html: [this.makeButtonHtml("PackageLogic", "By Namespace"), this.makeButtonHtml("InheritanceLogic", "By Inheritance")].join("")
        },
        {
            xtype: "container",
            dock: "bottom",
...
createLinkRecord: function (d, c) {
    return {
        id: c.id,
        url: d + "-" + c.id,
        label: (c.tagname === "method" && c.name === "constructor") ? "new " + d : (c.display_name||c.name),
        inherited: c.owner !== d,
    ...
    (function () {
        function h(J) {
            ...
            function L(Q) {
                ...
                if (Q.ignoreCase && y) {
                    for (P = 0; P < S; ++P) {
                        M = N[P], Q = M.charAt(0), M.length >= 2 && Q === "[" ? N[P] = I(M) : Q !== "\\" && (N[P] = M.replace(/[A-ZА-Яa-zа-я]/g, function (T) {
                            T = T.charCodeAt(0);
        ...
        function u(v) {
            ...
            p = "^.[^\\s\\w.$@'\"`/\\\\]*";
            v.regexLiterals && (p += "(?!s*/)");
            C.push(["lit", /^@[$_a-zа-я][\w$@]*/i, a], ["typ", /^(?:[@_]?[A-ZА-Я]+[a-zа-я][\w$@]*|\w+_t\b)/, a], ["pln", /^[$_a-zа-я][\w$@]*/i, a], ["lit", /^(?:0x[\da-f]+|(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d\+)(?:e[+-]?\d+)?)[a-z]*/i, a, "0123456789"], ["pln", /^\\[\S\s]?/, a], ["pun", RegExp(p), a]);
            return A(D, C)
        }
