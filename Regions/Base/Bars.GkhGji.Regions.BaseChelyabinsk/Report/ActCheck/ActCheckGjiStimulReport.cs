...
#warning разобраться с этой фигней
        ...
        private void FillActCheckViolations()
        {
            ...
            try
            {
                ...
                if (actRobjects.Length > 0)
                {
                    ...
                    this.ReportParams["Описание"] = this.GetViolationDescription(firstRecord.Id);
        ...
        private ChelyabinskDisposal FillParentDisposal(ChelyabinskActCheck act)
        {
            ...
            try
            {
                ...
                if (!disposal.KindCheck.Return(x => x.Name).IsEmpty())
                {
                    ...
                    this.ReportParams["ВидОбследованияРП"] = this.GetMorpher().Проанализировать(disposal.KindCheck.Name).Родительный;
                }
        ...
        private void FillInspection(InspectionGji inspection)
        {
            ...
            try
            {
                ...
                {
                    this.ReportParams["ФИОФизлицаРП"] = this.GetMorpher().Проанализировать(inspection.PhysicalPerson).Родительный;
                }
                ...
                else
                {
                    ...
                    {
                        this.ReportParams["ЮрАдресКонтрагента"] = this.GetAddress(contragent.FiasJuridicalAddress);
                    }
                    else
                    {
