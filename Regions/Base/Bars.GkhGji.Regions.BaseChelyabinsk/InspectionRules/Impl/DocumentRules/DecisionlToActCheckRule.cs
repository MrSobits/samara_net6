namespace Bars.GkhGji.Regions.BaseChelyabinsk.InspectionRules.Impl.DocumentRules
{
    ...
    /// </summary>
    public class DecisionlToActCheckRule : IDocumentGjiRule
    {
        #region injection
        ...
        {
            get { return "DecisionlToActCheckRule"; }
        }

        public virtual string Description
        {
            get { return "Правило создание документа 'Акт проверки' из документа 'Решение' (общий)"; }
        }
        ...
        public virtual IDataResult CreateDocument(DocumentGji document)
        {
            ...
            var DecisionControlListDomain = this.Container.Resolve<IDomainService<DecisionControlList>>();
            var ControlListQuestionDomain = this.Container.Resolve<IDomainService<ControlListQuestion>>();
            var ActCheckProvDocDomain = this.Container.Resolve<IDomainService<ActCheckProvidedDoc>>();
            var ActCheckControlListAnswerDomain = this.Container.Resolve<IDomainService<ActCheckControlListAnswer>>();
            ...
            try
            {
                ...
                var actCheck = new ChelyabinskActCheck
                                   {
                                       ...
                                       TypeCheckAct = decision.KindCheck.Code
                };
                ...
                #region Формируем проверочные листы
                var listControlListAnswerToSave = new List<ActCheckControlListAnswer>();
                var ctListids = DecisionControlListDomain.GetAll()
                    .Where(x => x.Decision.Id == document.Id)
                    .Select(x => x.ControlList.Id)
                    .Distinct()
                    .ToList();

                foreach (var newId in ctListids)
                {
                    ControlListQuestionDomain.GetAll()
                        .Where(x => x.ControlList.Id == newId)
                        .ToList()
                        .ForEach(x =>
                        {
                            listControlListAnswerToSave.Add(new ActCheckControlListAnswer
                            {
                                ActCheck = actCheck,
                                ControlListQuestion = x,
                                NpdName = x.NPDName,
                                Question = x.Name,
                                YesNoNotApplicable = Gkh.Enums.YesNoNotApplicable.NotApplicable
                            });
                        });
                ...
                using (var tr = this.Container.Resolve<IDataTransaction>())
                {
                    try
                    {
                        ...
                        listControlListAnswerToSave.ForEach(ActCheckControlListAnswerDomain.Save);
