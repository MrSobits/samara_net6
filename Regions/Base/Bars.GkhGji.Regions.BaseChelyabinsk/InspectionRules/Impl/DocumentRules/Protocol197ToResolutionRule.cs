    ...
    public class Protocol197ToResolutionRule : IDocumentGjiRule
    {
        ...
        public IDataResult CreateDocument(DocumentGji document)
        {
            ...
            var resolPros = Protocol197Domain.GetAll()
                                    ...
                                    .Select(x => new { x.Executant, x.Contragent, x.PhysicalPerson, x.PhysicalPersonInfo, x.PhysicalPersonDocType,
                                    x.PhysicalPersonDocumentNumber, x.PhysicalPersonDocumentSerial, x.PhysicalPersonIsNotRF, x.PhysicalPersonPosition, x.PersonPosition })
                                    .FirstOrDefault();

            if (resolPros == null)
            {
                throw new Exception("Неудалось получить протокол 19.7");
            }
