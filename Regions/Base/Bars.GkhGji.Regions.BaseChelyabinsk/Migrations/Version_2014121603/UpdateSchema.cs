namespace Bars.GkhGji.Regions.BaseChelyabinsk.Migrations.Version_2014121603
{
    ...
    [global::Bars.B4.Modules.Ecm7.Framework.MigrationDependsOn(typeof(global::Bars.GkhGji.Regions.BaseChelyabinsk.Migrations.Version_2014121602.UpdateSchema))]
    public class UpdateSchema : global::Bars.B4.Modules.Ecm7.Framework.Migration
    {
        ...
        {
            var maxId = this.GetNextId("GJI_DICT_SURVEY_SUBJ");

            var insert = new StringBuilder();
            var codeName = new Dictionary<string, string>();
            using (var reader = this.Database.ExecuteQuery(@"select distinct(TYPE_VERIF_SUBJ) as type,
            ...
            if (insert.Length > 0)
            {
                this.Database.ExecuteNonQuery(insert.ToString());
            }
        }

        public override void Down()
        {
            this.Database.RemoveColumn("GJI_NSO_DISP_VERIFSUBJ", "SURVEY_SUBJECT_ID");
        }

        private long GetNextId(string tableName)
        {
            var maxId = 0L;
            using (var idReader = this.Database.ExecuteQuery(string.Format("SELECT MAX(ID) FROM {0}", tableName)))
            {
                if (idReader.Read())
                {
