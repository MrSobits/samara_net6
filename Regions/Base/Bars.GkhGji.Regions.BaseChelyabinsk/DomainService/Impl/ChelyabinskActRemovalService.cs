namespace Bars.GkhGji.Regions.BaseChelyabinsk.DomainService.Impl
{
	...
	/// </summary>
	public class ChelyabinskActRemovalService : IChelyabinskActRemovalService
	{
		...
		private readonly IDomainService<ChelyabinskActRemoval> actDomain;
        ...
        public ChelyabinskActRemovalService(
            IDomainService<ChelyabinskActRemoval> actDomain,
		...
		public IDataResult MergeActs(BaseParams baseParams)
	    {
		    ...
		    if (disposal == null)
		    {
				return new BaseDataResult(false, "Не найден родительское распоряжение");
			}

			var prescription = Utils.Utils.GetParentDocumentByType(this.documentGjiChildrenDomain, targetAct, TypeDocumentGji.Prescription);
			...
			var sourceActs = this.documentGjiChildrenDomain.GetAll()
			    ...
			    .Where(x => x.Children.State.Code == ChelyabinskActRemovalService.ForProcessingStateCode)//Статус = "К обработке"
		...
		private void ChangeState(ActRemoval targetAct, ICollection<ActRemoval> listSourceActs)
		{
			var newState = this.stateDomain.GetAll()
				.FirstOrDefault(x => x.Code == ChelyabinskActRemovalService.ForProcessedStateCode);//Обработан
