namespace Bars.GkhGji.Regions.BaseChelyabinsk.DomainService.Impl
{
    ...
    public class ActCheckRoChelyabinskService : IActCheckRoChelyabinskService
    {
        private readonly IDomainService<ActCheckRealityObject> _actRoDomain;

        public ActCheckRoChelyabinskService(IDomainService<ActCheckRealityObject> actRoDomain)
        {
            this._actRoDomain = actRoDomain;
        }
        ...
        public IDataResult GetRobjectCharacteristics(BaseParams baseParams)
        {
            ...
            var ro = this._actRoDomain.GetAll()
                .Where(x => x.Id == actRoId)
                .Select(x => x.RealityObject)
                .FirstOrDefault();

            return new BaseDataResult(new {chars = this.GetRobjectChars(ro)});
        }
