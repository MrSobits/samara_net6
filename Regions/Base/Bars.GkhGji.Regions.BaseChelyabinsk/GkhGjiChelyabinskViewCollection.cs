namespace Bars.GkhGji.Regions.BaseChelyabinsk
{
    ...
    public class GkhGjiChelyabinskViewCollection : BaseGkhViewCollection
    {
        ...
        /// <summary>
        /// gjiGetExecutantsBaseChelyabinsk(bigint)
        /// </summary>
        /// <param name="dbmsKind"></param>
        /// <returns></returns>
        private string CreateFunctionGetExecutantsBaseChelyabinsk(DbmsKind dbmsKind)
        {
            return @"
                CREATE OR REPLACE FUNCTION gjiGetExecutantsBaseChelyabinsk(param_id  bigint)
                RETURNS text AS
                $func$
                BEGIN
                RETURN ( 
                        ...
                        ARRAY(select s.FIO
                              from GJI_APPCIT_EXECUTANT  gas
                              join GKH_DICT_INSPECTOR s ON s.id = gas.EXECUTANT_ID
                              where s.FIO is not null
                               and gas.appcit_id = param_id), ',')
                        );
                END
        ...
        /// <summary>
        /// gjiSubjectsBaseChelyabinsk(bigint)
        /// </summary>
        /// <param name="dbmsKind"></param>
        /// <returns></returns>
        private string CreateFunctionSubjectsBaseChelyabinsk(DbmsKind dbmsKind)
        {
            return @"
                CREATE OR REPLACE FUNCTION gjiSubjectsBaseChelyabinsk(param_id  bigint)
                RETURNS text AS
                $func$
                BEGIN
                RETURN ( 
                        ...
                        ARRAY(select s.NAME
                              ...
                              join GJI_DICT_STATEMENT_SUBJ s ON s.id = gas.STATEMENT_SUBJECT_ID
        ...
        /// <summary>
		/// Вьюха запроса в ЕРКНМ
		/// </summary>
		/// <param name="dbmsKind"></param>
		/// <returns></returns>
		private string CreateViewChERKNM(DbmsKind dbmsKind)
        {
            return @"
                     DROP VIEW IF EXISTS view_gji_ch_gis_erknm;
                     CREATE VIEW view_gji_ch_gis_erknm AS 
                        SELECT 
                            erknm.id, 
                            erknm.object_create_date, 
                            erknm.messageid, 
                            erknm.doc_type, 
                            erknm.request_type, 
                            erknm.request_state,
                            ...
                            erknm.req_date, 
                            erknm.erpid,
                            insp.fio AS inspector,
                            CASE 
                                WHEN (doc.document_date is not null)
                                    THEN CONCAT(' ', doc.document_number, ' от ', doc.document_date::date)
                                WHEN (adm.document_date is not null)
                                    THEN CONCAT(' ', adm.document_num, ' от ', adm.document_date::date)
                                ELSE ''
                            END AS disposal,

                            (SELECT 
                                 fil.object_create_date 
                             FROM gji_ch_gis_erknm_file fil
                                 JOIN b4_file_info fi ON fi.id = fil.file_info_id
                             WHERE fil.gis_erknm_id = erknm.id AND fi.name = 'SendRequestRequest'
							 ORDER BY fil.object_create_date limit 1) AS send_time,
							 
							 (SELECT 
                                 fil.object_create_date 
                             FROM gji_ch_gis_erknm_file fil
                                 JOIN b4_file_info fi ON fi.id = fil.file_info_id
                             WHERE fil.gis_erknm_id = erknm.id AND fi.name = 'GetResponceResponse'
							 ORDER BY fil.object_create_date limit 1) AS ans_date,
							...
							((SELECT 
                             ...
                             FROM gji_ch_gis_erknm_file fil
                                 JOIN b4_file_info fi ON fi.id = fil.file_info_id
                             WHERE fil.gis_erknm_id = erknm.id AND fi.name = 'GetResponceResponse'
							 ORDER BY fil.object_create_date limit 1) - 
							 (SELECT 
                                 fil.object_create_date 
                             FROM gji_ch_gis_erknm_file fil
                                 JOIN b4_file_info fi ON fi.id = fil.file_info_id
                             WHERE fil.gis_erknm_id = erknm.id AND fi.name = 'SendRequestRequest'
							 ORDER BY fil.object_create_date limit 1))::varchar AS time_spent

                        FROM gji_ch_gis_erknm erknm
                            JOIN gkh_dict_inspector insp ON insp.id = erknm.inspector_id
                            LEFT JOIN gji_document doc ON doc.id = erknm.disposal_id
                            LEFT JOIN gji_ch_appcit_admonition adm ON adm.id = erknm.admon_id";
        }
...
SELECT 
    ...
    gjiSubjectsBaseChelyabinsk(gac.id::bigint) AS SUBJECTS_NAME,  
    gjiGetExecutantsBaseChelyabinsk(gac.id::bigint) AS EXECUTANTS_FIO,
    ''::text as executant_names,
    ''::text as subjects
...
CREATE VIEW view_gji_protocol AS 
 SELECT 
    ...
    gjiGetDocPlaceByViolStage(pr.id) AS place_names, 
    gjiGetDocMuIdByViolStage(pr.id) AS mu_id, 
    gjiGetDocArticleLaw(pr.id) as art_law,
    mu.name as ctr_mu_name,
    c.municipality_id as ctr_mu_id,
    c.name AS contragent_name, 
    exec.name AS type_exec_name, 
...
CREATE VIEW view_gji_resolution AS 
 SELECT 
    ...
    gjiGetDocArticleLaw(doc.id) as art_law,
...
FROM gji_document doc
    ...
    LEFT JOIN gji_dict_sanction s ON s.id = res.sanction_id";
        ...
        private string DeleteViewChERKNM(DbmsKind dbmsKind)
        {
            return this.DropViewPostgreQuery("VIEW_GJI_CH_GIS_ERKNM");
        }
		...
		{
			return this.DropViewPostgreQuery("view_gji_ins_jurpers");
		}

		private string DeleteViewInsProsclaim(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_ins_prosclaim");
		}

		private string DeleteViewInsBasedef(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_ins_basedef");
		}

		private string DeleteViewInsDisphead(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_ins_disphead");
		}

		private string DeleteViewInsInschek(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_ins_inschek");
		}

		private string DeleteViewLicenseApp(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_ins_license_app");
		}

		private string DeleteViewActRemoval(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_act_removal");
		}

		private string DeleteViewActcheck(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_actcheck");
		}

		private string DeleteViewActsurvey(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_actsurvey");
		}

		private string DeleteViewDisposal(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_disposal");
		}

		private string DeleteViewPrescription(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_prescription");
		}

		private string DeleteViewProtocol(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_protocol");
		}

		private string DeleteViewProtocol197(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_protocol197");
		}

		private string DeleteViewResolution(DbmsKind dbmsKind)
		{
			return this.DropViewPostgreQuery("view_gji_resolution");
		}
        ...
        /// <summary>
        /// Удаление функции gjiGetDocParentPlaceByViolStg(bigint)
        /// </summary>
        /// <param name="dbmsKind"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Удаление функции gjiGetExecutantsBaseChelyabinsk(bigint)
        /// </summary>
        /// <param name="dbmsKind"></param>
        /// <returns></returns>
        private string DeleteFunctionGetExecutantsBaseChelyabinsk(DbmsKind dbmsKind)
        {
            return @"DROP FUNCTION if exists gjiGetExecutantsBaseChelyabinsk(bigint)";
        }

        /// <summary>
        /// Удаление функции gjiSubjectsBaseChelyabinsk(bigint)
        /// </summary>
        /// <param name="dbmsKind"></param>
        /// <returns></returns>
        private string DeleteFunctionSubjectsBaseChelyabinsk(DbmsKind dbmsKind)
        {
            return @"DROP FUNCTION if exists gjiSubjectsBaseChelyabinsk(bigint)";
        }
