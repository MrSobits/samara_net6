namespace Bars.GkhGji.Regions.BaseChelyabinsk.SahalinStateChange
{
    using System;
    using System.Linq;
    ...
    public abstract class SahalinBaseDocNumberValidationChelyabinskRule : IRuleChangeStatus
    {
        ...
        private void GetDocNum(DocumentGji document)
        {
            ...
            switch (document.TypeDocumentGji)
            {
                ...
                case TypeDocumentGji.Prescription:
                {
                    ...
                    var lastNumByInspector = this.DocumentGjiDomain.GetAll()
                        .Where(x => x.Id != document.Id)
                ...
                {
                    var parentAct = this.GetParentDocument(document, TypeDocumentGji.ActCheck);
                    ...
                    var query = this.DocumentGjiChildrenDomain.GetAll()
                ...
                default:
                {
                    ...
                    var parentDisposal = this.GetParentDocument(document, TypeDocumentGji.Disposal);

                    if (parentDisposal == null)
                    {
                        throw new Exception("Родительское распоряжение ненайден");
                    }

                    if (!parentDisposal.DocumentNum.HasValue || string.IsNullOrEmpty(parentDisposal.DocumentNumber))
                    {
                        throw new Exception("Родительскому распоряжению не присвоен номер");
                    }

                    docNumber = parentDisposal.DocumentNumber;
                    docNum = parentDisposal.DocumentNum.Value;

                    var query = this.DocumentGjiDomain.GetAll()
                        .Where(x => x.Id != document.Id)
                        .Where(x => x.DocumentDate.HasValue && document.DocumentDate.HasValue)
