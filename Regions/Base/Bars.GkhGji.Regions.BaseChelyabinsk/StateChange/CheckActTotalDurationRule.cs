namespace Bars.GkhGji.Regions.BaseChelyabinsk.StateChange
{
    ...
    public class CheckActTotalDurationRule : IRuleChangeStatus
    {
        ...
        public ValidateResult Validate(IStatefulEntity statefulEntity, State oldState, State newState)
        {
            ...
            if (statefulEntity is ChelyabinskActCheck)
            {
                var actCheck = statefulEntity as ChelyabinskActCheck;
        ...
        {
            var conf = this.Container.GetGkhConfig<HousingInspection>();

            var timeIntervals = this.GetTimeIntervals(id,conf);
            var actCheckDuration = this.GetActCheckDuration(timeIntervals,conf);
            var configCheckDuration = this.GetConfigCheckDuration(type, conf);
