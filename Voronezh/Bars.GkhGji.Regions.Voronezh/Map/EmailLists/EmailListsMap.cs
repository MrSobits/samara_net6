...
namespace Bars.GkhGji.Regions.Voronezh.Map
{
    ...
    public class EmailListsMap : BaseEntityMap<EmailLists>
    {
        ...
        protected override void Map()
        {
            ...
            Property(x => x.AnswerDate, "Дата ответа").Column("ANSWER_DATE");
            Property(x => x.AppealNumber, "Номер обращения").Column("APPEAL_NUMBER");
            Property(x => x.GjiNumber, "Номер гжи").Column("GJI_NUMBER");
            Property(x => x.MailTo, "Кому").Column("MAIL_TO");
            Property(x => x.Executor, "Исполнитель").Column("EXECUTOR");
