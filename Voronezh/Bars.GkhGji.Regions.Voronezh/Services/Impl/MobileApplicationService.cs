namespace Bars.GkhGji.Regions.Voronezh.Services.Impl
{
    ...
    using Bars.B4.Utils;
    using Bars.GkhGji.Regions.Voronezh.Services.DataContracts.SyncOperators;
    ...
    using Bars.GkhGji.Regions.Voronezh.Services.DataContracts.SyncDictionaries;
    using Bars.GkhGji.Regions.Voronezh.Services.DataContracts.SyncInspections;
    ...
    public class MobileApplicationService : IMobileApplicationService
    {
        ...
        public OperatorGJIResponse GetOperatorsList(string token)
        {
            ...
            var operatorsList = OperatorDomain.GetAll()
                .Select(x => new
                {
                    ...
                    UserId = x.User.Id,
                    x.User.Login,
                    Inspector = x.Inspector != null ? new Inspector { Id = x.Inspector.Id, Fio = x.Inspector.Fio, Position = x.Inspector.Position } : null,
                    x.User.Name,
                    x.ObjectEditDate
                }).ToList();
            ...
            operatorsList.ForEach(obj =>
            {
                ...
                operators.Add(
                    new OperatorGJI
                    {
                        ...
                        Name = IsGJI ? obj.Inspector.Fio : obj.Name,
                        IsGJI = IsGJI,
                        ObjectEditDate = obj.ObjectEditDate,
                        Position = IsGJI ? obj.Inspector.Position : "Не является сотрудником ГЖИ",
                    });
            });
        ...
        public RequestResult ImportInspectionResult(string token, long inspectorId, IncpectionGJIRequest request)
        {
            ...
            try
            {
                ...
                foreach (var inspection in request.IncpectionsGJI)
                {
                    ...
                    //сначала ищем все акты. И только после сохранения актов ищем зависимые документы
                    foreach (var doc in inspection.DocumentsGJI)
                    {
                        if (doc.TypeDocumentGji == TypeDocumentGji.ActCheck)
                        {
                            ...
                            var actCheckRule = new DisposalToActCheckRule();
                    ...
                    foreach (var doc in inspection.DocumentsGJI)
                    {
                        if (doc.TypeDocumentGji == TypeDocumentGji.ActRemoval)
                        {
                            ...
                            var rule = new DisposalToActRemovalPrescriptionRule();
        ...
        public IncpectionGJIResponse GetInspections(string token, long operatorId)
        {
            try
            {
                ...
                var actsWithResult = this.ActCheckRealityObjectDomain.GetAll()
                    ...
                    .Where(x => x.HaveViolation != Gkh.Enums.YesNoNotSet.NotSet)
                ...
                actsWithResult.AddRange(ActRemovalDomain.GetAll()
                    ...
                    .Where(x => x.TypeRemoval != Gkh.Enums.YesNoNotSet.NotSet)
                    .Select(x => x.Id).Distinct().ToList()
                );

                //получаем распоряжения c закрытыми актами по искомым проверкам, эти распоряжения будут исключены из передачи на планшет
                var disposalDocsWithActs = this.ParentChildrenDomain.GetAll()
                    ...
                    .Where(x => x.Parent.TypeDocumentGji == GkhGji.Enums.TypeDocumentGji.Disposal)
                ...
                // получаем распоряжения которые пойдут на мобильное приложение
                var disposals = this.DisposalDomain.GetAll()
                    .Where(x => x.DocumentDate.HasValue)
                    .Where(x => inspectionList.Contains(x.Inspection.Id))
                    .Where(x => !disposalDocsWithActs.Contains(x.Id)).ToList();
                ...
                List<DisposalProxy> disposalProxyList = new List<DisposalProxy>();
                ...
                //бежим по проверкам в полученных распоряжениях, которые предстоит отработать
                foreach (InspectionGji inpectionGji in disposals.Select(x => x.Inspection).Distinct().ToList())
                {
                    ...
                    //бежим по всем распоряжениям в проверке
                    foreach (ChelyabinskDisposal disp in disposals.Where(x => x.Inspection == inpectionGji).ToList())
                    {
                        //если проверка первичная - то есть по основанию, то есть не проверка исполнения предписания
                        if (disp.TypeDisposal == GkhGji.Enums.TypeDisposalGji.Base)
                        {
                            if (disp.TimeVisitStart.HasValue)
                            {
                                checkDate = disp.TimeVisitStart.Value < checkDate ? disp.TimeVisitStart.Value : checkDate;
                            }

                            //добавляем сведения о текущем распоряжении в общий список
                            disposalProxyList.Add(new DisposalProxy
                            {
                                DateStart = disp.DateStart,
                                BaseText = baseInsp,
                                Inspectors = GetInspectorsFromDocument(disp.Id),
                                IssuedDisposal = disp.IssuedDisposal != null
                                    ? new InspectorProxy
                                        { Id = disp.IssuedDisposal.Id, FIO = disp.IssuedDisposal.Fio, Position = disp.IssuedDisposal.Position }
                                    : null,
                                KindCheckGjiId = disp.KindCheck.Id,
                                ObjectVisitEnd = disp.ObjectVisitEnd,
                                ObjectVisitStart = disp.ObjectVisitStart,
                                SurveyObjective = GetSurveyObjectivesForDisposal(disp.Id),
                                SurveyPurpose = GetSurveyPurposesForDisposal(disp.Id),
                                TypeAgreementProsecutor = disp.TypeAgreementProsecutor,
                                Id = disp.Id,
                                TypeAgreementResult = disp.TypeAgreementResult,
                                AgreementResult = disp.ProcAprooveDate.HasValue
                                    ? $"Прокуратура воронежской области решение от {disp.ProcAprooveDate.Value.ToShortDateString()} N{disp.ProcAprooveNum}"
                                    : "",
                                TypeDisposalGji = disp.TypeDisposal,
                                PeriodCorrect = disp.PeriodCorrect,
                                KindKNDGJI = disp.KindKNDGJI
                            });
                            ...
                            documentsProxyList.Add(new DocumentGJIProxy
                            {
                                ...
                                TypeDocumentGji = GkhGji.Enums.TypeDocumentGji.Disposal,
                                ParentId = null
                            });
                        ...
                        //распоряжение на проверку исполнения предписания
                        else if (disp.TypeDisposal == GkhGji.Enums.TypeDisposalGji.DocumentGji)
                        {
                            if (disp.TimeVisitStart.HasValue)
                            {
                                checkDate = disp.TimeVisitStart.Value < checkDate ? disp.TimeVisitStart.Value : checkDate;
                            ...
                            if (prescriptiondoc != null)
                            {
                                ...
                                //добавляем предписание в список предписаний
                                prescriptionProxyList.Add(new PrescriptionProxy
                                {
                                    ...
                                    ContragentProxy = prescription.Contragent != null
                                        ? new ContragentProxy
                                        {
                                            Id = prescription.Contragent.Id, Inn = prescription.Contragent.Inn, Name = prescription.Contragent.Name,
                    ...
                    inspProxyList.Add(new IncpectionGJIProxy
                    {
                        ...
                        ContragentProxy = inpectionGji.Contragent != null
                            ? new ContragentProxy
                            {
                                Id = inpectionGji.Contragent.Id, Inn = inpectionGji.Contragent.Inn, Name = inpectionGji.Contragent.Name,
                ...
                var docdataresp = new DocumentsGJIData
                {
                    ...
                    Disposals = disposalProxyList.ToArray(),
            ...
            switch (inspection.TypeBase)
            {
                case TypeBase.CitizenStatement:
                    ...
                    if (appcit.Count > 0)
                    {
                        appcit.ForEach(x =>
                        {
                            result += $"Обращение № {x.AppealCits.DocumentNumber} от {x.AppealCits.DateFrom.Value.ToShortDateString()} \n";
                        });
                    }
        ...
        {
            var violations = this.ViolGroupService.GetAll()
                .Where(x => x.FeatureViolGji.FullName.Contains("НОГБ") || x.FeatureViolGji.FullName.Contains("НСМКД")).Select(x => x.ViolationGji)
                .ToList();
