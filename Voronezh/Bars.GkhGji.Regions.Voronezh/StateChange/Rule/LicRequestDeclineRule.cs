namespace Bars.GkhGji.Regions.Voronezh.StateChanges
{
    ...
    public class LicRequestDeclineRule : IRuleChangeStatus
    {
        /// <summary>
        ...
        public string Id
        {
            get { return "LicRequestDeclineRule"; }
        }

        public string Name { get { return "Отправка отчета об отклонении заявки"; } }
        public string TypeId { get { return "gkh_manorg_license_request"; } }
        public string Description
        {
            get
            {
                return "При переводе статуса будет отправлен отчет об отклонении заявки";
            }
        }

        public ValidateResult Validate(IStatefulEntity statefulEntity, State oldState, State newState)
        {
            var appeal = statefulEntity as ManOrgLicenseRequest;
            if (!string.IsNullOrEmpty(appeal.MessageId))
            {
                if (RPGUService.SendAcceptMessage(Convert.ToInt64(statefulEntity.Id), false))
                {
