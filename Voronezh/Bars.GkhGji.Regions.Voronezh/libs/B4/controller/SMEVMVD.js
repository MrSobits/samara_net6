Ext.define('B4.controller.SMEVMVD', {
    ...
    aspects: [
        ...
        },
        {
            ...
            otherActions: function (actions) {             
                actions[this.editFormSelector + ' #sfContragentContact'] = { 'beforeload': { fn: this.onBeforeLoadContragent, scope: this } };
            },           
            onBeforeLoadContragent: function (store, operation) {
                operation = operation || {};
                operation.params = operation.params || {};

                operation.params.contragentId = this.getForm().down('#sfContragent').getValue();
                this.getForm().down('#dtBirthDate').allowBlank = true;
                this.getForm().down('#tfSurname').allowBlank = true;
                this.getForm().down('#tfName').allowBlank = true;
                this.getForm().down('#tfPatronymicName').allowBlank = true;
                this.getForm().down('#tfName').allowBlank = true;
            },
            listeners: {
    ...
    onLaunch: function () {
        ...
        if (this.params && this.params.get('Id') > 0) {
            this.getAspect('smevmvdGridAspect').editRecord(this.params);
            this.params = null;
        }
    },
    ////////////////////
