Ext.define('B4.controller.SMEVEGRN', {
    ...
    aspects: [
          
        {
            ...
            onAfterSetFormData: function (aspect, rec, form) {
                var sfEGRNApplicantType = form.down('#sfEGRNApplicantType');
                if (!sfEGRNApplicantType.value) {
                    var newApplicantType = {
                        Id: 1,
                        Name: 'Органы государственной власти субъектов Российской Федерации'
                    };
                    sfEGRNApplicantType.setValue(newApplicantType);
                }
                var sfRegionCodePrimary = form.down('#sfRegionCodePrimary');
                if (!sfRegionCodePrimary.value) {
                    var newRegionCodePrimary = {
                        Id: 1,
                        Name: 'Информационный центр Главного управления МВД России по Воронежской области'
                    };
                    sfRegionCodePrimary.setValue(newRegionCodePrimary);
                }
            },
        }
    ],
    ...
    onLaunch: function () {
        ...
        if (this.params && this.params.get('Id') > 0) {
            this.getAspect('smevegrnGridAspect').editRecord(this.params);
            this.params = null;
        }
    },
    ////////////////////
