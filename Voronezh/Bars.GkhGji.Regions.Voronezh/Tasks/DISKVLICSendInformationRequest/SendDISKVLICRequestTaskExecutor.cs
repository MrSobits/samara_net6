...
{
    public class SendDISKVLICRequestTaskExecutor : ITaskExecutor
    {
        ...
        public SendDISKVLICRequestTaskExecutor(ISMEVDISKVLICService SMEVDISKVLICService)
        {
            _SMEVDISKVLICService = SMEVDISKVLICService;
        }
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            SMEVDISKVLIC request = SMEVDISKVLICDomain.Get(taskId);
            if (request == null)
            ...
            try
            {
                var result = _SMEVDISKVLICService.SendInformationRequest(request, indicator);
                if (!result)
