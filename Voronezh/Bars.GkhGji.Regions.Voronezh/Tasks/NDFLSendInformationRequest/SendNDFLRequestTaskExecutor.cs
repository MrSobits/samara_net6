...
{
    public class SendNDFLRequestTaskExecutor : ITaskExecutor
    {
        ...
        public SendNDFLRequestTaskExecutor(ISMEVNDFLService SMEVNDFLService)
        {
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            SMEVNDFL request = SMEVNDFLDomain.Get(taskId);
            if (request == null)
            ...
            try
            {
                var result = _SMEVNDFLService.SendInformationRequest(request, indicator);
                if (!result)
