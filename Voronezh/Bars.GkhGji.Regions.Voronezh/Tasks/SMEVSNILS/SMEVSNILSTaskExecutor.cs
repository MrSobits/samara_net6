...
{
    public class SMEVSNILSTaskExecutor : ITaskExecutor
    {
        ...
        public SMEVSNILSTaskExecutor(ISMEVSNILSService SMEVSNILSService)
        {
            _SMEVSNILSService = SMEVSNILSService;
        }
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            SMEVSNILS request = SMEVSNILSDomain.Get(taskId);
            if (request == null)
            ...
            try
            {
                var result = _SMEVSNILSService.SendInformationRequest(request, indicator);
                if (!result)
