namespace Bars.GkhGji.Regions.Archangelsk.DomainService
{
    ...
    public class AppealCitsExecutantService : IAppealCitsExecutantService
    {
        ...
        {
            var appealCitsExecutantDomain = Container.Resolve<IDomainService<AppealCitsExecutant>>();
            try
            {
            ...
            {
                Container.Release(appealCitsExecutantDomain);
            }
        }
        ...
        public IDataResult RedirectExecutant(BaseParams baseParams)
        {
            try
            {
                ...
                var appealCitsExecutantDomain = Container.Resolve<IDomainService<AppealCitsExecutant>>();
                var stateDomain = Container.Resolve<IDomainService<State>>();

                using (Container.Using(appealCitsExecutantDomain, stateDomain))
                {
