namespace Bars.GkhGji.Regions.Zabaykalye.InspectionRules
{
    ...
    /// Данное правило для Забайкалья иземнено потомучто при создании предписания из акта пользователь выбирает не нарушения, а Описания (то есть группы нарушений)
    /// </summary>
    public class ZabaykalyeActCheckToPrescriptionRule : IDocumentGjiRule
    {
        ...
        {
            get { return "Правило создание документа 'Предписания' из документа 'Акт проверки' (по выбранным описаниям)"; }
        }
        ...
        public IDataResult ValidationRule(DocumentGji document)
        {
            ...
            {
                return new BaseDataResult(false, "В Сахе для Акта проверки предписания нельзя формировать предписания");
            }
