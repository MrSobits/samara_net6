    ...
    public class AppealCitsServiceInterceptor : Bars.GkhGji.Interceptors.AppealCitsServiceInterceptor
    {
        ...
        private void TrackChange(AppealCits entity)
        {
            ...
            try
            {
                ...
                User user = null;
                if (userIdent.UserId == 0)
                {
                    user = userDomain.FirstOrDefault(x => x.Login == "delo_ased");
                } 
                else if (!(userIdent is AnonymousUserIdentity))
                {
                ...
                if (user == null)
                {
                    throw new Exception("Невозможно определить пользователя. Создайте пользователя с логином delo_ased");
                }
