    ...
    aspects: [
        {
            ...
            getUserParams: function (reportId) {
                var me = this,
                    param = { AppelCitsId: me.controller.appealCitizensId };
        ...
        },
        {
            ...
            onAfterSetFormData: function (aspect, rec, form) {
                ...
                var me = this;
                me.controller.getAspect('appealCitsAnswerPrintAspect').loadReportStore();
            },
            ...
            editRecord: function(record) {
                ...
                if (id) {
                    ...
                    me.getForm().down('[name=DateFrom]').setValue(date);
                    me.getForm().down('[name=CheckTime]').setValue(Ext.Date.add(date, Ext.Date.DAY, 29));

                    B4.Ajax.request(B4.Url.action('GetByOkato', 'ZonalInspection', {
                        Okato: '76401373'
                    })).next(function(record) {
                        var obj = Ext.JSON.decode(record.responseText);
                        me.getForm().down('#sflZonalInspection').setValue(obj.data);
                    }).error(function(result) {
                         ...
                         case 'add':
                             {
                                 ...
                                 me.updateSelectGrid();
                                 me.getForm().down('[name=PerformanceDate]').setValue(Ext.ComponentQuery.query('#appealCitsEditWindow [name=CheckTime]')[0].getValue());
                             }
                             break;
                         case 'update':
                             me.updateGrid();
