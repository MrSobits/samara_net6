Ext.define('B4.controller.resolpros.Edit', {
    ...
    aspects: [
        ...
        },
        {
            ...
            setDefinitionId: function (asp, id) {
                asp.controller.params.definitionId = id;
                if (id) {
                    asp.controller.getAspect('resolProsDefinitionPrintAspect').loadReportStore();
                }
            }
        },
        {
            ...
            buttonSelector: 'resolprosdefeditwin #btnPrint',
    ...
    init: function () {
        ...
        me.getStore('resolpros.Annex').on('beforeload', me.onBeforeLoad, me);
        me.getStore('resolpros.ArticleLaw').on('beforeload', me.onBeforeLoad, me);
        me.getStore('resolpros.RealityObject').on('beforeload', me.onBeforeLoad, me);
    ...
    onLaunch: function () {
        ...
        if (me.params) {
            me.getAspect('resolProsEditPanelAspect').setData(me.params.documentId);

            //Обновляем стор приложений
            me.getStore('resolpros.Annex').load();

            //Обновляем стор статьи закона
            me.getStore('resolpros.ArticleLaw').load();

            //Обновляем стор домов
            me.getStore('resolpros.RealityObject').load();

            me.getMainView().down('resolprosdefgrid').getStore().filter('documentId', me.params.documentId);
        }
    },
