namespace Bars.GkhGji.Regions.Zabaykalye.DomainService
{
    ...
    /// колонки новые
    /// </summary>
    public class ZabaykalyeActSurveyService : GkhGji.DomainService.ActSurveyService
    {
        ...
        {
            var zabaykalyeSurveyDomain = Container.Resolve<IDomainService<ZabaykalyeActSurvey>>();
            ...
            */
            ...
            // Так желаю чтобы просто вьюху неправить sql 
                var dictZabaykalyeActSurveys =
                    zabaykalyeSurveyDomain.GetAll()
            ...
            var data = GetViewList()
                ...
                .Select(x => new {
                    ...
                    x.TypeBase,
                    x.TypeDocumentGji,
                    ConclusionIssued = dictZabaykalyeActSurveys.Get(x.Id)
                })
                .AsQueryable()
            ...
            {
                Container.Release(zabaykalyeSurveyDomain);
            }
