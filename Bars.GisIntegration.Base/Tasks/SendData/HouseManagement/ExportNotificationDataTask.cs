namespace Bars.GisIntegration.Base.Tasks.SendData.HouseManagement
{
    ...
    /// <summary>
    /// Задача получения результатов для экспортера новостей
    /// </summary>
    public class ExportNotificationDataTask : BaseSendDataTask<importNotificationRequest, getStateResult, HouseManagementPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        ...
        protected override string ExecuteRequest(RequestHeader header, importNotificationRequest request)
        {
            AckRequest result;
            ...
            soapClient.importNotificationData(header, request, out result);
        ...
        {
            if (!transportGuidDictByType.ContainsKey(typeof(RisNotification)))
            {
                throw new Exception("Не удалось обработать результат выполнения метода getState");
            }
            var notifiacationsByTransportGuid = transportGuidDictByType[typeof(RisNotification)];
            ...
            foreach (var item in responce.Items)
            {
                ...
                {
                    var processingResult = this.CheckResponseItem(responseItem, notifiacationsByTransportGuid);
        ...
        {
            var domain = this.Container.ResolveDomain<RisNotification>();
            ...
            try
            {
                ...
                var notification = domain.Get(notificationId);
                ...
                return new ObjectProcessingResult
                {
                    ...
                    RisId = notificationId,
                    State = ObjectProcessingState.Success,
                    ObjectsToSave = new List<PersistentObject> { notification }
                };
            }
            finally
            {
