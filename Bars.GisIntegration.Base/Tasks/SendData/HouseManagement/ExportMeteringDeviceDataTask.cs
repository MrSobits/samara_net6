namespace Bars.GisIntegration.Base.Tasks.SendData.HouseManagement
{
    ...
    /// <summary>
    /// Задача получения результатов для экспортера данных о ПУ
    /// </summary>
    public class ExportMeteringDeviceDataTask : BaseSendDataTask<importMeteringDeviceDataRequest, getStateResult, HouseManagementPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        ...
        protected override string ExecuteRequest(RequestHeader header, importMeteringDeviceDataRequest request)
        {
            ...
            if (soapClient != null)
            {
                soapClient.importMeteringDeviceData(header, request, out result);
            }
        ...
        {
            if (!transportGuidDictByType.ContainsKey(typeof(RisMeteringDeviceData)))
            {
                throw new Exception("Не удалось обработать результат выполнения метода getState");
            }

            var devicesByTransportGuid = transportGuidDictByType[typeof(RisMeteringDeviceData)];
            ...
            foreach (var item in responce.Items)
            {
                ...
                {
                    var processingResult = this.CheckResponseItem(responseItem, devicesByTransportGuid);
        ...
        {
            var domain = this.Container.ResolveDomain<RisMeteringDeviceData>();
            ...
            try
            {
                ...
                var deviceId = transportGuidDict[responseItem.TransportGUID];
                ...
                var device = domain.Get(deviceId);
                device.Guid = responseItem.GUID;
                ...
                return new ObjectProcessingResult
                {
                    ...
                    RisId = deviceId,
                    State = ObjectProcessingState.Success,
                    ObjectsToSave = new List<PersistentObject> { device }
                };
            }
            finally
            {
