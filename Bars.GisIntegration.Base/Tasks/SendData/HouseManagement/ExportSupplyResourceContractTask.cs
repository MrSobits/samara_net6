namespace Bars.GisIntegration.Base.Tasks.SendData.HouseManagement
{
    ...
    /// </summary>
    public class ExportSupplyResourceContractTask : BaseSendDataTask<importSupplyResourceContractRequest, getStateResult, HouseManagementPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        ...
        protected override string ExecuteRequest(RequestHeader header, importSupplyResourceContractRequest request)
        {
            ...
            if (soapClient != null)
            {
                soapClient.importSupplyResourceContractData(header, request, out result);
            }
        ...
        {
            if (!transportGuidDictByType.ContainsKey(typeof(SupplyResourceContract)))
            {
                throw new Exception("Не удалось обработать результат выполнения метода getState");
            }
            var contractsByTransportGuid = transportGuidDictByType[typeof(SupplyResourceContract)];
        ...
        {
            var domain = this.Container.ResolveDomain<SupplyResourceContract>();
            ...
            try
            {
                ...
                var contractId = transportGuidDict[responseItem.TransportGUID];
                ...
                var contract = domain.Get(contractId);
