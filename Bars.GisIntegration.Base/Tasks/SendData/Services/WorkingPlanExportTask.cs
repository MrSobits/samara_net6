namespace Bars.GisIntegration.Base.Tasks.SendData.Services
{
    ...
    /// <summary>
    /// Задача экспорта планов
    /// </summary>
    public class WorkingPlanExportTask : BaseSendDataTask<importWorkingPlanRequest, getStateResult, ServicesPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        /// Домен-сервис "План по перечню работ/услуг"
        /// </summary>
        public IDomainService<WorkingPlan> WorkingPlanDomainService { get; set; }

        /// <summary>
        /// Домен-сервис "План работ по перечню работ/услуг"
        /// </summary>
        public IDomainService<WorkPlanItem> WorkPlanItemDomainService { get; set; }
        ...
        protected override string ExecuteRequest(RequestHeader header, importWorkingPlanRequest request)
        {
            ...
            soapClient.importWorkingPlan(header, request, out result);
            ...
            Dictionary<Type, Dictionary<string, long>> transportGuidDictByType)
        {
            var privateTransportGuidByType = transportGuidDictByType
                .Where(x => x.Key == typeof(WorkPlanItem) || x.Key == typeof(WorkingPlan))
            ...
            {
                { typeof(WorkingPlan), this.WorkingPlanDomainService },
                { typeof(WorkPlanItem), this.WorkPlanItemDomainService }
            };
            ...
            foreach (var responseItem in response.Items)
            {
                ...
                if (commonResult != null)
                {
                    ...
                    var processingResult = WorkingPlanExportTask.ProcessCommonResult(
                        commonResult,
