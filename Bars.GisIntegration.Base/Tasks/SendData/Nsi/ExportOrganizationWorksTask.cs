namespace Bars.GisIntegration.Base.Tasks.SendData.Nsi
{
    ...
    /// <summary>
    /// Задача отправки данных справочника «Работы и услуги организации»
    /// </summary>
    public class ExportOrganizationWorksTask : BaseSendDataTask<importOrganizationWorksRequest, getStateResult, NsiPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        ...
        protected override string ExecuteRequest(RequestHeader header, importOrganizationWorksRequest request)
        {
            ...
            if (soapClient != null)
            {
                soapClient.importOrganizationWorks(header, request, out result);
            }
        ...
        {
            if (!transportGuidDictByType.ContainsKey(typeof(RisOrganizationWork)))
            {
                throw new Exception("Не удалось обработать результат выполнения метода getState");
            }
            var contractsByTransportGuid = transportGuidDictByType[typeof(RisOrganizationWork)];
        ...
        {
            var domain = this.Container.ResolveDomain<RisOrganizationWork>();
            ...
            try
            {
                ...
                var organizationWorkId = transportGuidDict[responseItem.TransportGUID];
                ...
                if (responseItem.GUID.IsEmpty())
                {
                    ...
                    return new ObjectProcessingResult
                    {
                        ...
                        RisId = organizationWorkId,
                        State = ObjectProcessingState.Error,
                        Message = errorNotation
                    };
                }

                var contract = domain.Get(organizationWorkId);
