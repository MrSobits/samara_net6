namespace Bars.GisIntegration.Base.Tasks.SendData.Nsi
{
    ...
    /// <summary>
    /// Задача отправки данных справочника «Коммунальные услуги»
    /// </summary>
    public class ExportMunicipalServicesTask : BaseSendDataTask<importMunicipalServicesRequest, getStateResult, NsiPortsTypeAsyncClient, RequestHeader>
    {
        /// <summary>
        ...
        protected override string ExecuteRequest(RequestHeader header, importMunicipalServicesRequest request)
        {
            ...
            if (soapClient != null)
            {
                soapClient.importMunicipalServices(header, request, out result);
            }
        ...
        {
            if (!transportGuidDictByType.ContainsKey(typeof(RisMunicipalService)))
            {
                throw new Exception("Не удалось обработать результат выполнения метода getState");
            }
            var contractsByTransportGuid = transportGuidDictByType[typeof(RisMunicipalService)];
        ...
        {
            var domain = this.Container.ResolveDomain<RisMunicipalService>();
            ...
            try
            {
                ...
                var municipalServiceId = transportGuidDict[responseItem.TransportGUID];
                ...
                if (responseItem.GUID.IsEmpty())
                {
                    ...
                    return new ObjectProcessingResult
                    {
                        ...
                        RisId = municipalServiceId,
                        State = ObjectProcessingState.Error,
                        Message = errorNotation
                    };
                }

                var contract = domain.Get(municipalServiceId);
