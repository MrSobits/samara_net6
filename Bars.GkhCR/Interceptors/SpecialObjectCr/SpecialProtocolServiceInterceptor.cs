namespace Bars.GkhCr.Interceptors
{
    ...
    /// </summary>
    public class SpecialProtocolServiceInterceptor : EmptyDomainInterceptor<SpecialProtocolCr>
    {
        /// <summary>
        ...
        public override IDataResult BeforeCreateAction(IDomainService<SpecialProtocolCr> service, SpecialProtocolCr entity)
        {
            var documents =
                service.GetAll()
            ...
            var useDocumentAddingOrder = this.Container.GetGkhConfig<GkhCrConfig>().General.DocumentAddingOrder == DocumentAddingOrder.Use;

            if (entity.TypeDocumentCr == null)
	        {
				return this.Failure("Не заполнено поле 'Тип документа'");
	        }
            ...
            if (useDocumentAddingOrder)
            {
                ...
                switch (entity.TypeDocumentCr.Key)
                {
                    ...
                    case TypeDocumentCrLocalizer.ActExpluatatinAfterCrKey:
                        ...
                        if (!actAuditDataExpenseList.Any())
                        {
                            return this.Failure("'Акт ввода в эксплуатацию после капремонта' можно добавить если у данного объекта есть документы с типом 'Акт сверки данных о расходах' ");
                        }
            ...
            if (useDocumentAddingOrder)
            {
                var res = this.CheckDates(entity, protocolNeedCrList, protocolCompleteCrList, actExpluatatinAfterCrList, protocolChangeCrList, dateProtocolCompleteCr);
                if (!res.Success)
                {
                    return res;
                }
        ...
        /// <param name="entity">Сущность</param>
        /// <returns>Результат операции</returns>
        public override IDataResult BeforeUpdateAction(IDomainService<SpecialProtocolCr> service, SpecialProtocolCr entity)
        {
        ...
        /// <param name="entity">Сущность</param>
        /// <returns>Результат операции</returns>
        public override IDataResult BeforeDeleteAction(IDomainService<SpecialProtocolCr> service, SpecialProtocolCr entity)
        {
            ...
            if (useDocumentAddingOrder && entity.TypeDocumentCr != null)
			{
				switch (entity.TypeDocumentCr.Key)
				{
					case TypeDocumentCrLocalizer.ProtocolNeedCrKey:
						...
						{
							return this.Failure("Нельзя удалить документ \"Протокол о необходимости проведения\", так как существуют зависимые от него документы: \"Протокол о завершении капремонта\"");
						}

						if (documents.Any(x => x.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ProtocolChangeCrKey))
						{
							return this.Failure("Нельзя удалить документ \"Протокол о необходимости проведения\", так как существуют зависимые от него документы: \"Протокол о внесение изменений в КР\"");
						}
					...
					case TypeDocumentCrLocalizer.ProtocolCompleteCrKey:
						...
						{
							return this.Failure("Нельзя удалить документ \"Протокол о завершении капремонта\", так как существуют зависимые от него документы: \"Акт ввода в эксплуатацию дома после капремонта\"");
						}

						if (documents.Any(x => x.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ActAuditDataExpenseKey))
						{
							return this.Failure("Нельзя удалить документ \"Протокол о необходимости проведения\", так как существуют зависимые от него документы: \"Акт сверки данных о расходах\"");
						}
					...
					case TypeDocumentCrLocalizer.ActAuditDataExpenseKey:
						if (documents.Any(x => x.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ActExpluatatinAfterCrKey))
						{
							return this.Failure("Нельзя удалить документ \"Акт сверки данных о расходах\", так как существуют зависимые от него документы: \"Акт ввода в эксплуатацию дома после капремонта\"");
						}
        ...
        private IDataResult CheckDates(SpecialProtocolCr entity, 
            ...
            DateTime dateProtocolCompleteCr)
        {
            ...
            if (entity.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ProtocolCompleteCrKey && protocolNeedCrList.Count > 0)
            {
                ...
                {
                    return this.Failure("Дата протокола о завершении капремонта должна быть больше или равна чем любая из дат протокола о необходимости проведения капремонта");
                }
            }
            ...
            if (entity.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ActExpluatatinAfterCrKey)
            {
                ...
                {
                    return this.Failure("Дата акта ввода в эксплуатацию после капремонта должна быть больше или равна чем дата протокола о завершении капремонта");
                }
            }
            ...
            if (entity.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ProtocolChangeCrKey && protocolNeedCrList.Count > 0)
            {
                if (!protocolNeedCrList.All(x => x.DateFrom.GetValueOrDefault() <= entity.DateFrom.GetValueOrDefault()))
                {
                    return this.Failure("Дата должна быть больше или равна дате протокола о необходимости проведения капремонта");
                }
            }
            ...
            if (entity.TypeDocumentCr.Key == TypeDocumentCrLocalizer.ProtocolNeedCrKey && protocolChangeCrList.Count > 0)
            {
                ...
                {
                    return this.Failure("Дата протокола о внесении изменений в капремонт должна быть больше или равна чем любая из дат протокола о необходимости проведения капремонта");
                }
            }
            return this.Success();
        }
