namespace Bars.GkhCr.Report
{
    ...
    /// <summary>
    /// Отчет информация по объектам о ходе КР
    /// </summary>
    public class ObjectCrInfo : BasePrintForm
    {
        #region Свойства
        ...
        public ObjectCrInfo()
            : base(new ReportTemplateBinary(Properties.Resources.ObjectCrInfo))
        {
        }

        public override string RequiredPermission
        {
            get { return "Reports.CR.ObjectCrInfo"; }
        }

        public override string Desciption
        {
            get { return "Отчет информация по объектам о ходе КР"; }
        }
        ...
        {
            get { return "B4.controller.report.ObjectCrInfo"; }
        }
        ...
        public override void SetUserParams(BaseParams baseParams)
        {
            ...
            var financeSourceIdsList = baseParams.Params.GetAs("finSourceIds", string.Empty);
            financeSourceIds = !string.IsNullOrEmpty(financeSourceIdsList) ? financeSourceIdsList.Split(',').Select(id => id.ToLong()).ToArray() : new long[0];
        }
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            dictCodeWork.Add("1018", "ПСД разработка");
            dictCodeWork.Add("1019", "ПСД экспертиза");
            dictCodeWork.Add("1020", "Технадзор");
            dictCodeWork.Add("30", "Энергообследование");
            dictCodeWork.Add("141", "Ремонт лифтов");
            dictCodeWork.Add("142", "Замена лифтов");
            dictCodeWork.Add("143", "Модернизация лифтов");
            dictCodeWork.Add("88", "Установка ИТП");
            ...
            var workCodes = dictCodeWork.Keys.ToList();
            ...
            var progCr = Container.Resolve<IDomainService<ProgramCr>>().Get(programCrId);

            var year = progCr.Period.DateEnd.HasValue ? progCr.Period.DateEnd.Value.Year : reportDate.Year;

            var archiveRecs = (year < 2013) ? null
                : Container.Resolve<IDomainService<ArchiveSmr>>().GetAll()
                .WhereIf(financeSourceIds.Length > 0, x => financeSourceIds.Contains(x.TypeWorkCr.FinanceSource.Id))
