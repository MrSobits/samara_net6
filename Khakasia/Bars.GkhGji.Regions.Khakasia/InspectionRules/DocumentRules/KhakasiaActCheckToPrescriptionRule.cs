namespace Bars.GkhGji.Regions.Khakasia.InspectionRules
{
    ...
    /// Данное правило для Хакасии иземнено потомучто при создании предписания из акта пользователь выбирает не нарушения, а Описания (то есть группы нарушений)
    /// </summary>
    public class KhakasiaActCheckToPrescriptionRule : IDocumentGjiRule
    {
        ...
        public IDataResult ValidationRule(DocumentGji document)
        {
            ...
            {
                return new BaseDataResult(false, "В Хакасии для Акта проверки предписания нельзя формировать предписания");
            }
