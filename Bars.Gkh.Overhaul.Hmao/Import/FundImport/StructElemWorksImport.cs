namespace Bars.Gkh.Overhaul.Hmao.Import
{
    using System;
    ...
    public class StructElemWorksImport : GkhImportBase
    {
        ...
        public override ImportResult Import(BaseParams baseParams)
        {
            ...
            try
            {
                this.dictHeaders = this.dictCodes.ToDictionary(x => x.Key, y => -1);

                //словарь id жилого дома - коды конструктивных элементов
                var dictRoStructEls = this.RoStructElemRep.GetAll()
                ...
                var dictRobject = this.RobjectRep.GetAll()
                ...
                using (this.Container.Using(excel))
                {
                    ...
                    using (var memoryStreamFile = new MemoryStream(fileData.Data))
                    {
                        ...
                        {
                            if (this.dictHeaders.ContainsKey(headerRow[i].Value))
                            {
                                this.dictHeaders[headerRow[i].Value] = i;
                            }
                        }

                        if (this.dictHeaders["ID_MKD"] == -1)
                        {
                            this.LogImport.Error("Ошибка", "Отсутствует столбец с федеральным номером дома");
                        ...
                        for (var i = 1; i < rows.Count; i++)
                        {
                            ...
                            var federalNumber = row[this.dictHeaders["ID_MKD"]].Value;

                            if (string.IsNullOrEmpty(federalNumber))
                            {
                                this.LogImport.Warn("Предупреждение", string.Format("Не указан федеральный номер дома, строка {0}", i));
                                continue;
                            }

                            if (!dictRobject.ContainsKey(federalNumber))
                            {
                                this.LogImport.Warn("Предупреждение", string.Format("Не удалось получить дом по федеральному номеру, строка {0}", i));
                                continue;
                            }
                            ...
                            var planDate = row[this.dictHeaders["PLAN_DATE"]].Value.ToDateTime();

                            if (planDate == DateTime.MinValue)
                            {
                                this.LogImport.Warn("Предупреждение", string.Format("Не указана плановая дата ремонта, строка {0}", i));
                                continue;
                            }

                            var roStructElems = dictRoStructEls[federalNumber];

                            foreach (var header in this.dictHeaders.Where(x => x.Value > -1))
                            {
                                ...
                                var codes = this.dictCodes[header.Key];
                ...
                foreach (var item in recordsForUpdate)
                {
                    ...
                    {
                        var newRec = this.RoStructElemRep.Load(id);
                        newRec.LastOverhaulYear = item.PlanDate.Year;
                        newStructElems.Add(newRec);
                    }
                }

                this.SaveOrUpdate(newStructElems, this.RoStructElemRep);
            ...
            finally
            {
                ...
                this.LogImportManager.FileNameWithoutExtention = fileData.FileName;
