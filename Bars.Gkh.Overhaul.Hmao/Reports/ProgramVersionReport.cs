namespace Bars.Gkh.Overhaul.Hmao.Reports
{
    ...
    public class ProgramVersionReport : BasePrintForm
    {
        ...
        public ProgramVersionReport()
            : base(new ReportTemplateBinary(Properties.Resources.ProgramVersion))
        {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            var workNamesByStage3Dict = Stage1Domain.GetAll()
                ...
                .WhereIf(municipalityIds.Length > 0, x => municipalityIds.Contains(x.Stage2Version.Stage3Version.ProgramVersion.Municipality.Id))
                .Select(x => new
                {
            ...
            var versionsData = Container.Resolve<IDomainService<VersionRecord>>().GetAll()
                ...
                .AsEnumerable()
                .GroupBy(x => x.MuId)
                .ToDictionary(x => x.Key, y => y.OrderBy(x => x.Year).ThenBy(x => x.IndexNumber).ToList());
            ...
            foreach (var muData in versionsData)
            {
                ...
                sectMunicipality["Municipality"] =
                    muNameById.ContainsKey(muData.Key)
                ...
                foreach (var roData in muData.Value)
                {
                    ...
                    sectRealObj["TypeWorks"] =
                        workNamesByStage3Dict.ContainsKey(roData.Id)
                        && workNamesByStage3Dict[roData.Id].Any()
                            ? workNamesByStage3Dict[roData.Id].AggregateWithSeparator(", ")
                            : string.Empty;
