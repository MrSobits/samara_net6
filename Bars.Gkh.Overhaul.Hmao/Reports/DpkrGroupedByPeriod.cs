namespace Bars.Gkh.Overhaul.Hmao.Reports
{
    ...
    public class DpkrGroupedByPeriod : BasePrintForm
    {
        public DpkrGroupedByPeriod()
            : base(new ReportTemplateBinary(Properties.Resources.DpkrGroupedByPeriod))
        {
        }
        ...
        {
            get { return "Региональная программа капитального ремонта с группировкой по периодам"; }
        }
        ...
        {
            get { return "B4.controller.report.DpkrGroupedByPeriod"; }
        }

        public override string RequiredPermission
        {
            get { return "Ovrhl.DpkrGroupedByPeriod"; }
        }
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            var dpkrDomain = Container.Resolve<IDomainService<DpkrCorrectionStage2>>();

            var data =
                dpkrDomain.GetAll()
                    .Where(x => x.Stage2.Stage3Version.ProgramVersion.IsMain && x.Stage2.Stage3Version.ProgramVersion.Municipality != null)
                    ...
                    .Select(x => new
                    {
                        ...
                        x.PlanYear,
                        CeoGroup = x.Stage2.CommonEstateObject.GroupType.Name
                    })
                    .OrderBy(x => x.munName)
                    .ThenBy(x => x.Address)
                    .AsEnumerable()
            ...
            foreach (var munData in data)
            {
                ...
                foreach (var roData in munData.Value.Values)
                {
                    ...
                    sectionRealObj["MunName"] = relObjData.munName;
                    ...
                    do
                    {
                        ...
                        string[] arrString = roData
                            .Where(x => tempPeriodStart <= x.PlanYear)
                            .Where(x => tempPeriodEnd >= x.PlanYear)
                            .Where(x => !string.IsNullOrEmpty(x.CeoGroup))
                            .Select(x => x.CeoGroup)
                            .Distinct()
                            .ToArray();

                        sectionRealObj[string.Format("CeoGroups{0}", periodCnt)] =
                            arrString.Any()
            ...
            int periodEnd)
        {
            ...
            do
            {
                ...
                verticalSection["Years"] = tempPeriodEnd - tempPeriodStart > 0
                    ? string.Format("{0}-{1}", tempPeriodStart, tempPeriodEnd)
                    : tempPeriodStart.ToString();
                verticalSection["CeoGroups"] = string.Format("$CeoGroups{0}$", periodCnt);
