SubDPKR = (function () {
    ...
    Array.prototype.cw = function () {
        var temp = [];
        for (var y = this.length - 1; y >= 0; y--) {
            for (var x = 0; x < this[y].length; x++) {
                if (!temp[x]) temp[x] = [];
                temp[x][y] = this[this.length - 1 - y][x];
            }
        }
        return temp;
    }
    ...
    Array.prototype.copy = function () {
        ...
        for (var i = 0; i < this.length; i++) {
            copy[i] = this[i] instanceof Array ? this[i].copy() : this[i];
        }
        return copy;
    }

    Array.prototype.exist = function (/* arguments */) {
        var a = this;
        for (var i = 0; i < arguments.length; i++) {
            if (!(arguments[i] >= 0 && arguments[i] < a.length)) {
                return false;
            }
				...
				(function(){
					...
					div.appendChild(
						(function(){
							...
							label.innerHTML = "РРіСЂРѕРє&nbsp;" + (this.N + 1) +"&nbsp;&nbsp;РЎС‡РµС‚:&nbsp;";
							return label;
						}).call(this)
					)
			...
			for(var i = yo; i >= 0; i --){
				if(maps.isFree4(maps.queue[0], maps.y + i, maps.x + xo)){
					...
					if(maps.isFree4(maps.queue[1], maps.y, maps.x)){
						...
						this.gameover = true;
						this.dom.childNodes[1].style.backgroundColor = "#f00";
						/* Game Over */
					}
					break;
				}
