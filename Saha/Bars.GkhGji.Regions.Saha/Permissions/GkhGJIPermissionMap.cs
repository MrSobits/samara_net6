namespace Bars.GkhGji.Regions.Saha.Permissions
{
    ...
    public class GkhGjiPermissionMap: PermissionMap
    {
        ...
        private void AddPermission(string name, string alias, bool onlyView = true)
        {
            this.Namespace(name, alias);
            this.Permission(name + ".View", "Просмотр");
            if (onlyView) return;
            this.Permission(name + ".Edit", "Редактирование");
            this.Permission(name + ".Delete", "Удаление записей");
        }
    }
}