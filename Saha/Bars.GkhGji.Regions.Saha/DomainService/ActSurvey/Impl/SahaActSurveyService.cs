namespace Bars.GkhGji.Regions.Saha.DomainService
{
    ...
    /// колонки новые
    /// </summary>
    public class SahaActSurveyService : GkhGji.DomainService.ActSurveyService
    {
        ...
        {
            var sahaSurveyDomain = Container.Resolve<IDomainService<SahaActSurvey>>();
            ...
            */
            ...
            // Так желаю чтобы просто вьюху неправить sql 
                var dictSahaActSurveys =
                    sahaSurveyDomain.GetAll()
            ...
            var data = GetViewList()
                ...
                .Select(x => new {
                    ...
                    x.TypeBase,
                    x.TypeDocumentGji,
                    ConclusionIssued = dictSahaActSurveys.Get(x.Id)
                })
                .AsQueryable()
            ...
            {
                Container.Release(sahaSurveyDomain);
            }
