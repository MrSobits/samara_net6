//ToDo Контроллер перенесен в самар потому что добавлена субтаблица проверяющих в Самаре, следвоательно добавлены аспекты 
...
Ext.define('B4.controller.AppealCits', {
    ...
    aspects: [
        ...
        },
        {
            ...
            onAfterSetFormData: function (aspect, rec, form) {
                var grid = form.down('appealcitsTesterGrid'),
                    store = grid.getStore();
                
                //Подгружаем стор Проверяющих по Id Обращения
                store.filter('appealCitizensId', rec.getId());
            },
        ...
        {
            /*
            Аспект взаимодействия таблицы проверяющих и массового добавления
            */
            xtype: 'gkhgridmultiselectwindowaspect',
            name: 'appealCitsTesterAspect',
            gridSelector: 'appealcitsTesterGrid',
            modelName: 'appealcits.Tester',
            multiSelectWindow: 'SelectWindow.MultiSelectWindow',
            multiSelectWindowSelector: '#appealCitizensTesterMultiSelectWindow',
            storeSelect: 'dict.InspectorForSelect',
            storeSelected: 'dict.InspectorForSelected',
            titleSelectWindow: 'Выбор проверяющих',
            titleGridSelect: 'Проверяющие инспектора для отбора',
            titleGridSelected: 'Выбранные проверяющие',
            columnsGridSelect: [
            ...
            listeners: {
                getdata: function (asp, records) {
                    ...
                    if (recordIds[0] > 0) {
                        asp.controller.mask('Сохранение', asp.controller.getMainComponent());
                        B4.Ajax.request({
                            url: B4.Url.action('AddTesters', 'AppealCitsTester', {
                            ...
                            asp.controller.unmask();
                            asp.getGrid().getStore().reload();
                            return true;
                        }).error(function () {
                            asp.controller.unmask();
                        });
                    } else {
                        Ext.Msg.alert('Ошибка!', 'Необходимо выбрать проверяющих');
                        return false;
                    }
                    return true;
                }
    ...
    setCurrentId: function(id, numberGji) {
        ...
        tabpanel.down('appealcitsTesterGrid').tab.setDisabled(!id);
