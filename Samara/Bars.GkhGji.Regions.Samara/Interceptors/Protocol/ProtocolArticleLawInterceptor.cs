namespace Bars.GkhGji.Regions.Samara.Interceptors
{
    ...
    {
        private const string articleCode19 = "1";

        public override IDataResult BeforeCreateAction(IDomainService<ProtocolArticleLaw> service, ProtocolArticleLaw entity)
        {
            /*
            Если для протокола созданного из предписания патаются создать cтатью с кодом 19.5 то выдаем сообщение - По требованию 41181
            */
            ...
            try
            {
                if (artLawDomain.GetAll().Any(x => x.Id == entity.ArticleLaw.Id && x.Code == articleCode19) &&
            ...
            finally
            {
                ...
                Container.Release(artLawDomain);
            }
        }
    }
}
