...
namespace Bars.GkhGji.Regions.Samara
{
    ...
    public class InspectionReminderRule : IReminderRule
    {
        ...
        private List<Reminder> GetRemindersIfPrescriptionNotValid(InspectionGji obj)
        {
            ...
            var prescrCanceldomain = Container.Resolve<IDomainService<PrescriptionCancel>>();
            ...
            try
            {
                ...
                var prescriptions = servPrescription.GetAll()
                                ...
                                .Where(x => !prescrCanceldomain.GetAll().Any(y => y.Prescription.Id == x.Id && y.TypeCancel == TypePrescriptionCancel.CompletelyCancel))
            ...
            finally
            {
                ...
                Container.Release(prescrCanceldomain);
            }
        }
            ...
            try
            {
                ...
                if (!servDocumentGji.GetAll().Any(x => x.Inspection.Id == obj.Id))
                {
                    ...
                    if (obj.TypeBase == TypeBase.DisposalHead)
                    {
                        ...
                        if (dispHead != null && dispHead.Head != null
                            && !inspectionInspectors.Contains(dispHead.Head.Id))
                        {
        ...
        private List<Reminder> GetRemindersIfDisposalNotValid(InspectionGji obj)
        {
            ...
            try
            {
                ...
                listIds = this.FilterDisposalsForSave(listIds, obj);
            ...
            try
            {
                ...
                if (listActCheckYes.Any())
                {
                    ...
                    var actWithProtocolChildren = servDocumentGjiChildren.GetAll()
                        .Where(x => listActCheckYes.Contains(x.Parent.Id))
                    ...
                    actCheckNotValidIds.AddRange(servActCheckViol.GetAll()
                                        .Where(x => listActCheckYes.Contains(x.Document.Id) && !actWithProtocolChildren.Contains(x.Document.Id))
        ...
        private List<long> FilterDisposalsForSave(List<long> disposalIdsForSave, InspectionGji obj)
        {
            var servDisposal = Container.Resolve<IDomainService<Disposal>>();
            var servDocumentGjiChildren = Container.Resolve<IDomainService<DocumentGjiChildren>>();
            var servPrescriptionGjiViolation = Container.Resolve<IDomainService<PrescriptionViol>>();
            ...
            try
            {
                ...
                var disposalIdsToExclude = servDisposal.GetAll()
                    .Where(x => x.Inspection.Id == obj.Id)
                ...
                var disposalIdsNotToExclude = servDocumentGjiChildren.GetAll()
                    .Join(
                    ...
                    .Where(x => x.disposal.Inspection.Id == obj.Id)
                    .Where(x => x.prescription.Inspection.Id == obj.Id)
                ...
                disposalIdsToExclude = disposalIdsToExclude.Except(disposalIdsNotToExclude).ToList();

                var result = disposalIdsForSave.Where(x => !disposalIdsToExclude.Contains(x)).ToList();
