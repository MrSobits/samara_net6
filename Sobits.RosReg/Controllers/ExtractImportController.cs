namespace Sobits.RosReg.Controllers
{
    ...
    public class ExtractImportController : BaseController
    {
        public ActionResult GetImportList(BaseParams baseParams)
        {
            try
            {
                ...
                using (this.Container.Using(importList, importProvider, authorizationServices, userIdentity))
                {
        ...
        public ActionResult Import(BaseParams baseParams)
        {
            ...
            using (this.Container.Using(importProvider))
            {
                ...
                try
                {
                    ...
                    if (result.Success)
                    {
                        actionResult = new JsonNetResult(
                            new
                            {
                    ...
                    if (result.Success)
                    {
                        actionResult = new JsonNetResult(
                            new
                            {
                                success = result.Success,
                                title = string.Empty,
                                message = $"Импорт завершился {result.StatusImport}"
                            });
                    }
                    else
                    {
