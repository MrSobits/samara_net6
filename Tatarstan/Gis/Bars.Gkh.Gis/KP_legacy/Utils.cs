namespace Bars.Gkh.Gis.KP_legacy
{
    ...
    //----------------------------------------------------------------------
    {
        ...
        public static readonly Regex Ipv4Regex =
            new Regex(
                @"((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])",
                RegexOptions.IgnoreCase | RegexOptions.Compiled);

        public static readonly Regex Ipv6Regex =
            new Regex(
                ...
                (2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|
                ...
                if (RolesVal.Count > 0)
                {
                    ...
                    foreach (_RolesVal role in RolesVal)
                    {
                        if (role.tip == tip & role.kod == kod)
                        {
                            ...
                            if (Utils.ValInString(role.val, val, ",")) return true;
                        }
                    }
                }
        ...
        //----------------------------------------------------------------------
        {
            return Utils.EFlo0(f.ToString(), "0.00");
        }
        //----------------------------------------------------------------------
        public static string EFlo0(string f)
        //----------------------------------------------------------------------
        {
            return Utils.EFlo0(f, "0.00");
        }
        //----------------------------------------------------------------------
        ...
        {
            int num;
            return Utils.GetParams(prms, p.ToString(), out num);
        }
        public static bool GetParams(string prms, string p)
        {
            int num;
            return Utils.GetParams(prms, p, out num);
        }
        public static bool GetParams(string prms, int p, out int sequenceNumber)
        {
            return Utils.GetParams(prms, p.ToString(), out sequenceNumber);
        }
        ...
        public class CssClasses
        {
            ...
            public CssClasses(string classes)
            {
                ...
                this._classes = cls.ToList<string>();
                this._count = this._classes.Count;
            }
            ...
            public CssClasses AddClass(string className)
            {
                ...
                {
                    if (this._classes[i] == className) return this;
                }
            ...
            public CssClasses RemoveClass(string className)
            {
                if (className != "")
                {
                    while (this._classes.IndexOf(className) >= 0) this._classes.Remove(className);
                }
                this._count = this._classes.Count;
                return this;
            }
        ...
        //----------------------------------------------------------------------
        {
            Utils.GetIdMonth(id.ToString(), ref y, ref m);
        }
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
        {
            ...
            {
                s = s.Substring(0, l - k);
                if (int.TryParse(s, out var sResult))
                {
                    return sResult;
                }
        ...
        {
            long nzp;
            return Utils.FindKeyFromSpis(pFindString, pDelimiter, pPositionInSpis, out nzp);
        }
        //----------------------------------------------------------------------
        // Поиск и выдача целого значения из строкового списка (pFindString) значений разделенных 
        // символьной строкой (pDelimiter) по позиции в списке (pPositionInSpis)
        ...
        //----------------------------------------------------------------------
        {
            ...
            return Utils.BarcodeCRC10(kod_erc + s);
        }
        //----------------------------------------------------------------------
        ...
        //----------------------------------------------------------------------
        {
            ...
            Int64 test = Utils.EncodePKod(kod_erc.ToString(), num_ls);
        ...
        {
            return Utils.ConvertDBFtoDataTable(fs, "", out ret);
        }

        public static DataTable ConvertDBFtoDataTable(Stream fs, string codePage, out Returns ret)
        {
            return Utils.ConvertDBFtoDataTable(fs, codePage, false, out ret);
        }
        ...
        /// <summary>Получение названия месяца в родительном падеже по целочисленному значению</summary>
        /// <param name="month">Месяц</param>
        /// <returns>Наименование месяца</returns>
        static public string GetMonthNameR(int month)
        {
            switch (month)
            {
                case 1:
        ...
        /// <summary>
        /// Получить ip адрес текущего компьютера, если ip не получен, то вернет string.Empty
        /// </summary>
        /// <returns></returns>        public static string GetCurrentIpAddressOrEmpty()        {            return GetCurrentIpAddressOrDefault()?.ToString() ?? string.Empty;

    /// <summary>
    /// Получить объект IpAddres
    /// </summary>
    /// <returns></returns>
    public static IPAddress GetCurrentIpAddressOrDefault()
    {
        var host = Dns.GetHostEntry(Dns.GetHostName());
        return host.AddressList.FirstOrDefault(x => x.AddressFamily == AddressFamily.InterNetwork);
    }

    /// <summary>
    /// Парсит строку на наличие ip адреса в формате ipv4 или ipv6
    /// </summary>
    /// <param name="input">строка для парсинга</param>
    /// <param name="ipAddress">результат</param>
    /// <returns></returns>
    public static bool TryParseIpAddressRegex(string input, out IPAddress ipAddress)
    {
        ipAddress = null;
        string ip = string.Empty;
        //IPv4 RegEx
        if (Utils.Ipv4Regex.IsMatch(input))
        {
            ip = Utils.Ipv4Regex.Match(input).Value;
        }
        //IPv6 RegEx
        else if (Utils.Ipv6Regex.IsMatch(input))
        {
            ip = Utils.Ipv6Regex.Match(input).Value;
        }
        return ip.IsNotEmpty() && IPAddress.TryParse(ip, out ipAddress);
    }
    ...
    public class ObjectMemberInfoAttribute : Attribute
    {
        /// <summary>
        /// Наименование поля
        /// </summary>
        ...
        public ObjectMemberInfoAttribute(string description)
        {
            this.Ignore = false;
            this.Description = description;
        }

        public ObjectMemberInfoAttribute(string description, bool ignore)
        {
            this.Ignore = ignore;
            this.Description = description;
        }
    ...
    /// <summary>
    /// Получение различающихся полей сущности
    /// </summary>
    /// <typeparam name="T">Тип сравниваемой сущности</typeparam>
    /// <param name="oldEntity">Старое значение сущности</param>
    /// <param name="newEntity">Новое значение сущности</param>
    /// <returns>Описание измененных полей</returns>
    public static List<KeyValuePair<string, KeyValuePair<object, object>>> GetEntityDifference<T>(T oldEntity,
        T newEntity) where T : class, new()
    {
        var result = new List<KeyValuePair<string, KeyValuePair<object, object>>>();

        //получаем все свойства сущности и записываем различающиеся свойства
        result.AddRange(from property in typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                        where
                        property.GetAttribute<ObjectMemberInfoAttribute>(false) != null &&
                        !property.GetAttribute<ObjectMemberInfoAttribute>(false).Ignore &&
                            property.GetValue(oldEntity, null) != null &&
                            !(property.GetValue(oldEntity, null) == null ? "" : property.GetValue(oldEntity, null).ToString().Trim()).Equals(property.GetValue(newEntity, null) == null ? "" : property.GetValue(newEntity, null).ToString().Trim())
                        let propertyDescription =
                            property.GetAttribute<ObjectMemberInfoAttribute>(false)?.Description
                        select
                            new KeyValuePair<string, KeyValuePair<object, object>>(propertyDescription,
                                new KeyValuePair<object, object>(property.GetValue(oldEntity, null),
                                    property.GetValue(newEntity, null))));


        //получаем все поля сущности и записываем различающиеся поля
        result.AddRange(from field in typeof(T).GetFields(BindingFlags.Public | BindingFlags.Instance)
                        where
                        field.GetAttribute<ObjectMemberInfoAttribute>(false) != null &&
                        !field.GetAttribute<ObjectMemberInfoAttribute>(false).Ignore &&
                        field.GetValue(oldEntity) != null &&
                        !field.GetValue(oldEntity).Equals(field.GetValue(newEntity))
                        let propertyDescription =
                            field.GetAttribute<ObjectMemberInfoAttribute>(false)?.Description
                        select
                            new KeyValuePair<string, KeyValuePair<object, object>>(propertyDescription,
                                new KeyValuePair<object, object>(field.GetValue(oldEntity), field.GetValue(newEntity))));
        return result;
    }
