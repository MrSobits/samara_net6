namespace Bars.Gkh.Ris.Extractors.Bills
{
    ...
    /// <summary>
    /// Экстрактор "Начисление по услуге - Главная коммунальная услуга с объемом потребления"
    /// </summary>
    public class RisMunicipalServiceChargeInfoDataExtractor : BaseSlimDataExtractor<RisMunicipalServiceChargeInfo>
    {
        /// <summary>
        ...
        public override List<RisMunicipalServiceChargeInfo> Extract(DynamicDictionary parameters)
        {
            ...
            var risMunicipalServiceChargeInfoDomain = this.Container.ResolveDomain<RisMunicipalServiceChargeInfo>();
            using (this.Container.Using(risMunicipalServiceChargeInfoDomain))
            {
                return risMunicipalServiceChargeInfoDomain.GetAll()
        ...
        private bool ExecuteProcedures(long[] houseIds, DateTime reportingPeriod)
        {
            ...
            try
            {
                ...
                var procedures = new[]
                {
                    "ris_transfer_municipal_service_charge_info"
                };
