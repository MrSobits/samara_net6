namespace Bars.Gkh.Regions.Tatarstan.Controller
{
    ...
    public class NormConsumptionHotWaterController : B4.Alt.DataController<NormConsumptionHotWater>
    {
        public INormConsumptionService Service { get; set; }

        /// <summary>
        /// Получить нормы потребление горячей воды
        /// </summary>
        /// <param name="baseParams">Базовые параметры</param>
        /// <returns>Результат выполнения запроса</returns>
        public ActionResult ListNormConsumptionHotWater(BaseParams baseParams)
        {
            int totalCount;
            var result = this.Service.GetNormConsumptionHotWaterQuery(baseParams, out totalCount).ToList();
        ...
        public ActionResult Export(BaseParams baseParams)
        {
            ...
            int totalCount;
            var data = this.Service.GetNormConsumptionHotWaterQuery(baseParams, out totalCount).ToList();
