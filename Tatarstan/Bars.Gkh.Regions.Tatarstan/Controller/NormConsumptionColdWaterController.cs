namespace Bars.Gkh.Regions.Tatarstan.Controller
{
    ...
    public class NormConsumptionColdWaterController : B4.Alt.DataController<NormConsumptionColdWater>
    {
        public INormConsumptionService Service { get; set; }

        /// <summary>
        /// Получить нормы потребление холодной воды
        /// </summary>
        /// <param name="baseParams">Базовые параметры</param>
        /// <returns>Результат выполнения запроса</returns>
        public ActionResult ListNormConsumptionColdWater(BaseParams baseParams)
        {
            int totalCount;
            var result = this.Service.GetNormConsumptionColdWaterQuery(baseParams, out totalCount).ToList();
        ...
        public ActionResult Export(BaseParams baseParams)
        {
            ...
            int totalCount;
            var data = this.Service.GetNormConsumptionColdWaterQuery(baseParams, out totalCount).ToList();
