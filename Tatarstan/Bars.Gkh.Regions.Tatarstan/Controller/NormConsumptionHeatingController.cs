namespace Bars.Gkh.Regions.Tatarstan.Controller
{
    ...
    public class NormConsumptionHeatingController : B4.Alt.DataController<NormConsumptionHeating>
    {
        public INormConsumptionService Service { get; set; }

        /// <summary>
        /// Получить нормы потребление подогрева
        /// </summary>
        /// <param name="baseParams">Базовые параметры</param>
        /// <returns>Результат выполнения запроса</returns>
        public ActionResult ListNormConsumptionHeating(BaseParams baseParams)
        {
            int totalCount;
            var result = this.Service.GetNormConsumptionHeatingQuery(baseParams, out totalCount).ToList();
        ...
        public ActionResult Export(BaseParams baseParams)
        {
            ...
            int totalCount;
            var data = this.Service.GetNormConsumptionHeatingQuery(baseParams, out totalCount).ToList();
