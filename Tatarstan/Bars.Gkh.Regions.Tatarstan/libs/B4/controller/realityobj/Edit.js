Ext.define('B4.controller.realityobj.Edit', {
    ...
    requires: [
        ...
        'B4.aspects.permission.realityobj.Toolbar',
    ...
    aspects: [
        {
            xtype: 'realityobjtoolbarpermissionaspect',
            name: 'realityObjToolbarPermissionAspect'
        },
        {
        ...
        },
        {
            ...
            afterSetRequirements: function (rec) {
                ...
                var privDateArr =
                    Ext.ComponentQuery.query(asp.controller.mainViewSelector +
                        ' [name="PrivatizationDateFirstApartment"]');
                var hasPrivatizedFlatsArr =
                    Ext.ComponentQuery.query(asp.controller.mainViewSelector + ' #hasPrivatizedFlats');
                ...
                if (privDateArr.length > 0 && hasPrivatizedFlatsArr.length) {
                    ...
                    if (hasPrivatizedFlats.value &&
                        (!hasPrivatizedFlats.isDisabled() || hasPrivatizedFlats.resetDisable)) {
                        privDate.enable();
                    } else {
                        privDate.disable();
                    }
        ...
        },
        {
            ...
            otherActions: function (actions) {
                ...
                actions[this.editPanelSelector + ' #btnSendTp'] = { 'click': { fn: this.onClickbtnSendTp, scope: this } };
            ...
            onClickbtnSendTp: function () {
                var me = this,
                    realityObjectId = me.controller.getContextValue(me.controller.getMainComponent(),
                        'realityObjectId');

                me.controller.mask('Отправка', me.controller.getMainComponent());

                B4.Ajax.request({
                    url: B4.Url.action('SendTechPassport', 'Format40DataExport'),
                    params: {
                        realityObjectId: realityObjectId
                    }
                })
                    .next(function (response) {
                        B4.QuickMsg.msg('Отправка ТехПаспорта в ГИС ЖКХ', 'Задача успешно добавлена', 'success');
