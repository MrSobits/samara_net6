//ToDo перекрыл виджет поскольку в РТ добавляется еще напоминание с типом Постановление
        ...
        if (!this.isBuilt) {
            if (records.length > 0) {
                        ...
                        '<div class="task-list">',
                             '<div class="legend-item">',
                                 '<ul class="squares max">',
                                    ...
                                    '<li class="white" data-qtip="Основания проверок, по которым не сформированы распоряжения">Нет распоряжения</li>',
                                 '</ul>',
                             '</div>',
                             '<div class="head-item">',
                             ...
                             '<div class="task-list-item">',
                                 '<div class="task-list-item-inner">',
                                     '<div class="pull-left">',
                                         '<span class="title">Распоряжение</span>',
                                     '</div>',
                                     '<div class="pull-right">',
                             ...
                             '<div class="task-list-item">',
                                 '<div class="task-list-item-inner">',
                                     '<div class="pull-left">',
                                         '<span class="title">Постановление</span>',
                                     '</div>',
                                     '<div class="pull-right">',
                                         '<ul class="squares" typeWidget="resolution">',
                                             '<li class="red">' + (recordsDict.containsKey('50') ? recordsDict.get('50').red : 0) + '</li>',
                                             '<li class="yellow">' + (recordsDict.containsKey('50') ? recordsDict.get('50').yellow : 0) + '</li>',
                                             '<li class="green">' + (recordsDict.containsKey('50') ? recordsDict.get('50').green : 0) + '</li>',
                                             '<li class="white">0</li>',
                                         '</ul>',
                                     '</div>',
                                     '<div class="clearfix"></div>',
                ...
                Ext.select('ul[typeWidget="appeal"] li').on('click', function () {
                    me.fireEvent('openregistrtaskstate', this.getAttribute('class'), 10);
                });
                
                Ext.select('ul[typeWidget="prescription"] li').on('click', function () {
                    me.fireEvent('openregistrtaskstate', this.getAttribute('class'), 40);
                });
                
                Ext.select('ul[typeWidget="resolution"] li').on('click', function () {
                    me.fireEvent('openregistrtaskstate', this.getAttribute('class'), 50);
                });
                
                Ext.select('ul[typeWidget="disposal"] li').on('click', function () {
                    me.fireEvent('openregistrtaskstate', this.getAttribute('class'), 30);
                });
                
                Ext.select('ul[typeWidget="total"] li').on('click', function () {
                    me.fireEvent('openregistrtaskstate', this.getAttribute('class'), 0);
                });
