Ext.define('B4.controller.GkhConfig', {
    ...
    onAfterSave: function(form, keys, resp) {
        ...
        if (failedFields.length == 0) {
            ...
            // Запуск доп. действия при сохранении конфигурации
            this.runAction(keys);
        } else {
            Ext.Msg.alert('Сохранение', 'Часть параметров не удалось сохранить:<br/>' + failedFields.join('<br/>'));
        }
    },
    runAction: function (keys){
        if (Ext.Array.contains(keys,'Appeal.RapidResponseSystemConfig.ControlPeriodParameter')){
            this.updateSoprControlPeriod();
        }
    },
    updateSoprControlPeriod: function () {
        var me = this;
        me.mask();

        B4.Ajax.request({
            url: B4.Url.action('UpdateSoprControlPeriod', 'RapidResponseSystemAppeal'),
        })
        .next(function () {
            B4.QuickMsg.msg('Успешно','Контрольный срок решения обращения в реестре СОПР успешно обновлён');
            me.unmask()
        })
        .error(function () {
            Ext.Msg.alert('Ошибка','Произошла ошибка при выполнении действия обновления контрольного срока решения обращения в реестре СОПР');
            me.unmask()
        });
    }
});