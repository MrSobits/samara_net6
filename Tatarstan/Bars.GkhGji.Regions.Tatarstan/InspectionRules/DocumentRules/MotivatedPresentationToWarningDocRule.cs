namespace Bars.GkhGji.Regions.Tatarstan.InspectionRules
{
    ...
    public class MotivatedPresentationToWarningDocRule : IDocumentGjiRule
    {
        ...
        /// <inheritdoc />
        public string Id => nameof(MotivatedPresentationToWarningDocRule);

        /// <inheritdoc />
        public string Description => "Правило создания документа 'Предостережение' из документа 'Мотивированное представление КНМ без взаимодействия с контролируемыми лицами'";
        ...
        /// <inheritdoc />
        public TypeDocumentGji TypeDocumentInitiator => TypeDocumentGji.MotivatedPresentation;
        ...
        {
            var _houseIds = baseParams.Params.GetAs("houseIds", "");

            this.houseIds = _houseIds.ToLongArray();
        ...
        public IDataResult CreateDocument(DocumentGji document)
        {
            ...
            var currentStage = this.InspectionStageDomain
                .GetAll()
                .FirstOrDefault(x => x.Parent.Id == parentStage.Id && x.TypeStage == TypeStage.WarningDoc);
            ...
            var listActViol = this.MotivatedPresentationViolationDomain.GetAll()
                .Where(x => x.MotivatedPresentationRealityObject.MotivatedPresentation.Id == document.Id && this.houseIds.Contains(x.MotivatedPresentationRealityObject.RealityObject.Id))
                .Select(x => new
                {
                    x.MotivatedPresentationRealityObject.RealityObject,
                    x.Violation
                });
        ...
        public IDataResult ValidationRule(DocumentGji document)
        {
            if (this.ChildrenDomain.GetAll()
                .Any(x => x.Children.Id == document.Id 
                    && x.Parent.TypeDocumentGji == TypeDocumentGji.VisitSheet))
            {
                return new BaseDataResult(false, "Мотивированное представление не должно быть сформировано из документа 'Лист визита'");
            }
