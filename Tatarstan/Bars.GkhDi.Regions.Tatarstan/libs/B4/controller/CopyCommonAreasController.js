Ext.define('B4.controller.CopyCommonAreasController', {
    ...
    views: ['service.CopyCommonAreasEditPanel'],
    mainView: 'service.CopyCommonAreasEditPanel',
    mainViewSelector: '#copyCommonAreasEditPanel',
    ...
    init: function () {
        var actions = {};
        actions['copyCommonAreasEditPanel b4savebutton'] = { 'click': { fn: this.btnCopyClick, scope: this } };
    ...
    onMainViewAfterRender: function () {
        if (this.params) {
            ...
            B4.Ajax.request(B4.Url.action('GetInfoServPeriod', 'Service', {
                ...
                me.unmask();
                Ext.Msg.alert('Ошибка', 'Не удалось получить сведения о местах общего пользования');
        ...
        if (copyEditPanel.getForm().isValid()) {
            Ext.Msg.confirm('Подтверждение', 'Скопировать список сведений в выбранный отчетный период?', function(result) {
                if (result == 'yes') {
                    ...
                    Ext.Ajax.request({
                        url: B4.Url.action('/Service/CopyCommonAreasPeriod'),
                        params: {
