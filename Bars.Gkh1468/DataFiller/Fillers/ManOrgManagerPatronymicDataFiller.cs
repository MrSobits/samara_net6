namespace Bars.Gkh1468.DataFiller.Fillers
{
    ...
    /// <summary>
    /// Отчество руководителя для загрузки/выгрузки данных паспорта МКД
    /// </summary>
    public class ManOrgManagerPatronymicDataFiller : IDataFiller
    {
        /// <summary>
        /// Code
        /// </summary>
        public static string Code
        {
            get
            {
                return "ManOrgManagerPatronymicDataFiller";
            }
        }
        ...
        public void To1468()
        {
            ...
            if (contractData.Count > 1)
            {
                ...
                if (contractId.HasValue)
                {
                    ...
                    {
                        var managerPatronymic = this.Container.Resolve<IDomainService<ContragentContact>>().GetAll()
                            .Where(x => x.Contragent.Id == contragentId)
                            .Select(x => x.Patronymic)
                            .FirstOrDefault();

                        result = managerPatronymic;
                    }
                }
            }
            else
