        ...
        public void Create(IEntity entity)
        {
            if (entity is AppealCits)
            {
                ...
                try
                {
                    ...
                    if (!obj.Return(x => x.State).Return(x => x.FinalState))
                    {
                        ...
                        var executants =
                            servAppCitsExecutant.GetAll()
                                                .Where(x => x.AppealCits.Id == obj.Id)
                                                .Select(
                                                    x => new { AuthorId = x.Author.Id, ExecutantId = x.Executant.Id })
                                                .ToList();

                        var listInspectors = executants.Select(x => x.ExecutantId).Distinct().ToList();

                        listInspectors.AddRange(executants.Where(x => !listInspectors.Contains(x.AuthorId)).Select(x => x.AuthorId).Distinct().ToList());
