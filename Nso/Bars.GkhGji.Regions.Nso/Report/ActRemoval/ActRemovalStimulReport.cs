namespace Bars.GkhGji.Regions.Nso.Report
{
	...
	public class ActRemovalStimulReport : GkhBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            var actRemovalWitnessDomain = Container.Resolve<IDomainService<ActRemovalWitness>>();
            var actRemovalProvidedDocDomain = Container.Resolve<IDomainService<ActRemovalProvidedDoc>>();
            ...
            var actRemovalPeriodDomain = Container.Resolve<IDomainService<ActRemovalPeriod>>();
            var dayDomain = Container.Resolve<IDomainService<Day>>();
            ...
            try
            {
                ...
                if (actCheckPeriods.Count > 0)
                {
                    ...
                    {
                        interval = GetInteval(firstDay.DateStart.ToDateTime(), firstDay.DateEnd.ToDateTime());
                        workDays--;
                    }

                    if (actCheckPeriods.Count > 1 && days.ContainsKey(lastDay.DateCheck.ToDateTime()))
                    {
                        interval += GetInteval(lastDay.DateStart.ToDateTime(), lastDay.DateEnd.ToDateTime());
                        workDays--;
                    }
            ...
            finally 
            {
                ...
                Container.Release(actRemovalProvidedDocDomain);
                Container.Release(contragentContactDomain);
                Container.Release(disposalProvidedDocDomain);
                Container.Release(actRemovalWitnessDomain);
                ...
                Container.Release(actRemovalPeriodDomain);
