...
namespace Autotests.Utilities.WebElement
{
    ...
    public partial class WebElement
    {
        ...
        public void DoubleClick()
        {
            var element = FindSingle();
            //Contract.Assert(element.Enabled);
            var builder = new Actions(Browser.WebDriver);
            try
            {
        ...
        public WebElement WaitHideElement()
        {
            try
            {
                Browser.WaitUntil(!Displayed);
        ...
        public WebElement WaitUntilEnabled()
        {
            var x = new Func<bool>(() => GetAttribute(TagAttributes.Disabled) == null);
            WaitHelper.SpinWait(x, TimeSpan.FromSeconds(20), TimeSpan.FromMilliseconds(100));
            return this;
        }

        public WebElement FindWebElementWithExJS(string query)
        {
            string js = "return Ext.ComponentQuery.query(\'" + query + "\')[0].id";
            //Contract.Assert(!string.IsNullOrEmpty(js));
            Thread.Sleep(TimeSpan.FromMilliseconds(1500));
            string id = (string)Browser.ExecuteJavaScript(js);
            return new WebElement().ById(id,false);
        }
        public static string FindIdWithExtJS(string query)
        {
            string js = "return Ext.ComponentQuery.query(\'" + query + "\')[0].id";
            //Contract.Assert(!string.IsNullOrEmpty(js));
            Thread.Sleep(TimeSpan.FromMilliseconds(1500));
            return (string)Browser.ExecuteJavaScript(js);
        }

        public WebElement WaitIsClickable()
        {
            Browser.WaitUntil(Displayed & Enabled);
            return this;
        }
        #endregion
