...
namespace Autotests.Utilities
{
    ...
    public static class Browser
    {
        ...
        {
            var ready = new Func<bool>(() => (bool)ExecuteJavaScript("return (typeof($) === 'undefined') ? true : jQuery.active == 0"));
            WaitHelper.SpinWait(ready, TimeSpan.FromSeconds(60), TimeSpan.FromMilliseconds(100));
        }
        ...
        {
            WebDriverWait iWait = new WebDriverWait(Browser.WebDriver, TimeSpan.FromSeconds(120));
            iWait.Until(ExpectedConditions.InvisibilityOfElementLocated(iClassName));
        }
        public static bool WaitUntil(bool condition)
        {
            return WaitHelper.SpinWait(() => condition, TimeSpan.FromSeconds(40));
        }
        ...
        private static ChromeDriver StartChrome()
        {
            ...
            chromeOptions.AddUserProfilePreference("download.default_directory", pathDirectory);
