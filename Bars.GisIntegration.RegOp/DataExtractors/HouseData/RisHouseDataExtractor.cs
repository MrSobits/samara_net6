namespace Bars.GisIntegration.RegOp.DataExtractors.HouseData
{
    ...
    using Gkh.Decisions.Nso.Entities;
    using Gkh.Domain;
    using Gkh.Enums.Decisions;
    using Gkh.Modules.Gkh1468.Entities;
    ...
    public class RisHouseDataExtractor : BaseDataExtractor<RisHouse, RealityObject>
    {
        ...
        public override List<RealityObject> GetExternalEntities(DynamicDictionary parameters)
        {
            ...
            if (parameters.ContainsKey("regOperatorId"))
            {
                return this.GetRealityObjectListByRegOperator(parameters);
            }
        ...
        protected override void UpdateRisEntity(RealityObject realityObject, RisHouse risHouse)
        {
            ...
            risHouse.FiasHouseGuid = realityObject.HouseGuid ?? realityObject.FiasAddress.HouseGuid?.ToString();
            ...
            risHouse.OktmoCode = this.oktmoByFiasId?.Get(realityObject.FiasAddress.PlaceGuidId);  //realityObject.FiasAddress Municipality?.Oktmo?.ToString() ?? "";
        ...
        private List<RealityObject> GetRealityObjectListByOMS(DynamicDictionary parameters)
        {
            ...
            selectedIds = selectedHouses.ToUpper() == "ALL" ? null : selectedHouses.ToLongArray();
        ...
        /// <summary>
        /// Получить список домов по региональному оператору
        /// </summary>
        /// <param name="parameters">Параметры запроса</param>
        /// <returns>Список домов</returns>
        private List<RealityObject> GetRealityObjectListByRegOperator(DynamicDictionary parameters)
        {
            long[] selectedIds;
            var selectedHouses = parameters.GetAs("selectedHouses", string.Empty);
            ...
            var contragentId = parameters.GetAs<long>("regOperatorId");
            var result = new List<RealityObject>();
            ...
            try
            {
                ...
                var regOperator = regOperatorDomain.GetAll().FirstOrDefault(x => x.Contragent.Id == contragentId);

                if (regOperator != null)
                {
                    result.AddRange(
                        calcAccountRealityObjectDomain.GetAll()
                                .WhereIf(selectedIds != null, x => selectedIds.Contains(x.RealityObject.Id))
            ...
            {
                this.Container.Release(calcAccountRealityObjectDomain);
