namespace Bars.GisIntegration.RegOp.DataExtractors.Bills
{
    ...
    /// <summary>
    /// Экстрактор "Задолженность за капитальный ремонт"
    /// </summary>
    public class RisCapitalRepairDebtDataExtractor : BaseSlimDataExtractor<RisCapitalRepairDebt>
    {
        /// <summary>
        ...
        public override List<RisCapitalRepairDebt> Extract(DynamicDictionary parameters)
        {
            ...
            var risCapitalRepairDebtDomain = this.Container.ResolveDomain<RisCapitalRepairDebt>();

            using (this.Container.Using(risCapitalRepairDebtDomain))
            {
                return risCapitalRepairDebtDomain.GetAll()
        ...
        private bool ExecuteProcedures(long[] extractedPayDocs, DateTime reportingPeriod)
        {
            ...
            try
            {
                ...
                var procedures = new[] { "ris_transfer_capital_repair_debt" };
