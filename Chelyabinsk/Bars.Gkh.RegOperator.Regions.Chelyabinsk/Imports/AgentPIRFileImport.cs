namespace Bars.Gkh.RegOperator.Regions.Chelyabinsk.Imports
{
    ...
    /// Импорт ПИР
    /// </summary>
    public partial class AgentPIRFileImport : GkhImportBase
    {
        /// <summary>
        /// Id
        /// </summary>
        ...
        /// Ключ импорта
        /// </summary>
        public override string Key => AgentPIRFileImport.Id;

        /// <summary>
        /// Код группировки импорта (например группировка в меню)
        /// </summary>
        public override string CodeImport => nameof(AgentPIRFileImport);

        /// <summary>
        /// Наименование импорта
        /// </summary>
        public override string Name => "Импорт файлов документов агента ПИР";
        ...
        /// Права
        /// </summary>
        public override string PermissionName => "Import.AgentPIRFileImport";
        ...
        private void CreateDocument(ZipEntry[] zipEntries)
        {
            ...
            foreach (var file in zipEntries)
            {
                ...
                {
                    var idfrombane = file.FileName.Split('.')[0];
                    try
                    {
                        var docId = Convert.ToInt64(idfrombane);
                        var docForPDF = AgentPIRDocumentDomain.Get(docId);
                        if (docForPDF != null)
                        {
                            docForPDF.File = fileinfo;
                            AgentPIRDocumentDomain.Update(docForPDF);
                        }
                    }
                    catch
                    {
