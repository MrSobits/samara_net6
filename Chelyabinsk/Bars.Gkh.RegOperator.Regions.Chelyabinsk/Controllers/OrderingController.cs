namespace Bars.Gkh.RegOperator.Regions.Chelyabinsk.Controllers
{
    ...
    {
        public ActionResult DownloadOrdering(BaseParams baseParams)
        {
            ...
            try
            {
                var accId = baseParams.Params.GetAs<long>("Id");
                var debtor = debtorDomain.GetAll().FirstOrDefault(x => x.BasePersonalAccount.Id == accId);
                if (debtor != null && debtor.Ordering != null)
                {
                    return new JsonNetResult(new BaseDataResult(debtor.Ordering.Id));
                }
