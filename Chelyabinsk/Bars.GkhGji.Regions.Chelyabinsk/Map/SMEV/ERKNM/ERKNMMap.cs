namespace Bars.GkhGji.Regions.Chelyabinsk.Map
{
    ...
    public class ERKNMMap : BaseEntityMap<ERKNM>
    {
        
        public ERKNMMap() : 
                base("Обмен данными с ГИС ЕРП", "GJI_CH_GIS_ERKNM")
        {
        }
        ...
        protected override void Map()
        {
            ...
            Reference(x => x.AppealCitsAdmonition, "AppealCitsAdmonition").Column("ADMON_ID");
            Reference(x => x.Inspector, "Инспектор").Column("INSPECTOR_ID").Fetch();
            Reference(x => x.Disposal, "Протокол").Column("DISPOSAL_ID").Fetch();
