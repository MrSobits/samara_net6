...
{
    public class SendCalcRequestTaskExecutor : ITaskExecutor
    {
        ...
        public SendCalcRequestTaskExecutor(IGISGMPService GISGMPService)
        {
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            try
            {
                var result = _GISGMPService.SendCalcRequest(smevRequestData, indicator);
                if(!result)
