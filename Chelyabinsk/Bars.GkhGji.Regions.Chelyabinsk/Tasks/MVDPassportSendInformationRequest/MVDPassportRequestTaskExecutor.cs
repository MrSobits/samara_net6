...
{
    public class MVDPassportRequestTaskExecutor : ITaskExecutor
    {
        ...
        public MVDPassportRequestTaskExecutor(IMVDPassportService MVDPassportService)
        {
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            MVDPassport request = MVDPassportDomain.Get(taskId);
            if (request == null)
            ...
            try
            {
                var result = _MVDPassportService.SendInformationRequest(request, indicator);
                if (!result)
