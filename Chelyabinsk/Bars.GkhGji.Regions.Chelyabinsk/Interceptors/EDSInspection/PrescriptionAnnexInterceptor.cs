namespace Bars.GkhGji.Regions.Chelyabinsk.Interceptors
{
    ...
    public class PrescriptionAnnexInterceptor : EmptyDomainInterceptor<PrescriptionAnnex>
    {
        public IDomainService<EDSInspection> EDSInspectionDomain { get; set; }

        public override IDataResult AfterUpdateAction(IDomainService<PrescriptionAnnex> service, PrescriptionAnnex entity)
        {
            ...
            // снимаем задачу
            {
                ...
                {
                    if (entity.Prescription.Inspection.Contragent != null)
                    {
                        if (entity.Prescription.Inspection.Contragent.IsEDSE)
                        {
                            var edsinsp = EDSInspectionDomain.GetAll()
                       .FirstOrDefault(x => x.InspectionGji == entity.Prescription.Inspection);
                            if (edsinsp == null)
                            {
                                EDSInspection newEDS = new EDSInspection
                                {
                                    Contragent = entity.Prescription.Inspection.Contragent,
                                    InspectionGji = entity.Prescription.Inspection,
                                    InspectionDate = entity.Prescription.Inspection.ObjectCreateDate,
                                    InspectionNumber = entity.Prescription.Inspection.InspectionNumber,
                                    NotOpened = true,
                                    ObjectCreateDate = DateTime.Now,
                                    ObjectEditDate = DateTime.Now,
                                    ObjectVersion = 1,
                                    TypeBase = entity.Prescription.Inspection.TypeBase
                                };
                            ...
                            string email = entity.Prescription.Inspection.Contragent.Email;
                ...
                logEntityHistoryService.UpdateLog(entity, TypeEntityLogging.DocumentGjiAnnex, entity.Id, entity.GetType(), GetPropertyValues(), entity.Prescription.DocumentNumber + " " + entity.Name);
            }
            catch
            {
        ...
        public override IDataResult AfterCreateAction(IDomainService<PrescriptionAnnex> service, PrescriptionAnnex entity)
        {
            var logEntityHistoryService = this.Container.Resolve<ILogEntityHistoryService>();
            try
            {
                logEntityHistoryService.UpdateLog(entity, TypeEntityLogging.DocumentGjiAnnex, entity.Id, entity.GetType(), GetPropertyValues(), entity.Prescription.Id.ToString() + " " + entity.Name);
            }
            catch
            {
        ...
        public override IDataResult AfterDeleteAction(IDomainService<PrescriptionAnnex> service, PrescriptionAnnex entity)
        {
            var logEntityHistoryService = this.Container.Resolve<ILogEntityHistoryService>();
            try
            {
                logEntityHistoryService.UpdateLog(null, TypeEntityLogging.DocumentGjiAnnex, entity.Id, entity.GetType(), GetPropertyValues(), entity.Prescription.DocumentNumber + " " + entity.Name);
            }
            catch
            {
        ...
        private Dictionary<string, string> GetPropertyValues()
        {
            var result = new Dictionary<string, string>
            {
                ...
                { "TypePrescriptionAnnex", "Номер документа" },
