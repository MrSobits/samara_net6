    ...
    class AppealCitsAdmonitionInterceptor : EmptyDomainInterceptor<AppealCitsAdmonition>
    {
        ...
        public override IDataResult AfterCreateAction(IDomainService<AppealCitsAdmonition> service, AppealCitsAdmonition entity)
        {
            ...
            try
            {
                ...
                long appCitsId = AppCitAdmonAppealDomain.GetAll().Where(x => x.AppealCitsAdmonition.Id == entity.Id).Select(x => x.AppealCits.Id).FirstOrDefault();

                logEntityHistoryService.UpdateLog(entity, TypeEntityLogging.AppealCitsAdmonition, entity.Id, entity.GetType(), GetPropertyValues(), appCitsId + " " + entity.DocumentNumber);
