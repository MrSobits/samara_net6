namespace Bars.GkhGji.Regions.Chelyabinsk.Report
{
    using System.Collections.Generic;
    using System.Linq;
    using B4.DataAccess;
    using B4.Modules.Reports;
    using B4.Utils;
    using Entities;
    using Bars.GkhGji.Entities;
    using Utils;
    using Gkh.Report;
    using System;
    using B4;

    using Bars.B4.Modules.Analytics.Reports.Enums;

    /// <summary>
    /// Извещение
    /// </summary>
    public class SMEVEGRIPReport : GkhBaseStimulReport
    {
        #region .ctor

        /// <summary>
        /// .ctor
        /// </summary>
        public SMEVEGRIPReport()
            : base(new ReportTemplateBinary(Properties.Resources.SMEVEGRIPReport))
        {
        }

        #endregion .ctor

        #region Private fields

        private long _SMEVEGRIPId;

        #endregion Private fields

        #region Protected properties

        /// <summary>
        /// Код шаблона (файла)
        /// </summary>
        protected override string CodeTemplate { get; set; }

        #endregion Protected properties

        #region Public properties

        /// <summary>
        /// Наименование отчета
        /// </summary>
        public override string Name
        {
            get { return "Выписка"; }
        }

        /// <summary>
        /// Описание отчета
        /// </summary>
        public override string Description
        {
            get { return "Выписка"; }
        }

        /// <summary>
        /// Идентификатор отчета
        /// </summary>
        public override string Id
        {
            get { return "SMEVEGRIP"; }
        }

        /// <summary>
        /// Код формы, на которой находится кнопка печати
        /// </summary>
        public override string CodeForm
        {
            get { return "SMEVEGRIP"; }
        }

        /// <summary>Формат печатной формы</summary>
        public override StiExportFormat ExportFormat => StiExportFormat.Word2007;
        #endregion Public properties

        #region Public methods

        /// <summary>
        /// Подготовить параметры отчета
        /// </summary>
        /// <param name="reportParams"></param>
        public override void PrepareReport(ReportParams reportParams)
        {
            var SMEVEGRIPDomain = Container.ResolveDomain<SMEVEGRIP>();

            var data = SMEVEGRIPDomain.GetAll()
                .Where(x => x.Id == _SMEVEGRIPId).FirstOrDefault();

            try
            {
                this.ReportParams["ДатаВыписки"] = data.ResponceDate.HasValue? data.ResponceDate.Value.ToShortDateString():"";
                this.ReportParams["НомерВыписки"] = data.MessageId;
                this.ReportParams["ФИО"] = data.FIO;
                this.ReportParams["ТипИП"] = data.IPType;
                this.ReportParams["ОГРН"] = data.OGRN;
                this.ReportParams["ДатаОГРН"] = data.OGRNDate.HasValue? data.OGRNDate.Value.ToShortDateString(): "";
                this.ReportParams["Гражданство"] = data.Citizenship;
                this.ReportParams["ВидРег"] = data.CreateWayName;
                this.ReportParams["КодОргана"] = data.CodeRegOrg;
                this.ReportParams["РегОрган"] = data.RegOrgName;
                this.ReportParams["АдресРегОрг"] = data.AddressRegOrg;
                this.ReportParams["Регион"] = data.RegionName;
                this.ReportParams["ТипРегион"] = data.RegionType;
                this.ReportParams["КодОквэд"] = data.OKVEDCodes;
                this.ReportParams["ОКВЭД"] = data.OKVEDNames;
            }
            finally
            {

            }

            
        }

        /// <summary>
        /// Установить пользовательские параметры
        /// </summary>
        /// <param name="userParamsValues"></param>
        public override void SetUserParams(UserParamsValues userParamsValues)
        {
            _SMEVEGRIPId = userParamsValues.GetValue<long>("Id");
        }

        /// <summary>
        /// Получить список шаблонов
        /// </summary>
        /// <returns></returns>
        public override List<TemplateInfo> GetTemplateInfo()
        {
            return new List<TemplateInfo>
            {
                new TemplateInfo
                {
                    Name = "Выписка",
                    Description = "Выписка ЕГРИП",
                    Code = "SMEVEGRIPReport",
                    Template = Properties.Resources.SMEVEGRIPReport
                }
            };
        }

        #endregion Public methods
    }
}