namespace Bars.GkhGji.Regions.Chelyabinsk.Services.Impl
{
    ...
    //[ServiceBehavior(Namespace = "http://xsd.smev.ru/ppu/GZHIgzhiPereofIP")]
    public class pmvGZHIgzhiPereofIP : IPereofIP
    {
        [XmlSerializerFormat]
        public createSmvGzhiPereofIPResponse1 createSmvGzhiPereofIP(createSmvGzhiPereofIPRequest1 request)
        {
            // smevHistory
            var service = new smevHistoryServiceV2();
            string smevHistoryInnerId = Guid.NewGuid().ToString();
            string smevHistoryActionCode = "gzhiPereofIP";
            ...
            var requestData = request.createSmvGzhiPereofIPRequest.smvGzhiPereofIP;

            //Prepare response
            var retVal = new createSmvGzhiPereofIPResponse1();
            var resp = new createSmvGzhiPereofIPResponse();
            resp.smvGzhiPereofIP = requestData;
            retVal.createSmvGzhiPereofIPResponse = resp;
            ...
            if (licReis.Contragent == null)
            {
                var newContragent = new Contragent
                {
                    Name = string.IsNullOrEmpty(requestData.G1LastName)
                        ? requestData.G1Familiy + " " + requestData.G1Name
                        : requestData.G1Familiy + " " + requestData.G1Name + " " + requestData.G1LastName,
            ...
            //    manOrgRequestProvDocDomain.Save(licReisDoc25);
            //}

            // Но есть 4-й, что это за файл - неизвестно
            if (requestData.gzhiDoc4Name != null && requestData.gzhiDoc4 != null)
            {
                var doc4 = fileMan.SaveFile(requestData.gzhiDoc4Name, requestData.gzhiDoc4);
                LicenseReissuanceProvDoc licReisDoc4 = new LicenseReissuanceProvDoc();
                licReisDoc4.File = doc4;
                // но предположим, что вместо 25-го, поэтому код 13
                LicenseProvidedDoc lcp = provDocDomain.GetAll()
                    .Where(x => x.Code == "13").FirstOrDefault();
                licReisDoc4.LicProvidedDoc = lcp;
                licReisDoc4.Date = DateTime.Now;
                licReisDoc4.ObjectCreateDate = DateTime.Now;
                licReisDoc4.ObjectEditDate = DateTime.Now;
                licReisDoc4.ObjectVersion = 333;
                licReisDoc4.LicenseReissuance = licReis;
                reissProvDocDomain.Save(licReisDoc4);
            }
        ...
        {
            var Container = ApplicationContext.Current.Container;
            var molRequestDomain = Container.ResolveDomain<LicenseReissuance>();
