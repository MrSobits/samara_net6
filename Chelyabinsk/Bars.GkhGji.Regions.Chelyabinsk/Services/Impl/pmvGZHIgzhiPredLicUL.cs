namespace Bars.GkhGji.Regions.Chelyabinsk.Services.Impl
{
    ...
    // TODO: wcf
   /* [AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequ*irementsMode.Allowed)]
    [ServiceBehavior(Namespace = "http://xsd.smev.ru/ppu/GZHIgzhiPredLicUL")]*/
    public class pmvGZHIgzhiPredLicUL : IPredLicUL
    {


        [XmlSerializerFormat]
        public createSmvGzhiPredLicULResponse1 createSmvGzhiPredLicUL(createSmvGzhiPredLicULRequest1 request)
        {
            ...
            string smevHistoryActionCode = "gzhiPredLicUL";
            ...
            var requestData = request.createSmvGzhiPredLicULRequest.smvGzhiPredLicUL;

            //Prepare response
            var retVal = new createSmvGzhiPredLicULResponse1();
            var resp = new createSmvGzhiPredLicULResponse();
            resp.smvGzhiPredLicUL = requestData;
            retVal.createSmvGzhiPredLicULResponse = resp;
        ...
        private void SaveFile(IFileManager fileMan, IDomainService reqDocDomain, string fileName, byte[] data, ManOrgLicenseRequest molRequest)
        {
            var file = fileMan.SaveFile(fileName, data);
            ManOrgRequestProvDoc molDoc = new ManOrgRequestProvDoc { File = file, LicRequest = molRequest };
            reqDocDomain.Save(molDoc);
        }
