namespace Bars.GkhGji.Regions.Chelyabinsk.Services.Impl
{
   ...
   /* [AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    [ServiceBehavior(Namespace = "http://xsd.smev.ru/ppu/GZHIgzhiDublikatUL")]*/
    public class pmvGZHIgzhiDublikatUL : IDublikatUL
    {
        [XmlSerializerFormat]
        public createSmvGzhiDublikatULResponse1 createSmvGzhiDublikatUL(createSmvGzhiDublikatULRequest1 request)
        {
            // smevHistory
            string smevHistoryInnerId = Guid.NewGuid().ToString();
            string smevHistoryActionCode = "gzhiDublikatUL";
            var service = new smevHistoryServiceV2();
            SmevHistory smevHistory = new SmevHistory
            {
                ActionCode = smevHistoryActionCode,
                LicenseRequestType = LicenseRequestType.IssuingDuplicateLicense,
                InnerId = smevHistoryInnerId
            };

            var requestData = request.createSmvGzhiDublikatULRequest.smvGzhiDublikatUL;

            //Prepare response
            var retVal = new createSmvGzhiDublikatULResponse1();
            var resp = new createSmvGzhiDublikatULResponse();
            resp.smvGzhiDublikatUL = requestData;
            retVal.createSmvGzhiDublikatULResponse = resp;
            ...
            {
                var stateProvider = Container.Resolve<B4.Modules.States.IStateProvider>();
                try
                {
