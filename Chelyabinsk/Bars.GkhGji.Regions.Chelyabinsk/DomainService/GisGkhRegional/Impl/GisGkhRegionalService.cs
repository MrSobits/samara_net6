namespace Bars.GkhGji.Regions.Chelyabinsk.DomainService.GisGkhRegional.Impl
{
    using Bars.B4;
    ...
    public class GisGkhRegionalService : Bars.GkhGji.DomainService.GisGkhRegional.Impl.GisGkhRegionalService
    {
        ...
        {
            return AppealCitsExecutantDomain.GetAll().Where(x => x.AppealCits == appeal).Where(x => x.Executant != null).Select(x => x.Executant).FirstOrDefault();
        }
        ...
        {
            if (appeal.ControlDateGisGkh.HasValue)
            {
                return appeal.ControlDateGisGkh.Value;
            }
            else
            {
        ...
        public override string GetFunctionRegistryNumber(bool licensecontrol)
        {
            if (licensecontrol)
            {
                return "7400000000163262794";
            }
            else
            {
                return "7400000000163062804";
            }
        }
        ...
        public override void FindActRemovalAnnexes(DocumentGji docChildAct, out List<long> ids, out List<FileInfo> files, out List<string> gisGkhGuids)
        {
            ...
            var actAnnexes = ActRemovalAnnexRepo.GetAll()
                 .Where(x => x.File.Extention.ToLower() == "pdf")
                            .Where(x => x.ActRemoval == docChildAct).ToList();
