namespace Bars.Gkh.RegOperator.Report
{
    ...
    public class RaionChargeReport : BasePrintForm
    {
        ...
        public RaionChargeReport()
            : base(new ReportTemplateBinary(Properties.Resources.RaionChargeReport))
        {
        }

        public override string Name
        {
            get { return "Отчет о начислениях в разрезе МР"; }
        }
        ...
        {
            get { return "B4.controller.report.RaionChargeReport"; }
        }

        public override string RequiredPermission
        {
            get { return "Reports.GkhRegOp.RaionChargeReport"; }
        }
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            List<MunicipalityChargeRecord> records = null;
            ...
            try
            {
                ...
                var mr = moDomain.GetAll().FirstOrDefault(x => x.Id == mrId);
                if (mr == null)
                {
                    throw new ReportProviderException("Не удалось определить муниципальный район");
                }
                ...
                var moName = склонятель.Проанализировать(mr.Name);

                reportParams.SimpleReportParams["Район"] = moName.Дательный;

                records = service.GetMunicipalityChargeRecords(mrId, moIds, periodId);
                ...
                foreach (var rec in records)
                {
                    ...
                    section["Номер"] = i;
                    section["МО"] = rec.MunicipalityName;
        ...
        public override void SetUserParams(BaseParams baseParams)
        {
            ...
            mrId = baseParams.Params.GetAs("mrId", 0L);

            var moIdsStr = baseParams.Params.GetAs("moIds", string.Empty);
            moIds = !string.IsNullOrEmpty(moIdsStr)
                                  ? moIdsStr.Split(',').Select(id => id.ToLong()).ToArray()
                                  : new long[0];
        }
    }
}