namespace Bars.Gkh.RegOperator.Tasks.PaymentDocuments.V2
{
    ...
    /// </summary>
    internal class LegalPartiallyOwnerDocumentExecutor : BaseDocumentExecutor
    {
        /// <summary>
        ...
        public LegalPartiallyOwnerDocumentExecutor(
            ...
            CancellationToken ct)
        {
            ...
            using (var docSource = new PaymentDocumentDataSource(this.container, periodEntity, primarySources, PaymentDocumentType.RegistryLegal, true))
            {
                docSource.CreatePartiallyRegistrySnapshots();

                var options = this.config.PaymentDocumentConfigContainer;
                var docData = docSource.DocumentData;

                var ownerData = docData.GetOwnerData().Where(x => !x.IsBase);
                ...
                {
                    DocumentsPath = $"{filesPath} (по частичному реестру) {DateTime.Now:dd.MM.yyyy HH-mm-ss}",
                ...
                using (var reportMan = new PaymentDocReportManager(this.templateCopyRepo))
                {
                    try
                    {
                        ICodedReport report = (ICodedReport)(new InvoiceRegistryAndActReport(args.Snapshots, args.AccountInfos));
