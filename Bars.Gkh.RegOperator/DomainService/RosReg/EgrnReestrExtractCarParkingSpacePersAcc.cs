namespace Bars.Gkh.RegOperator.DomainService.Impl
{
    ...
    public class EgrnReestrExtractCarParkingSpacePersAcc : ExtractTypeBase
    {
        /// <inheritdoc />
        ...
        public override void Parse(Extract extract)
        {
            ...
            try
            {
                ...
                newExtractEgrn.CadastralNumber = xRoot.SelectSingleNode("//*/car_parking_space_record/object/common_data/cad_number")?.InnerText;
                newExtractEgrn.Address = xRoot.SelectSingleNode("//*/address_room/address/address/readable_address")?.InnerText;
                newExtractEgrn.Type = "Парковочное место";
                newExtractEgrn.Purpose = "Нежилое помещение";
                newExtractEgrn.Area = xRoot.SelectSingleNode("//*/car_parking_space_record/params/area")?.InnerText.ToDecimal() ?? 0;
                ...
                for (var indexRight = 0; indexRight < rightNodes.Count; indexRight++)
                {
                    ...
                    for (var legalOwnerIndex = 0; legalOwnerIndex < legalNodes.Count; legalOwnerIndex++)
                    {
                        ...
                        if (isresident)
                        {
                            ...
                            resident.Name = legalNodes[legalOwnerIndex].SelectSingleNode("entity/resident/name")?.InnerText;
