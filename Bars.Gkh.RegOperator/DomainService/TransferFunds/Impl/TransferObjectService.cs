namespace Bars.Gkh.RegOperator.DomainService.Impl
{
    ...
    public class TransferObjectService : ITransferObjectService
    {
        ...
        /// <returns>Пустой результат расчёта либо ошибка, если ведётся расчёт</returns>
        ...
        private IDataResult CalcInternal(TransferRfRecord record)
        {
            ...
            var chargeAccountRoIds = Container.ResolveDomain<RealityObjectChargeAccountOperation>().GetAll()
                ...
                .Where(y => filterInlcuded.Any(x => x == y.Account.RealityObject.Id))
        ...
        private Dictionary<long, decimal> CalcChargeAccountPaids(ChargePeriod period, List<long> chargeAccountRoIds)
        {
            ...
            var persAccPerQuery = this.PersonalAccountPeriodSummaryDomain.GetAll()
                .Where(x => x.Period.Id == period.Id)
                .Where(x => x.PersonalAccount.Room.OwnershipType != RoomOwnershipType.Municipal)
                ...
                {
                    x.Key,
                    PaidSum = x.Sum(y => y.TariffPayment + y.PenaltyPayment),
