namespace Bars.Gkh.RegOperator.Distribution.Impl
{
    ...
    /// <summary>
    /// Распределение типа расход "Возврат Пени"
    /// </summary>
    public class RefundPenaltyDistribution : AbstractPersonalAccountDistribution
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        public RefundPenaltyDistribution(
        ...
        /// Код распределения
        /// </summary>
        public override DistributionCode DistributionCode => DistributionCode.RefundPenaltyDistribution;

        /// <summary>
        /// Идентификатор права доступа
        /// </summary>
        public override string PermissionId => "GkhRegOp.Accounts.SuspenseAccount.EnrollmentTypes.RefundPenaltyDistribution";
        ...
        public override IDataResult Apply(IDistributionArgs distrArgs)
        {
            ...
            var command = new PersonalAccountRefundCommand(RefundType.Penalty);
            ...
            {
                if (record.SumPenalty > 0)
                {
                    var localTransfers = record.PersonalAccount.ApplyRefund(command,
                        new MoneyStream(args.Distributable, operation, paymentDate, record.SumPenalty));
        ...
        public override IDataResult ListDistributionObjs(BaseParams baseParams)
        {
            ...
            switch (type)
            {
                case SuspenseAccountDistributionParametersView.ProportionalArea:
                    ...
                    proxies =
                        Utils.MoneyAndCentDistribution(accounts,
                            ...
                            distribSum,
                            (acc, sumPenalty) => new PersAccProxy(acc.Account, 0, sumPenalty) { RoPayAccountNum = acc.PaymentAccountNum },
                            (proxy, coin) =>
                            {
                ...
                case SuspenseAccountDistributionParametersView.ByDebt:
                    proxies = RefundPenaltyDistribution.GetProxiesByDebtByOutcomeMoneyDirection(accounts, distribSum);
                ...
                accounts.Select(x => new
                    {
                        ...
                        PenaltyDebtSum = x.Account.Summaries.SafeSum(y => y.GetPenaltyDebt()),
                        x.PaymentAccountNum
                    })
                    .ToList();

            var debtsNeedsToDistibute = debts.Where(x => x.PenaltyDebtSum < 0).ToList();

            //сумма будет отрицательной
            var debtsSum = debtsNeedsToDistibute.Sum(x => x.PenaltyDebtSum);
            var sum = 0;

            //так как задолженность у ЛС отрицательная у тех, кому необходим возврат средств, то суммируем по модулю
            if (debtsNeedsToDistibute.SafeSum(x => Math.Abs(x.PenaltyDebtSum)) >= distribSum)
            {
                proxies =
                    Utils.MoneyAndCentDistribution(debts,
                        x => x.PenaltyDebtSum >= 0
                            ? 0M
                            : (distribSum * x.PenaltyDebtSum / debtsSum).RoundDecimal(2),
                        distribSum,
                        (debt, sumPenalty) => new PersAccProxy(debt.PersonalAccount, sum, sumPenalty) { RoPayAccountNum = debt.PaymentAccountNum },
                        (proxy, coin) =>
                        {
                            if (proxy.Sum > 0)
                            {
            ...
            else
            {
                proxies =
                    Utils.MoneyAndCentDistribution(debts,
                        x => x.PenaltyDebtSum >= 0
                            ? 0M
                            : -x.PenaltyDebtSum.RoundDecimal(2),
                        Math.Abs(debtsSum),
                        (debt, sumPenalty) => new PersAccProxy(debt.PersonalAccount, sum, sumPenalty) { RoPayAccountNum = debt.PaymentAccountNum },
                        (proxy, coin) =>
                        {
