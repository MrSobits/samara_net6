...
namespace Bars.Gkh.RegOperator.ViewModel
{
    ...
    public class EntityLogLightViewModel : BaseViewModel<EntityLogLight>
    {
        public override IDataResult List(IDomainService<EntityLogLight> domainService, BaseParams baseParams)
        {
            ...
            //если запрос на историю помещения происходит из карточки абонента
            if (!string.IsNullOrEmpty(className) && className.Equals("BasePersonalAccount"))
            {
                var accountDomain = this.Container.Resolve<IDomainService<BasePersonalAccount>>();
                entityId = accountDomain.GetAll().Where(x => x.Id == entityId)
