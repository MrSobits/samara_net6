namespace Bars.Gkh.RegOperator.CodedReports
{
    ...
    ///  </summary>
    public class LawSuitOwnerApplicationIssuanceCourtOrderReport : ExportFormatHelper, IClaimWorkCodedReport
    {
        /// <summary>
        /// Container
        /// </summary>
        ...
        public string Id => nameof(LawSuitOwnerApplicationIssuanceCourtOrderReport);

        public override string Name => "Заявление на выдачу судебного приказа (совместная собственность)";

        public override string Description => this.Name;

        public string CodeForm => "LawSuitOwnerApplicationIssuanceCourtOrderReport";

        public string OutputFileName { get; set; } = "Заявление на выдачу судебного приказа (совместная собственность)";
        ...
        public override IEnumerable<IDataSource> GetDataSources()
        {
            ...
            {
                new CodedDataSource("Сведения о собственниках", new LawSuitOwnerForReportDataProvider(ApplicationContext.Current.Container)
                {
        ...
        public void Generate()
        {
            ...
            try
            {
                ...
                this.OutputFileName =
                    $"Заявление на выдачу судебного приказа (совместная собственность){(lawSuit.DocumentDate.HasValue ? $"({lawSuit.DocumentDate.Value.ToShortDateString()})" : string.Empty)}.{format}"
                        .Replace("\"", "");
