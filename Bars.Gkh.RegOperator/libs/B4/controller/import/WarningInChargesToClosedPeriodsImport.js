/**
* Контроллер журнала предупреждений при импорте начислений в закрытый период
*/
Ext.define('B4.controller.import.WarningInChargesToClosedPeriodsImport', {
    extend: 'B4.base.Controller',
    views: [
        'import.WarningInChargesToClosedPeriodsImportPanel',
        'import.PersonalAccountCompareInChargesGrid'
    ],
    mainView: 'import.WarningInChargesToClosedPeriodsImportPanel',
    mainViewSelector: 'warninginchargestoclosedperiodsimportpanel',
    ...
    init: function () {
        var me = this;
        me.control({
            'warninginchargestoclosedperiodsimportpanel': {
            ...
            'warninginchargestoclosedperiodsimportpanel button[action=ConfirmAutoCompare]': {
                'click': { fn: me.onClickConfirmAutoCompare }
            },
            'warninginchargestoclosedperiodsimportpanel button[action=ManualCompare]': {
                'click': { fn: me.onClickManualCompare }
            },
            // Диалогове окно ручного сопоставления
            'compareinchargesgrid button[action=Accept]': {
                'click': { fn: me.onClickAcceptComparing },
            },
            'compareinchargesgrid gridpanel': {
    ...
    onMainViewRender: function (grid) {
        ...
        grid.down('#existWarningsGrid').getStore().on('beforeLoad', me.onStoreBeforeLoad, me);
    ...
    onCompareGridSelect: function (rowModel, record, index, eOpts) {
        ...
        if (grid.getSelectionModel().getSelection().length !== 1) { // Должна быть выбрана ровно 1 запись
    ...
    showManualCompareDialog: function (rec) {
        ...
        me.paGrid = me.getView('B4.view.import.PersonalAccountCompareInChargesGrid').create(
            {
                constrain: true,
