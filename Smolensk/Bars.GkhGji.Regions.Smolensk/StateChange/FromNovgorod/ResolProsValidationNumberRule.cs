...
namespace Bars.GkhGji.Regions.Smolensk.StateChange
{
    public class ResolProsValidationNumberRule : BaseDocValidationNumberRule
    {
        ...
        public override string Name { get { return "Перенесено из ННовгорода - Проверка возможности формирования номера постановления прокуратуры"; } }

        public override string TypeId { get { return "gji_document_resolpros"; } }

        public override string Description { get { return "Перенесено из ННовгорода - Данное правило проверяет формирование номера постановления прокуратуры в соответствии с правилами"; } }
        ...
        protected override void Action(DocumentGji document)
        {
            ...
            try
            {
                ...
                {
                    var ordinalNumber =
                        docNumbers.Select(x => x.Substring(0, x.IndexOf("/", StringComparison.InvariantCulture)- 2).ToInt()).ToList();
