    ...
    public class DocumentViolGroupViewModel : BaseViewModel<DocumentViolGroup>
    {
        public override IDataResult List(IDomainService<DocumentViolGroup> domainService, BaseParams baseParams)
        {
            ...
            try
            {
                ...
                var withRo = baseParams.Params.GetAs<bool>("withRo");

                var notDateFactRemoval = baseParams.Params.GetAs<bool>("notDateFactRemoval");

                var query = domainService.GetAll()
                    .WhereIf(withRo && roId > 0, x => x.RealityObject.Id == roId) // если четко просят по дому то даем только по дому
                    .WhereIf(withRo && roId == 0, x => x.RealityObject == null) // если надо по записи у которой нет дома то делаем без дома
                    .WhereIf(notDateFactRemoval, x => !x.DateFactRemoval.HasValue)
                    .WhereIf(documentId > 0, x => x.Document.Id == documentId); // по документу 
                                   ...
                                   .ToDictionary(
                                        ...
                                        y => new
                                        {
                                            PointCodes = y.Select(z => z.CodePin)
                                                        //.Distinct() тут нивкоем случае ненужно обрубат ьпо наименвоаниям потмоуч томогут быть одинаковые названия пунктов 
                ...
                var violPoints =
                    violPiontDomain.GetAll()
                                   ...
                                   .ToDictionary(
                                        ...
                                        {
                                            PointCodes = y.Select(z => z.CodePin)
                                                        // .Distinct() тут ни в коем случае не нужно обрубат ьпо дистинкту, так как могут быт ьодинаковые названия пунктов  
                                                        .Aggregate((str, result) => !string.IsNullOrEmpty(result) ? result + "," + str : str),
