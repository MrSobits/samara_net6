Ext.define('B4.controller.ActCheck', {
    ...
    stores: [
        ...
        'actcheck.Definition',
        'actcheck.InspectedPart',
        'actcheck.RealityObjectByInspection',
        'actcheck.RealityObject',
    ...
    refs: [
        {
            ref: 'roWindow',
            selector: '#actCheckRealityObjectEditWindow'
        }
    ],
    ...
    aspects: [
        ...
        {   /* 
            ...
            */
            ...
            otherActions: function (actions) {
                ...
                actions[this.editPanelSelector + ' [name=HaveViolation]'] = { 'change': { fn: this.changeHaveViolation, scope: this} };
            },
            
            changeHaveViolation: function (combobox, newValue) {
                var me = this,
                    panel = me.getPanel(),
                    roGridAddButton = panel.down('actCheckRealityObjectGrid b4addbutton[actionName=add]');

                roGridAddButton.setDisabled(false);
                
                // Нельзя добавлять если не 'да' блокируем кнопку добавления
                if (newValue != 10) {
                    roGridAddButton.setDisabled(true);
                }
            },
            ...
            onAfterSetPanelData: function (asp, rec, panel) {
                ...
                asp.controller.params.InspectionContragentMoId = rec.get('InspectionContragentMoId');
                ...
                // Вообщем в Смоленске решили что на вкладке Нарушений всегда будет толкь оодин вид, но если
                // решили поменять то закомментированный код раскоментируйте и будет взависимости от того на 1 дом или на несколько заменятся компоненты
                ...
                if (rec.get('ActCheckGjiRealityObject')) {
                    ...
                    if (rec.get('ActCheckGjiRealityObject').HaveViolation == 10) {
            ...
            */
            saveRealityObjectViolation: function (actObjectId, haveViolation, description, violationStore, deferred, notRevViol, roId) {
                var me = this;
                
                // Блокируем сохранение если не выполняется ряд условий
                if (violationStore.getCount() == 0 && haveViolation == 10) {
                    deferred.fail({ message: 'Не обходимо в таблице нарушений добавить записи нарушений' });
                    return false;
                }
                ...
                B4.Ajax.request({
                    ...
                    params: {
                        roId: roId,
                        actId: me.controller.params.documentId,
            ...
            */
            ...
            listeners: {
                aftersetformdata: function (asp, record, form) {
                    var me = this,
                        violGroupGridAddButton = form.down('actCheckViolationGroupGrid b4addbutton');
                    
                    if (record.getId() > 0) {
                        violGroupGridAddButton.setDisabled(false);
                    } else {
                        violGroupGridAddButton.setDisabled(true);
                    }
                        
                    me.controller.getStore('actcheck.Violation').load();
                    me.controller.getStore('actcheck.ViolationGroup').load();
                }
            },

            otherActions: function (actions) {
                actions[this.editFormSelector + ' [name=IsRealityObject]'] = { 'change': { fn: this.changeCheckBox, scope: this } };
                actions[this.editFormSelector + ' b4selectfield[name=RealityObject]'] = { 'beforeload': { fn: this.onRoStoreBeforeLoad, scope: this } };
            },

            onRoStoreBeforeLoad: function (field, options) {
                var me = this;
                
                options.params.actCheckId = me.controller.params.documentId;
            },

            changeCheckBox: function (cmp, newValue) {
                var wnd = cmp.up('#actCheckRealityObjectEditWindow'),
                    roField = wnd.down('[name=RealityObject]');

                // скрываем адрес если не true
                if (!newValue) {
                    roField.setDisabled(true);
                    roField.setValue(null);
                    roField.allowBlank = true ;
                } else {
                    roField.setDisabled(false);
                    roField.allowBlank = false;
                }
            ...
            saveRecord: function (rec) {
                var me = this,
                    ...
                    cbHaveViolation = 10, // всегда нарушения выявлены
                    // description = editWindow.down('#taDescription').getValue(), поле убрали из Смоленска
                    description= '',
                    roId = editWindow.down('[name=RealityObject]').getValue(),
                    notRevViol = "", // Нет в смоленске такого поля
                ...
                me.controller.getAspect('actCheckEditPanelAspect')
                    .saveRealityObjectViolation(rec.getId(), cbHaveViolation, description, storeViolation, deferred, notRevViol, roId);
            },
        ...
        /* Поскольку в смоленске убрали поле Описание из карточки Редактирвоаняи Дома (там где добавляется нарушение)
        и создали групп нарушений в которых пишутся описания, то этот закоменченый код уже ненужен, если вернут просто расоментируйте
        {
            ...
            */
            ...
            saveRecord: function (rec) {
                var me = this,
                    ...
                    roRec = this.controller.getRoWindow().getRecord();
                ...
                B4.Ajax.request({
                    ...
                    params: {
                        ...
                        roId: roRec.get('RealityObject') || 0,
                        description: description,
                        pointIds: points,
                        groupId: rec.get('Id')
                    }
        ...
        },
        {
            ...
            */
            ...
            columnsGridSelect: [
                { header: 'Пункт нормативного документа', xtype: 'gridcolumn', dataIndex: 'ViolationGjiPin', flex: 1, filter: { xtype: 'textfield' } },
                { header: 'Текст нарушения', xtype: 'gridcolumn', dataIndex: 'ViolationGjiName', flex: 1, filter: { xtype: 'textfield' } }
            ],
            columnsGridSelected: [
                { header: 'Пункт нормативного документа', xtype: 'gridcolumn', dataIndex: 'ViolationGjiPin', flex: 1, sortable: false }
            ],
            ...
            onBeforeLoad: function (store, operation) {
                var rec = this.controller.getRoWindow().getRecord();

                operation.params.objectId = (rec && rec.getId() ? rec.getId() : 0);
            },
            listeners: {
                getdata: function () {
    ...
    onLaunch: function () {
        if (this.params) {
            ...
            this.getStore('actcheck.RealityObject').load();
        }
    },
    ...
    onViolationBeforeLoad: function (store, operation) {
        var rec = this.getRoWindow().getRecord();
    ...
    onViolationGroupBeforeLoad: function (store, operation) {
        ...
        operation.params.roId = rec.get('RealityObject') || 0;
    }
});