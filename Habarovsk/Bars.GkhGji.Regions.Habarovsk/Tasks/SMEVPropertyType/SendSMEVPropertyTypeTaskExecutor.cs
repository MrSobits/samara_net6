...
namespace Bars.GkhGji.Regions.Habarovsk.Tasks.SendSMEVPropertyType
{
    public class SendSMEVPropertyTypeTaskExecutor : ITaskExecutor
    {
        private ISMEVPropertyTypeService _ISMEVPropertyTypeService;
        ...
        public SendSMEVPropertyTypeTaskExecutor(ISMEVPropertyTypeService SMEVPropertyTypeService)
        {
            _ISMEVPropertyTypeService = SMEVPropertyTypeService;
        }
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            SMEVPropertyType request = SMEVPropertyTypeDomain.Get(taskId);
            if (request == null)
            ...
            try
            {
                var result = _ISMEVPropertyTypeService.SendInformationRequest(request, indicator);
                if (!result)
