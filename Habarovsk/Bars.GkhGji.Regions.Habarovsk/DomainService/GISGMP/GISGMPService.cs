...
using Bars.GkhGji.Regions.Habarovsk.DomainService.GISGMP;
    ...
    public class GISGMPService : IGISGMPService
    {
        ...
        public bool SendReconcileRequest(GisGmp requestData, IProgressIndicator indicator = null)
        {
            try
            {
                ...
                indicator?.Report(null, 20, "Отправка запроса");
                var requestResult = _SMEV3Service.SendRequestAsync(request, null, true).GetAwaiter().GetResult();
                requestData.BillFor = "Запрос квитирования";
                requestData.MessageId = requestResult.MessageId;
                GisGmpDomain.Update(requestData);
        ...
        //        indicator?.Report(null, 20, "Отправка запроса");
        //        var requestResult = _SMEV3Service.SendRequestAsync(request, null, true).GetAwaiter().GetResult();
        //        requestData.GisGmp.BillFor = "Запрос квитирования";
        ...
        //            SetErrorState(requestData.GisGmp, "Ошибка при обработке сообщения в СМЭВ3, подробности в файле Fault.xml");
        //        }
        //        else if (requestResult.Status != "requestIsQueued")
        //        {
        ...
        private XElement GetReconcileRequestXML(GisGmp requestData)
        {
            ...
            var result = ToXElement<ForcedAckmowledgmentProxy.ForcedAcknowledgementRequest>(res);

            List<PayReg> paymentList = PayRegDomain.GetAll().Where(x => x.GisGmp == requestData).ToList();
            foreach (PayReg payment in paymentList)
            {
                // проверяем, на сквитирована ли уже оплата
                if (payment.Reconcile != Gkh.Enums.YesNoNotSet.Yes)
                {
                    result.Element(faNamespace + "Reconcile").Add(
                    new XElement(faNamespace + "PaymentId", payment.PaymentId));
                }

            }

            result.SetAttributeValue(XNamespace.Xmlns + "com", comNamespace);
            result.SetAttributeValue(XNamespace.Xmlns + "fa", faNamespace);
            ...
            return new ImportChargesProxy.Payee
            {
                ...
                OrgAccount = new ImportChargesProxy.OrgAccount
                {
                    ...
                    Bank = new ImportChargesProxy.BankType
                    {
                        name = Payee.BankName,
                        bik = Payee.BIK,
                        correspondentBankAccount = Payee.OrgAccountNumber
                    }
                }
            };
        }
