...
namespace Bars.GkhGji.Regions.Habarovsk.DomainService
{
    ...
    using Bars.GkhGji.Regions.BaseChelyabinsk.Entities.MVDRegPlaceProxy;
    using Bars.GkhGji.Regions.BaseChelyabinsk.Enums;
    using Fasterflect;

    public class MVDStayingPlaceRegistrationService : IMVDStayingPlaceRegistrationService
    {
        ...
        #region Constructors
        public MVDStayingPlaceRegistrationService(IFileManager fileManager, ISMEV3Service SMEV3Service)
        {
        ...
        /// <returns></returns>
        public bool SendInformationRequest(MVDStayingPlaceRegistration requestData, IProgressIndicator indicator = null)
        {
            try
            {
                //Очищаем список файлов
                indicator?.Report(null, 0, "Очистка старых файлов");
                MVDStayingPlaceRegistrationFileDomain.GetAll().Where(x => x.MVDStayingPlaceRegistration == requestData).ToList().ForEach(x => MVDStayingPlaceRegistrationFileDomain.Delete(x.Id));
                ...
                MVDStayingPlaceRegistrationDomain.Update(requestData);
        ...
        /// <param name="indicator"></param>
        /// <returns></returns>
        public bool TryProcessResponse(MVDStayingPlaceRegistration requestData, GetResponseResponse response, IProgressIndicator indicator = null)
        {
            try
            {
                //
                ...
                else
                {
                    ...
                    stayingPlaceRegistrationResponse newResponce = Deserialize<stayingPlaceRegistrationResponse>(XElement.Parse(response.MessagePrimaryContent.FirstNode.ToString()));
        ...
        private void ChangeState(MVDStayingPlaceRegistration requestData, RequestState state)
        {
            requestData.RequestState = state;
            MVDStayingPlaceRegistrationDomain.Update(requestData);
        }

        private void SetErrorState(MVDStayingPlaceRegistration requestData, string error)
        {
        ...
        private void SaveFile(MVDStayingPlaceRegistration request, Stream data, string fileName)
        {
            //сохраняем ошибку
            MVDStayingPlaceRegistrationFileDomain.Save(new MVDStayingPlaceRegistrationFile
            {
                ObjectCreateDate = DateTime.Now,
                ObjectEditDate = DateTime.Now,
                ObjectVersion = 1,
                MVDStayingPlaceRegistration = request,
                SMEVFileType = SMEVFileType.Request,
                FileInfo = _fileManager.SaveFile(data, fileName)
            });
        }

        private void SaveFile(MVDStayingPlaceRegistration request, byte[] data, string fileName)
        {
        ...
        private void SaveFile(MVDStayingPlaceRegistration request, XElement data, string fileName)
        {
        ...
        private void SaveException(MVDStayingPlaceRegistration request, Exception exception)
        {
            if (exception == null)
                return;
        ...
        private XElement GetPassportRequest(MVDStayingPlaceRegistration req)
        {
