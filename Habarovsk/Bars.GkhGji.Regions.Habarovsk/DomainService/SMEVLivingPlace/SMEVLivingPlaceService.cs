...
using Bars.GkhGji.Regions.Habarovsk.SMEV2.Proxy;
using Bars.GkhGji.Regions.Habarovsk.SMEV2.Proxy.BookRequest;
using Bars.GkhGji.Regions.Habarovsk.SMEV2.Proxy.GetResult;
...
{
    public class SMEVLivingPlaceService : ISMEVLivingPlaceService
    {
        #region Constants
        ...
        public SMEVLivingPlaceService(IFileManager fileManager, IWindsorContainer container)
        {
        ...
        /// <returns></returns>
        public bool SendInformationRequest(SMEVLivingPlace requestData, IProgressIndicator indicator = null)
        {
            try
            {
                ...
                SMEVLivingPlaceFileDomain.GetAll().Where(x => x.SMEVLivingPlace == requestData).ToList().ForEach(x => SMEVLivingPlaceFileDomain.Delete(x.Id));
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(taskId))
                    {
                        ...
                        SMEVLivingPlaceDomain.Update(requestData);
                    }
                    else
                    {
        ...
        static XmlElement SignXmlNode(XmlDocument Document, string SignedElement, AsymmetricAlgorithm Key, X509Certificate Certificate)
        {
          ...
          //  reference.DigestMethod = "http://www.w3.org/2001/04/xmldsig-more#gostr3411";
           // reference.DigestMethod =  CryptoPro.Sharpei.Xml.CPSignedXml.XmlDsigGost3411UrlObsolete;
            ...
            XmlDsigExcC14NTransform c14 = new XmlDsigExcC14NTransform();
            c14.InclusiveNamespacesPrefixList = "";
            reference.AddTransform(c14);
            ...
            keyInfo.Id = "KeyId-1E42AC2E0B920AAF70131180067340426";
        ...
        public bool GetResult(SMEVLivingPlace requestData, IProgressIndicator indicator = null)
        {
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(Properties.Resources.PropTypeXml);
            ...
            try
            {
                ...
                if (result.Count > 0)
                { 
                    requestData.Answer = result.ContainsKey("COMMENT") ? result["COMMENT"] : getAnswer(result["REG_TYPE"]);
        ...
        private getResult GetGetResult(SMEVLivingPlace requestData)
        {
            getResult getResult = new getResult
            {
                Message = new SMEV2.Proxy.GetResult.Message
                {
                    ...
                    CaseNumber = $"R001_{requestData.Id * 15}",
                },
        ...
        private bookRequest GetBookRequest(SMEVLivingPlace requestData)
        {
        ...
        private void SaveFile(SMEVLivingPlace request, byte[] data, string fileName)
        {
            //сохраняем отправленный пакет
            SMEVLivingPlaceFileDomain.Save(new SMEVLivingPlaceFile
            {
                ObjectCreateDate = DateTime.Now,
                ObjectEditDate = DateTime.Now,
                ObjectVersion = 1,
                SMEVLivingPlace = request,
                SMEVFileType = SMEVFileType.Request,
                FileInfo = _fileManager.SaveFile(fileName, data)
            });
        }

        private void SaveRequestFile(XmlDocument doc, string fileName, SMEVLivingPlace rt)
        {
            //сохраняем ошибку
