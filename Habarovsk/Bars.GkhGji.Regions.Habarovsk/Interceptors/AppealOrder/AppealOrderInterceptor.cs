    ...
    class AppealOrderInterceptor : EmptyDomainInterceptor<AppealOrder>
    {
        ...
        public override IDataResult BeforeUpdateAction(IDomainService<AppealOrder> service, AppealOrder entity)
        {
            try
            {
                ...
                {
                    var proofs = AppealOrderFileDomain.GetAll().Where(x=> x.FileInfo != null).OrderBy(x=> x.ObjectCreateDate).FirstOrDefault(x => x.AppealOrder == entity);
                    //отключит по просьбе Ковешниковой
                    if (proofs != null && !entity.PerformanceDate.HasValue)
                    {
                        entity.PerformanceDate = proofs.ObjectCreateDate;
                    }
                    //else
                    //{
