...
Ext.define('B4.controller.AppealCits', {
    ...
    requires: [
        ...
        'B4.aspects.GkhGjiSignatureAndSendToEdmGridAspect'
    ],

    appealCitizensId: null,
    appealCitizensGetBackId: null,
    ...
    views: [
        ...
        'appealcits.TypeOfFeedbackGrid',
        'appealcits.AppealCitsTypeOfFeedbackEditWindow',
        'appealcits.AppealAnswerExecutionTypeGrid',
        'appealcits.AppealCitsHistoryGrid'
    ],
    ...
    aspects: [

        {
            ...
            name: 'appealCitiTypeOfFeedbackWindowAspect',
            gridSelector: 'appealcitsTypeOfFeedbackGrid',
            editFormSelector: '#appealcitsTypeOfFeedbackEditWindow',
            storeName: 'appealcits.TypeOfFeedback',
            modelName: 'appealcits.TypeOfFeedback',
            editWindowView: 'appealcits.AppealCitsTypeOfFeedbackEditWindow',
        ...
        },
        {
            ...
            otherActions: function (actions) {
                ...
                actions['#appealcitsFilterPanel #sfResponcible'] = { 'change': { fn: this.onChangeResponcible, scope: this } };
                actions['#appealcitsFilterPanel #sfController'] = { 'change': { fn: this.onChangeController, scope: this } };
                actions['#appealcitsFilterPanel #sfSigner'] = { 'change': { fn: this.onChangeSigner, scope: this } };
                ...
                actions[this.editFormSelector + ' #btnBack'] = { 'click': { fn: this.getBack, scope: this } };
            ...
            getBack: function (btn) {
                var me = this,
                    portal = me.controller.getController('PortalController'),
                    controllerEditName = 'B4.controller.AppealCits',
                    params = {};

                params.appealId = me.controller.appealCitizensGetBackId;
                params.appealIdGetBack = null;
                me.controller.appealCitizensGetBackId = null;
            ...
            onAfterSetFormData: function (aspect, rec, form) {
                ...
                if (sflZonalInspection.value == null) {
                    var newZonal = {
                        //Воронеж
                        Id: 49,
                        ZoneName: 'Государственная жилищная инспекция Воронежской области'
                        //Хабаровск
                        //Id: 145,
                        //ZoneName: 'Главное управление регионального государственного контроля и лицензирования Правительства Хабаровского края'
                    };
            ...
            listeners: {
                aftersetformdata: function (asp, record, form) {
                    ...
                    if (state && (state.Code == 'СОПР' || state.Code == 'СОПР2')) {
                        ...
                        //    var weekDay = caseDateDate.getDay();
                        //    var addDay = (weekDay === 0 || weekDay === 1 || weekDay === 2)
                        //        ? 4
                        //        : weekDay === 6
                        //            ? 5
                        //            : 6;
                        //    var expiredDate = Ext.Date.add(caseDateDate > dateFromDate ? caseDateDate : dateFromDate, Ext.Date.DAY, addDay);
                        //    dfSoprDate.setValue(expiredDate);
                        dfSoprDate.show();
                    ...
                    var btnBack = form.down('#btnBack');
                    if (asp.controller.appealCitizensGetBackId) {
            ...
            onChangeResponcible: function (field, newValue) {
                if (newValue) {
                    var ids = [];
                    Ext.Array.each(newValue, function (elem) { ids.push(elem.Id); });
                    this.controller.params.responcibleId = ids;
                } else {
                    this.controller.params.responcibleId = null;
                }
            },
            ...
            onChangeSigner: function (field, newValue) {
                if (newValue) {
                    var ids = [];
                    Ext.Array.each(newValue, function (elem) { ids.push(elem.Id); });
                    this.controller.params.signerId = ids;
                } else {
                    this.controller.params.signerId = null;
                }
            },
        ...
        },
        {
            ...
            */
            ...
            name: 'appealcitsanswerexecutiontypegridAspect',
            multiSelectWindow: 'SelectWindow.MultiSelectWindow',
            multiSelectWindowSelector: '#appealcitsanswerexecutiontypeSelectWindow',
            storeName: 'appealcits.AppealAnswerExecutionType',
            modelName: 'appealcits.AppealAnswerExecutionType',
            storeSelect: 'dict.AppealExecutionType',
            storeSelected: 'dict.AppealExecutionType',
            gridSelector: 'appealanswerxecutiontypegrid',
            columnsGridSelect: [
                {
                    xtype: 'gridcolumn',
                    dataIndex: 'Name',
            ...
            listeners: {
                getdata: function (asp, records) {
                    ...
                    if (recordIds[0] > 0) {
                        ...
                        B4.Ajax.request(B4.Url.action('AddAnswerExecutionType', 'AppealCitsExecutionType', {
                            recordIds: Ext.encode(recordIds),
                            appealCitsAnswerId: asp.controller.appealCitsAnswerId
                        })).next(function (response) {
        ...
        {
            xtype: 'gkhinlinegridaspect',
            name: 'appealCitsHistoryGridInlineAspect',
            storeName: 'appealcits.AppealCitsHistory',
            modelName: 'appealcits.AppealCitsHistory',
            gridSelector: '#appealCitsHistoryGrid',
        },
        {
            /*
        ...
        },
        {
            ...
            listeners: {
                ...
                aftersetformdata: function (asp, record, form) {
                    ...
                    //грид мероприятий
                    var grid = form.down('appealanswerxecutiontypegrid'),
                        store = grid.getStore();
                    if (!record.getId()) {
                        ...
                        store.on('beforeload',
                            function (store, operation) {
                                operation.params.appealCitAnswerId = record.getId();
                            },
                            me);
                        store.load();
                    }
            ...
            */
            ...
            listeners: {
                ...
                getdata: function (asp, records) {
                    ...
                    var sfResolveGji = Ext.ComponentQuery.query(asp.multiSelectWindowSelector + ' #sfResolveGji')[0];
                    if (!sfResolveGji.allowBlank && !sfResolveGji.value) {
                ...
                panelrendered: function (asp, prm) {
                    ...
                    if (autoPerformanceDate) {
                        var appctstore = me.controller.getStore('AppealCits');
                        var appctrec = appctstore.getById(me.controller.appealCitizensId);
                        if (appctrec) {
                            ...
                            var cdval = me.controller.controlDateValue;
                            var performanceDateEl = prm.window.down('#dfPerformanceDate');
                            performanceDateEl.setValue(cdval);
                        }
        ...
        {
            xtype: 'gkhgjisignatureandsendtoedmgridaspect',
            gridSelector: '#appealCitsAnswerGrid',
            controllerName: 'AppealCitsAnswer',
            name: 'appCitsSignAndSendAspect',
            signedFileField: 'SignedFile'
        },
        {
            xtype: 'grideditwindowaspect',
    ...
    setCurrentId: function (id, numberGji, number) {
        ...
        var editWindow = Ext.ComponentQuery.query(this.editWindowSelector)[0],
            ...
            storeTypeOfFeedback = this.getStore('appealcits.TypeOfFeedback'),
        ...
        storeAppCitsBaseStatement = this.getStore('appealcits.AppealCitsBaseStatement'),
            ...
            storeAppealCitsHistory = this.getStore('appealcits.AppealCitsHistory'),
        ...
        tabpanel.down('#tabTypeOfFeedbackGrid').tab.setDisabled(!id);
        tabpanel.down('#baseStatementAppCitsGrid').tab.setDisabled(!id);
        tabpanel.down('#tabHistoryGrid').tab.setDisabled(!id);
    ...
    init: function () {
        ...
        this.getStore('appealcits.TypeOfFeedback').on('beforeload', this.onBeforeLoad, this);
        ...
        this.getStore('appealcits.AppealCitsHistory').on('beforeload', this.onBeforeLoad, this);

        var actions = {};

        //  actions['#gridRelatedAppealCits'] = { 'rowaction': { fn: this.rowActionRelApp, scope: this } };
        actions['relatedAppealCitsGrid b4editcolumn'] = { click: { fn: this.rowActionRelApp, scope: this } };
        actions['relatedAppealCitsGrid #btnclear'] = { click: { fn: this.deleteAllRelated, scope: this } };
        actions['#gridAnotherRelatedAppealCits'] = { 'rowaction': { fn: this.rowActionAnotherRelApp, scope: this } };
        actions['appealcitshistorygrid actioncolumn[action="GetJsonString"]'] = { 'click': { fn: this.showJson, scope: this } };
    ...
    showJson: function (grid, rowIndex, colIndex, param, param2, rec, asp) {
        var currentDescriptonText = rec.data.JsonString ?? '';
        win = Ext.create('B4.view.appealcits.DetailsEditWindow');
        var valuefield = win.down('#dfDescription');
        var closebutton = win.down('b4closebutton');
        var savebutton = win.down('b4savebutton');
    ...
    rowActionRelApp: function (a, b, t, y, r, rec) {
        var me = this,
            ...
            controllerEditName = 'B4.controller.AppealCits',
            params = {};
        me.appealCitizensGetBackId = null;
        params.appealId = rec.get('Id');
        params.appealIdGetBack = me.appealCitizensId;
        portal.loadController(controllerEditName, params);
    },
    ...
    */
    ...
    onLaunch: function () {
        ...
        if (this.params && this.params.appealId > 0) {
            ...
            var model = this.getModel('AppealCits');
            var aspect = this.getAspect('appealCitizensWindowAspect')//this.getAspect('appealCitizensWindowAspect').editRecord(new model({ Id: this.params.appealId }));
            aspect.editRecord(new model({ Id: this.params.appealId }));
    ...
    onBeforeLoadAppealCits: function (store, operation) {
        if (this.params) {
            ...
            operation.params.responcibleId = this.params.responcibleId;
            operation.params.controllerId = this.params.controllerId;
            operation.params.signerId = this.params.signerId;
