namespace Bars.GkhGji.Regions.Habarovsk.Controllers
{
    ...
    public class ERKNMExecuteController : BaseController
    {
        ...
        public ActionResult SendERKNMRequest(BaseParams baseParams)
        {
            ...
            if (docType == ERKNMDocumentType.PreventiveAct)
            {
                return SendPreventiveVisit(baseParams, docId);
            }
        ...
        private ActionResult SendPreventiveVisit(BaseParams baseParams, long docId)
        {
            var pv = PreventiveVisitDomain.Get(docId);
            var existsRequest = ERKNMDomain.GetAll().FirstOrDefault(x => x.Disposal != null && x.Disposal.Id == docId && x.RequestState != RequestState.Error);
            if (existsRequest != null)
            {
                return JsFailure("По данному профвизиту запрос уже направлен.");
            }
            baseParams.Params.Add("protocolData", docId);
