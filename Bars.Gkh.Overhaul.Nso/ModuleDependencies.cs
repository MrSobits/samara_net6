...
namespace Bars.Gkh.Overhaul.Nso
{
    ...
    public class ModuleDependencies : BaseModuleDependencies
    {
        ...
        public override IModuleDependencies Init()
        {
            ...
            References.Add(new EntityReference
            {
                ReferenceName = "Объект долгосрочной программы КР",
                BaseEntity = typeof(RealityObject),
                CheckAnyDependences = id => Container.Resolve<IDomainService<LongTermPrObject>>().GetAll().Any(x => x.RealityObject.Id == id)
            });
