...
Ext.define('B4.controller.AppealCits', {
    ...
    views: [
        ...
        'appealcits.AnswerAddresseeGrid',
    ...
    baseStatementTypeForm: '#baseStatementAppCitsAddWindow #cbFormCheck',
    ...
    aspects: [
        ...
        },
        {
            ...
            buttonSelector: '#appealCitsAnswerGrid #btnPrint',
            codeForm: 'AppealCitsAnswer',
            getUserParams: function () {
                var me = this,
                    param = { DocumentId: me.controller.appealCitizensId };
        ...
        },
        {
            ...
            otherActions: function (actions) {
                ...
                actions['#appealcitsFilterPanel #sfController'] = { 'change': { fn: me.onChangeController, scope: me } };
                ...
                actions[me.editFormSelector + ' #cbTypeCorrespondent'] = { 'change': { fn: me.onChangeTypeCorrespondent, scope: me } };
            },
            ...
            onRedtapeFlagChange: function(field, newValue, oldValue) {
                var wnd = Ext.ComponentQuery.query(this.controller.editWindowSelector)[0],
                    previousAppealCitizensGjiSelectField = wnd.down('#previousAppealCitsSelectField');
                //TODO! Переделать
                previousAppealCitizensGjiSelectField.setDisabled(newValue > 2 ? false : true);
            },
            listeners: {
                getdata: function (asp, record) {
                    var form = Ext.ComponentQuery.query(asp.editFormSelector)[0],
                        typCorr = form.down('#cbTypeCorrespondent').getValue();
                    if (typCorr != 10 && typCorr != 20 && typCorr != 30) {
                        record.set('SocialStatus',null);
                    }
                },
                aftersetformdata: function(asp, record, form) {
                    var me = this,
                        ...
                        zji = record.get('ZonalInspection'),
                        win = asp.getForm(),
                        typCorr;
                    ...
                    typCorr = form.down('#cbTypeCorrespondent').getValue();
                    if (typCorr != 10 && typCorr != 20 && typCorr != 30) {
                        form.down('#sfSocialStatus').setValue(null);
                    }

                    if (!zji) {
                        me.controller.mask('Пожалуйста, подождите', win);
                        B4.Ajax.request({
                            url: B4.Url.action('GetDefaultZji', 'AppealCits')
                        }).next(function(result) {
                            me.controller.unmask();
                            var res = Ext.decode(result.responseText);

                            win.down('b4selectfield[name = "ZonalInspection"]').setValue(res.Name);
                        }).error(function(e) {
                            me.controller.unmask();
                        });
                    }
            ...
            onChangeRealityObject: function (field, newValue, oldValue) {
                ...
                if (newValue) {
                    me.controller.params.realityObjectId = newValue.Id;
                } else {
                    me.controller.params.realityObjectId = null;
                }
            },
            onChangeExecutant: function (field, newValue) {
                ...
                if (newValue) {
                    me.controller.params.executantId = newValue.Id;
                } else {
                    me.controller.params.executantId = null;
                }
            },
            onChangeAuthor: function (field, newValue) {
                ...
                if (newValue) {
                    me.controller.params.authorId = newValue.Id;
                } else {
                    me.controller.params.authorId = null;
                }
            },
            onChangeController: function (field, newValue) {
                ...
                if (newValue) {
                    me.controller.params.controllerId = newValue.Id;
                } else {
                    me.controller.params.controllerId = null;
                }
            },
            ...
            onCreateStatement: function (btn) {
                var me = this;
                me.controller.mask('Пожалуйста, подождите', me.componentQuery(me.controller.editWindowSelector));
                me.controller
                    .getAspect("baseStatementAppCitEditWindowAspect")
                    .checkThematics(me.controller.appealCitizensId)
                    .next(function () {
                        // Отличия от основного модуля начинаются тут
                        B4.Ajax.request({
                            url: B4.Url.action('GetAppealCitizenResponder', 'RegionSpecific'),
                                params: {
                            ...
                            .next(function (result) {
                                var res = Ext.decode(result.responseText),
                                    sfContragent = Ext.ComponentQuery.query(me.controller.baseStatementContragentSelector)[0],
                                    sfRealityObject = Ext.ComponentQuery.query(me.controller.baseStatementRealityObjectSelector)[0];

                                me.controller.getAspect("baseStatementAppCitEditWindowAspect").editRecord();

                                if (sfContragent) {
                                    sfContragent.setValue(res.Id);
                                    sfContragent.setRawValue(res.Name);
                                    sfContragent.validate();
                                }
                                
                                if (sfRealityObject) {
                                    sfRealityObject.setValue(res.roId);
                                    sfRealityObject.setRawValue(res.roAddress);
                        ...
                        // Отличия от основного модуля заканчиваются тут
                        // me.controller.getAspect("baseStatementAppCitEditWindowAspect").editRecord();
                    })
                    .error(function (resp) {
                        me.controller.unmask();
            ...
            onChangeDateFromStart: function (field, newValue) {
                var me = this;
                if (newValue) {
                    me.controller.params.dateFromStart = newValue;
                } else {
                    me.controller.params.dateFromStart = null;
                }
            },
            onChangeDateFromEnd: function (field, newValue) {
                var me = this;

                if (newValue) {
                    me.controller.params.dateFromEnd = newValue;
                } else {
                    me.controller.params.dateFromEnd = null;
                }
            },
            onChangeCheckTimeStart: function(field, newValue) {
                var me = this;

                if (newValue) {
                    me.controller.params.checkTimeStart = newValue;
                } else {
                    me.controller.params.checkTimeStart = null;
                }
            },
            onChangeCheckTimeEnd: function (field, newValue) {
                var me = this;

                if (newValue) {
                    me.controller.params.checkTimeEnd = newValue;
                } else {
                    me.controller.params.checkTimeEnd = null;
                }
            },
            onChangeCheckbox: function (field, newValue) {
                var me = this;

                me.controller.params.showCloseAppeals = newValue;
                me.controller.getStore('AppealCits').load();
            },
            onChangeTypeCorrespondent: function (field, newValue) {
                var wnd = Ext.ComponentQuery.query(this.controller.editWindowSelector)[0],
                    stField = wnd.down('#sfSocialStatus');
                
                if (newValue == 10 || newValue == 20 || newValue == 30) {
                    stField.enable();
                } else {
                    stField.setValue(null);
                    stField.disable();
                }
            }
        },
        {
            ...
            */
            ...
            columnsGridSelect: [
                { header: 'Номер обращения', dataIndex: 'NumberGji', flex: 1, filter: { xtype: 'textfield' } },
                { xtype: 'datecolumn', header: 'Дата обращения', format: 'd.m.Y', dataIndex: 'DateFrom', flex: 1, filter: { xtype: 'datefield', format: 'd.m.Y', operand: CondExpr.operands.eq } },
                { header: 'Корреспондент', dataIndex: 'Correspondent', flex: 1, filter: { xtype: 'textfield' } },
                { header: 'Адрес', dataIndex: 'RealObjAddresses', flex: 1, filter: { xtype: 'textfield'} }
            ],
            columnsGridSelected: [
                { header: 'Номер обращения', dataIndex: 'NumberGji', flex: 1, sortable: false },
                { xtype: 'datecolumn', header: 'Дата обращения', format: 'd.m.Y', dataIndex: 'DateFrom', flex: 1 }
            ],
        ...
        },
        {
            ...
            btnAction: function () {
                ...
                if (grid) {
                    ...
                    {
                        if (this.controller.params.dateFromStart)
                        {
                            params.dateFromStart = Ext.Date.toString(this.controller.params.dateFromStart);
                        }
                        if (this.controller.params.dateFromEnd)
                        {
                            params.dateFromEnd = Ext.Date.toString(this.controller.params.dateFromEnd);
                        }
                        if (this.controller.params.checkTimeStart)
                        {
                            params.checkTimeStart = Ext.Date.toString(this.controller.params.checkTimeStart);
                        }
                        if (this.controller.params.checkTimeEnd)
                        {
                            params.checkTimeEnd = Ext.Date.toString(this.controller.params.checkTimeEnd);
                        }
                            params.showCloseAppeals = this.controller.params.showCloseAppeals;
                            params.authorId = this.controller.params.authorId;
                            params.executantId = this.controller.params.executantId;
                            params.controllerId = this.controller.params.controllerId;
                            params.realityObjectId = this.controller.params.realityObjectId;
            ...
            */
            ...
            selectedGridRowActionHandler: function (action, record) {
                ...
                if (gridSelect && gridSelected) {
                    ...
                    var node = gridSelect.store.getNodeById(record.getId());
                    if (node) {
        ...
        },
        {
            ...
            onSaveSuccess: function (asp, record) {
                if (asp.controller.params.AppealCitsAnswerId != record.getId()) {
                    asp.controller.setCurrentAnswerId(record.getId());
                }
            },
            listeners: {
                getdata: function(asp, record) {
                    if (!record.get('Id')) {
                        record.set('AppealCits', asp.controller.appealCitizensId);
                    }
                },
                aftersetformdata: function (asp, record, form) {
                    var me = this,
                        id = record.getId(),
                        win = asp.getForm(),
                        grid = win.down('appealcitsansweraddresseedrid');
                    
                    asp.controller.setCurrentAnswerId(id);
        ...
        {
            /*
            Аспект взаимодействия таблицы Адресат с массовым добавлением
            */
            xtype: 'gkhgridmultiselectwindowaspect',
            name: 'appealCitsAnswerAddresseeAspect',
            modelName: 'appealcits.AnswerAddressee',
            storeName: 'appealcits.AnswerAddressee',
            gridSelector: '#appealcitsAnswerAddresseeGrid',
            multiSelectWindow: 'SelectWindow.MultiSelectWindow',
            multiSelectWindowSelector: '#appealCitsAnswerAddresseeSelectWindow',
            ...
            titleSelectWindow: 'Выбор адресата',
            titleGridSelect: 'Адресаты для отбора',
            titleGridSelected: 'Выбранные адресаты',
            ...
            listeners: {
                getdata: function (asp, records) {
                    ...
                    asp.controller.mask('Сохранение', asp.controller.getMainComponent());
                    B4.Ajax.request(B4.Url.action('AddAddressee', 'AppealCitsAnswerAddressee', {
                        objectIds: Ext.encode(recordIds),
                        answerId: asp.controller.params.AppealCitsAnswerId
                    })).next(function (response) {
                        asp.controller.unmask();
                        asp.controller.getStore(asp.storeName).load();
                        Ext.Msg.alert('Сохранение!', 'Адресаты успешно сохранены');
                        return true;
                    }).error(function () {
                        asp.controller.unmask();
                    });
            ...
            */
            ...
            saveRecord: function (rec) {
                ...
                if (this.fireEvent('beforesave', this, rec) !== false) {
                    ...
                    me.checkThematics(me.controller.appealCitizensId).next(function () {
                        ...
                        var typeForm = Ext.ComponentQuery.query(me.controller.baseStatementTypeForm)[0].getValue();
                        ...
                        B4.Ajax.request({
                            ...
                            switch (typeBase) {
                                case 50:
                                    ...
                                    break;
                                default:
                                    model = me.controller.getModel('BaseStatement');
                                    break;
                            }
                            model.load(inspection.Id, {
                                success: function(recInsp) {
                                    me.fireEvent('savesuccess', me, recInsp);
                                },
                                scope: me
                            });
                            return true;
            ...
            editRecord: function (record) {
                var me = this,
                    ...
                    editForm = Ext.ComponentQuery.query(me.controller.editWindowSelector)[0],
                    typeCorr = editForm.down('combobox[name=TypeCorrespondent]').getValue(),
                    model,
                    portal;

                if (id) {

                    switch (record.get('TypeBase')) {
                        case 50:
                            me.controllerEditName = 'B4.controller.baseprosclaim.Navigation';
                            break;
                        default:
                            me.controllerEditName = 'B4.controller.basestatement.Navigation';
                            break;
                    }
                    ...
                    record = new model({ Id: 0 });

                    //если тип корреспондента = Прокуратура, 
                    //то тип основания проверки по требованию прокуратуры,
                    //иначе - по обращению
                    record.set('TypeBase', typeCorr == 90 ? 50 : 20);
            ...
            checkThematics: function (appealCitizensId) {
                return B4.Ajax.request({
                    url: B4.Url.action('AnyThematics', 'BaseStatement'),
                    params: {
    ...
    setCurrentAnswerId: function(id) {
        var me = this,
            store = me.getStore('appealcits.AnswerAddressee');
        
        me.params.AppealCitsAnswerId = id;
    ...
    setCurrentId: function(id, numberGji) {
        ...
        var storeAppealCitsExecutant = this.getStore('appealcits.AppealCitsExecutant');
    ...
    init: function() {
        ...
        this.getStore('appealcits.AnswerAddressee').on('beforeload', this.onAddresseeBeforeLoad, this);
    ...
    */
    ...
    onAddresseeBeforeLoad: function (store, operation) {
        operation.params.answerId = this.params.AppealCitsAnswerId;
    },
