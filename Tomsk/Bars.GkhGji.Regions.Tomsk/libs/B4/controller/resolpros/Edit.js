Ext.define('B4.controller.resolpros.Edit', {
    ...
    aspects: [
        ...
        },
        {
            ...
            */
            ...
            editFormSelector: 'resolprosdefwin',
            storeName: 'resolpros.Definition',
            modelName: 'resolpros.Definition',
            editWindowView: 'resolpros.DefinitionEditWindow',
            otherActions: function(actions) {
                actions['resolprosdefwin combobox[name=TypeResolProsDefinition]'] = {
                    'change': { fn: this.onChangeTypeDefinition, scope: this }
                };
            },
            onChangeTypeDefinition: function(cmp) {
                var fldReturnReason = cmp.up('resolprosdefwin').down('textfield[name=ReturnReason]');
                
                if(cmp.getValue() == 10) {
                    fldReturnReason.setDisabled(false);
                } else {
                    fldReturnReason.setDisabled(true);
                }
            },
    ...
    init: function () {
        this.control({
            'resolprosdefwin combobox[name="TypeResolProsDefinition"]': {
                'change': { fn: this.onTypeResolProsDefChange, scope: this }
            }
        });
    ...
    onTypeResolProsDefChange: function (cbx, newValue) {

        var resolprosdefwin = cbx.up('resolprosdefwin'),
            requestNeedField = resolprosdefwin.down('textfield[name="RequestNeed"]'),
            resolutionInitAdminViolationField = resolprosdefwin.down('textfield[name="ResolutionInitAdminViolation"]'),
            returnReasonField = resolprosdefwin.down('textfield[name="ReturnReason"]'),
            executionDateField = resolprosdefwin.down('datefield[name="ExecutionDate"]'),
            executionTimeField = resolprosdefwin.down('timefield[name="ExecutionTime"]'),
            additionalDocumentsField = resolprosdefwin.down('textfield[name="AdditionalDocuments"]'),
            dfDateSubmissionDoc = resolprosdefwin.down('[name=DateSubmissionDocument]');
        
        requestNeedField.enable();
        resolutionInitAdminViolationField.enable();
        returnReasonField.enable();
        dfDateSubmissionDoc.enable();
        executionDateField.enable();
        executionTimeField.enable();
        additionalDocumentsField.enable();
        
        switch (newValue) {
            case 10:  // Определение о возврате в прокуратуру
                dfDateSubmissionDoc.disable();
                additionalDocumentsField.disable();
                executionDateField.disable();
                executionTimeField.disable();
                requestNeedField.disable();
                break;
            case 20:  // Определение о назначении даты и времени
                dfDateSubmissionDoc.disable();
                additionalDocumentsField.disable();
                resolutionInitAdminViolationField.disable();
                returnReasonField.disable();
                requestNeedField.disable();
                break;

            case 30:  // Определение об отложении
                returnReasonField.disable();
                break;
        }
    }
