Ext.define('B4.controller.Prescription', {
    ...
    aspects: [
        ...
        },
        {
            ...
            */
            ...
            onAfterSetPanelData: function (asp, rec, panel) {
                ...
                asp.controller.getStore('prescription.Annex').load();
                asp.controller.getStore('prescription.ArticleLaw').load();
                asp.controller.getStore('prescription.Cancel').load();
                asp.controller.getStore('prescription.CloseDoc').load();
                asp.controller.getAspect('prescriptionPrintAspect').loadReportStore();

                //обновляем статусы
                asp.controller.getAspect('prescriptionStateButtonAspect').setStateData(rec.get('Id'), rec.get('State'));

                // обновляем кнопку Сформирвоать
                asp.controller.getAspect('prescriptionCreateButtonAspect').setData(rec.get('Id'));
        ...
        },
        {
            ...
            */
            ...
            saveButtonSelector: '#prescriptionViolationGrid #prescriptionViolationSaveButton',
        ...
        },
        {
            ...
            */
            ...
            columnsGridSelect: [
                { header: 'Нарушение', xtype: 'gridcolumn', dataIndex: 'ViolationGji', flex: 1, filter: { xtype: 'textfield' } },
                { header: 'Подробнее', xtype: 'gridcolumn', dataIndex: 'Description', flex: 1, filter: { xtype: 'textfield' } },
    ...
    onLaunch: function () {
        var me = this,
            ...
            closePanel = mainView.down('prescrclosepanel');

        if (me.params) {
            me.getAspect('prescriptionEditPanelAspect').setData(me.params.documentId);
            closePanel.down('b4enumcombo[name=Closed]').on('change', me.changeClosedCombo, me);
        }
    },
    ...
    onBeforeLoadRealityObjViol: function (store, operation) {
        ...
        if (rec) {
            ...
            this.params.realObjId = rec.getId();
            violGrid.setTitle(rec.get('RealityObject'));
        }
    },

    changeClosedCombo: function (combo, newVal) {
        var me = this,
            parent = combo.up('prescrclosepanel'),
            reasonCombo = parent.down('b4enumcombo[name=CloseReason]'),
            noteText = parent.down('textarea'),
            grid = parent.down('b4grid'),
            permissions = [
                'GkhGji.DocumentsGji.Prescription.Field.Close.Reason_Edit',
                'GkhGji.DocumentsGji.Prescription.Field.Close.Note_Edit'
            ];

        if (newVal != B4.enums.YesNoNotSet.Yes) {
            ...
            B4.Ajax.request({
                ...
                url: B4.Url.action('GetObjectSpecificPermissions', 'Permission', {
                    permissions: Ext.encode(permissions),
                    ids: Ext.encode([me.params.documentId])
                })
            }).next(function (response) {
                me.unmask();
                var permArray = Ext.decode(response.responseText)[0],
                    permReasonEdit = permArray[0],
                    permNoteEdit = permArray[1];

                reasonCombo.setDisabled(!permReasonEdit);
                noteText.setDisabled(!permNoteEdit);
