    ...
    public class InspectionReminderRule : IReminderRule
    {
            ...
            try
            {
                ...
                if (obj.State == null || (obj.State != null && !obj.State.FinalState))
                {
                    ...
                    else
                    {
                        ...
                        // 4 проверка. Получаем невалидные протоколы
                        var listProtocolsNotValid = this.GetRemindersIfProtocolNotValid(obj);
                        if (listProtocolsNotValid.Any())
                        {
                            listRemindersToSave.AddRange(listProtocolsNotValid);
                        }
                    }
                    ...
                    {
                        var newReminder = listRemindersToSave
                            .FirstOrDefault(
        ...
        private List<Reminder> GetRemindersIfInspectionNotValid(InspectionGji obj)
        {
            ...
            try
            {
                ...
                if (!servDocumentGji.GetAll().Any(x => x.Inspection.Id == obj.Id))
                {
                    ...
                    foreach (var id in inspectionInspectors)
                    {
                        var rem = new Reminder
                        {
                            Actuality = true,
        ...
        private List<Reminder> GetRemindersIfDisposalNotValid(InspectionGji obj)
        {
            ...
            try
            {
                ...
                foreach (var id in listIds.Distinct().ToList())
                {
        ...
        private List<long> GetDisposalWhereActsNotValid(InspectionGji obj)
        {
            ...
            try
            {
                ...
                var listActCheckYes =
                    listActCheckRO
                        .Where(x => x.HaveViolation == YesNoNotSet.Yes)
        ...
        /// <summary>
        /// Метод получения напоминалок, если из протокола не сформировано постановление,
        ///       то необходимо создать напоминание для каждого инспектора в протоколах
        /// </summary>
        private List<Reminder> GetRemindersIfProtocolNotValid(InspectionGji obj)
        {
            var servDocumentGjiInspectors = Container.Resolve<IDomainService<DocumentGjiInspector>>();
            ...
            try
            {
                ...
                var dictInspectors = servDocumentGjiInspectors.GetAll()
                                             .Where(x => x.DocumentGji.Inspection.Id == obj.Id)
                                             .Where(x => x.DocumentGji.TypeDocumentGji == TypeDocumentGji.Protocol)
                                             .Where(x => x.Inspector != null)
                                             .Select(x => new
                                             {
                                                 x.Id,
                ...
                // Если нет инспекторов значит нет сысла проверять Протоколы
                if (!dictInspectors.Any())
                    return result;

             
                // Теперь получаем протоколы у которых нет дочерних элементов Постановление
                var protocols = servProtocol.GetAll()
                                .Where(x => x.Inspection.Id == obj.Id)
                                .Where(x => !servDocumentGjiChildren.GetAll()
                                                .Any(y => y.Parent.Id == x.Id
                                                            && y.Children.TypeDocumentGji == TypeDocumentGji.Resolution))
                                .Select(x => new
                                {
                                    x.Id,
                                    ContragentId = x.Inspection.Contragent != null ? x.Inspection.Contragent.Id : 0,
                ...
                {
                    if (!dictInspectors.ContainsKey(protocol.Id))
                        continue;

                    var di = dictInspectors[protocol.Id];
                    ...
                    foreach (var inspectorId in di.Distinct())
                    {
                        ...
                        {
                            Actuality = true,
                            TypeReminder = TypeReminder.Protocol,
                            CategoryReminder = GetCategoryReminder(protocol.TypeBase),
                            Num = protocol.DocumentNumber,
                            DocumentGji = new DocumentGji { Id = protocol.Id },
                            Contragent = protocol.ContragentId > 0 ? new Contragent { Id = protocol.ContragentId } : null,
                            Inspector = new Inspector { Id = inspectorId },
                            InspectionGji = obj
                        };
