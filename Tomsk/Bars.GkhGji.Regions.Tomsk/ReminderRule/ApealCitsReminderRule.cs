            ...
            if (entity is AppealCits)
            {
                ...
                try
                {
                    ...
                    if (!obj.State.FinalState)
                    {
	                    ...
	                    var listInspectors = servAppCitsExecutant.GetAll()
		                    .Where(x => x.AppealCits.Id == obj.Id)
		                    .Select(x => new
		                    {
			                    AuthorId = x.Author != null ? x.Author.Id : 0,
			                    ExecutantId = x.Executant != null ? x.Executant.Id : 0,
			                    ControllerId = x.Controller != null ? x.Controller.Id : 0
		                    })
		                    .AsEnumerable()
		                    .SelectMany(x => new[] {x.AuthorId, x.ExecutantId, x.ControllerId})
		                    .Where(x => x > 0)
		                    .Distinct()
		                    .ToArray();
                        
                        // Если статус не Закрыто то выполняем действия для создания напоминалок
						foreach (var idIns in listInspectors)
                        {
