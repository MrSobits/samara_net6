...
{
    public class DocGjiValidationNumberRule : IRuleChangeStatus
    {
        ...
        public string Id { get { return "gji_document_validation_number_rule"; } }

        public string Name { get { return "Проверка возможности формирования номера документа ГЖИ Томска"; } }

        public string TypeId { get { return "gji_document"; } }

        public string Description { get { return "Данное правило проверяет формирование номера в соответствии с правилами Томска"; } }
        ...
        public ValidateResult Validate(IStatefulEntity statefulEntity, State currentState, State newState)
        {
            ...
            if (statefulEntity is DocumentGji)
            {
                ...
                if ((document is ActCheck) && (document as ActCheck).TypeActCheck == TypeActCheckGji.ActCheckDocumentGji)
                {
                    ...
                    var parentDocument = DocumentChildrenDomain.GetAll()
                    ...
                    var childrenDocumentCount = DocumentChildrenDomain.GetAll()
        ...
        private void Action(DocumentGji document)
        {
            ...
            if (document.TypeDocumentGji == TypeDocumentGji.Disposal)
            {
                ...
                var maxNum = DocumentDomain.GetAll()
                    .Where(x =>
                    ...
                    || document.TypeDocumentGji == TypeDocumentGji.Prescription)
            {
                ...
                document.DocumentNum = DocumentDomain.GetAll()
                ...
                if (!DocumentDomain.GetAll().Any(x => x.Stage.Id == document.Stage.Id && x.Id != document.Id))
                {
                ...
                {
                    document.DocumentSubNum = DocumentDomain.GetAll()
