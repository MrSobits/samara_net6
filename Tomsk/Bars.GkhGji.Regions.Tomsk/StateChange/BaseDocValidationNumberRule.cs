        ...
        public ValidateResult Validate(IStatefulEntity statefulEntity, State currentState, State newState)
        {
            ...
            if (statefulEntity is DocumentGji)
            {
                ...
                if ((document is ActCheck) && (document as ActCheck).TypeActCheck == TypeActCheckGji.ActCheckDocumentGji)
                {
                    ...
                    var parentDocument = this.Container.Resolve<IDomainService<DocumentGjiChildren>>().GetAll()
                    ...
                    if (parentDocument != null)
                    {
                        result.Message = string.Format(
                            ...
                            ((Enums.TypeDocumentGji)parentDocument.TypeDocumentGji).GetEnumMeta().Display,
                            parentDocument.DocumentDate.ToDateTime().ToShortDateString());
                        result.Success = false;
                        return result;
                    }

                    var childrenDocumentCount = 
                        Container.Resolve<IDomainService<DocumentGjiChildren>>().GetAll()
                            ...
                            .Count(x => x.Parent.Id == document.Id);
                    ...
                    else
                    {
                        document.DocumentSubNum = 
                            documentService.GetAll()
                                .Where(x => x.Stage.Id == document.Stage.Id 
                                            && x.Id != document.Id)
