...
{
    public class ExportPlanService : IExportPlanService
    {
        #region Constants
        ...
        public ExportPlanService(IWindsorContainer container, IFileManager fileManager, ITaskManager taskManager)
        {
        ...
        public void SaveRequest(GisGkhRequests req, string[] reqParams)
        {
            try
            {
                var request = HcsCapitalRepairAsync.exportPlanReq(reqParams);
                ...
                try
                {
                    ...
                    if (fileInfo != null)
                    {
                        ...
                        foreach (var responseItem in response.Items)
                        {
                            ...
                            else if (responseItem is ExportPlanPassportType)
                            {

                                var regProg = (ExportPlanPassportType)responseItem;
                                ...
                                if (regProg.Type == PlanPassportTypeType.Plan && (regProg.Status == StatusExtendedType.Published || regProg.Status == StatusExtendedType.PublishedAndProject))
                                {
                                    ...
                                    ProgramCrRepo.GetAll()
                                        ...
                                        (x.TypeProgramStateCr == TypeProgramStateCr.New || x.TypeProgramStateCr == TypeProgramStateCr.Open))
