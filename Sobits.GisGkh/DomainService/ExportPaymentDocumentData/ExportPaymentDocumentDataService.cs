...
{
    public class ExportPaymentDocumentDataService : IExportPaymentDocumentDataService
    {
        #region Constants
        ...
        public ExportPaymentDocumentDataService(IWindsorContainer container, IFileManager fileManager, ITaskManager taskManager)
        {
        ...
        public void SaveRequest(GisGkhRequests req, string roId, string chargePeriodId)
        {
            try
            {
                ...
                var accGuids = BasePersonalAccountRepo.GetAll()
                    .Where(x => x.Room.RealityObject == house)
                    .Where(x => x.GisGkhGuid != null && x.GisGkhGuid != "")
                    .Take(1000)
                    .Select(x => x.GisGkhGuid)
                    .ToList();
                var chargePeriod = ChargePeriodDomain.Get(long.Parse(chargePeriodId));

                var request = BillServiceAsync.exportPaymentDocumentDataReq((short)chargePeriod.StartDate.Year,
                                        chargePeriod.StartDate.Month, accGuids);
            ...
            try
            {
                ...
                if (response.RequestState == 3)
                {
                    ...
                    try
                    {
                        ...
                        {
                            req.Answer = $"Задача на обработку ответа exportPaymentDocumentData поставлена в очередь с id {taskInfo.TaskId}";
                            GisGkhRequestsDomain.Update(req);
                        }
                    }
                    catch (Exception e)
                ...
                try
                {
                    ...
                    if (fileInfo != null)
                    {
                        ...
                        foreach (var responseItem in response.Items)
                        {
                            ...
                            else if (responseItem is exportPaymentDocumentResultType)
                            {
                                exportPaymentDocumentResultType exportPaymentDocumentResult = (exportPaymentDocumentResultType)responseItem;
                                if (exportPaymentDocumentResult.Item is ErrorMessageType)
                                {

                                }
                                else if (exportPaymentDocumentResult.Item is exportPaymentDocumentResultTypePaymentDocument)
                                {
                                    exportPaymentDocumentResultTypePaymentDocument doc = (exportPaymentDocumentResultTypePaymentDocument)exportPaymentDocumentResult.Item;
                                    if (doc.ItemElementName == ItemChoiceType2.Expose && doc.Item == true)
                                    {
                                        var account = BasePersonalAccountRepo.GetAll()
                                            .Where(x => x.GisGkhGuid == doc.AccountGuid).FirstOrDefault();
                                        var chargePeriod = ChargePeriodDomain.GetAll()
                                            .Where(x => (short)x.StartDate.Year == doc.Year && x.StartDate.Month == doc.Month)
                                            .FirstOrDefault();
                                        if (account != null && chargePeriod != null)
                                        {
                                            foreach (var item in doc.Items)
                                            {
                                                if (item is PaymentDocumentExportTypeCapitalRepairCharge)
                                                {
                                                    GisGkhPayDoc payDoc = GisGkhPayDocDomain.GetAll()
                                                        .Where(x => x.Period == chargePeriod && x.Account == account).FirstOrDefault();
                                                    if (payDoc != null)
                                                    {
                                                        payDoc.PaymentDocumentID = doc.PaymentDocumentID;
                                                        GisGkhPayDocDomain.Update(payDoc);
                                                    }
                                                    else
                                                    {
                                                        payDoc = new GisGkhPayDoc
                                                        {
                                                            Account = account,
                                                            Period = chargePeriod,
                                                            PaymentDocumentID = doc.PaymentDocumentID
                                                        };
                                                        GisGkhPayDocDomain.Save(payDoc);
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        {
                                            //нет счёта или периода
                                        }
                                    }
                                    else
                                    {
                                        //счёт из ГИС ЖКХ не в статусе "Опубликован"
                                    }
                                }
                            }
                        }
