namespace Bars.Gkh.ExecutionAction.Impl
{
    ...
    /// </summary>
    public class GkhFiasAddressCorrectAction : BaseExecutionAction
    {
        /// <summary>
        /// Контейнер
        /// </summary>
        ...
        /// Название действия
        /// </summary>
        public override string Name => "ЖилФонд - Корректировка адресов Контрагентов и Жилых домов Относительно МО";
        ...
        private BaseDataResult Execute()
        {
            ...
            */
            ...
            var useFiasIdentification = configProvider.Get<GeneralConfig>().UseFiasHouseIdentification;
            ...
            try
            {
                ...
                var fiasData = fiasAddrRepo.GetAll()
                    ...
                    .WhereNotEmptyString(x => x.StreetGuidId)
                    .Where(
                ...
                foreach (var fiasAddress in fiasData)
                {
                    ...
                    if (useFiasIdentification == UseFiasHouseIdentification.Use && fiasAddress.HouseGuid != null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            addressName.Append($", лит. {fiasAddress.Letter}");
                        }
                        ...
                        {
                            addressName.Append($", секц. {fiasAddress.Building}");
                        }
                    }
                    ...
                    if (fiasAddress.AddressName != addressName.ToString() || (!string.IsNullOrEmpty(placeAdrName) && fiasAddress.PlaceAddressName != placeAdrName))
                    {
