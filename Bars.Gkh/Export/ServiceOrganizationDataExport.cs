namespace Bars.Gkh.Export
{
    ...
    public class ServiceOrganizationDataExport : BaseDataExportService
    {
        public override IList GetExportData(BaseParams baseParams)
        {
            var loadParam = GetLoadParam(baseParams);
            
            /* Галочка показывать актуальные */
            var showNotValid = baseParams.Params.ContainsKey("showNotValid") && baseParams.Params["showNotValid"].ToBool();

            return this.Container.Resolve<IDomainService<ServiceOrganization>>().GetAll()
