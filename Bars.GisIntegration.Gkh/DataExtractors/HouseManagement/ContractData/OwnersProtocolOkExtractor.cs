namespace Bars.GisIntegration.Gkh.DataExtractors.HouseManagement.ContractData
{
    ...
    /// <summary>
    /// Экстрактор для протоколов открытого конкурса ДУ собственников
    /// </summary>
    public class OwnersProtocolOkExtractor : BaseDataExtractor<RisProtocolOk, FileInfo>
    {
        private List<RisContract> contracts;
        private Dictionary<long, RisContract> contractsById;
        private Dictionary<long, ManOrgContractOwners> contractByFileId;
        ...
        {
            var selectedContractIds = this.contracts?.Select(x => x.ExternalSystemEntityId).ToArray() ?? new long[0];
            var manOrgContractDomain = this.Container.ResolveDomain<ManOrgContractOwners>();
        ...
        protected override void UpdateRisEntity(FileInfo externalEntity, RisProtocolOk risEntity)
        {
            ...
            try
            {
                if (externalEntity != null)
                {
                    ...
                    catch (FileNotFoundException)
                    {
                        ...
                        this.Log.Add(
                            new BaseLogRecord(MessageType.Info, "Файл протокола открытого конкурса ДУ собственников номер " + contract.DocumentNumber + " не найден"));
                    }
                }
            }
            finally
